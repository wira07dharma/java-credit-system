...
public class PstAgunanMapping extends DBHandler implements I_DBInterface, I_DBType, I_PersintentExc, I_Language {
   ...
   public static JSONObject fetchJSON(long oid){
      ...
      try {
         AgunanMapping agunanMapping = PstAgunanMapping.fetchExc(oid);
         object.put(PstAgunanMapping.fieldNames[PstAgunanMapping.FLD_AGUNAN_MAPPING_ID], agunanMapping.getOID());
         object.put(PstAgunanMapping.fieldNames[PstAgunanMapping.FLD_AGUNAN_ID], agunanMapping.getAgunanId());
         object.put(PstAgunanMapping.fieldNames[PstAgunanMapping.FLD_PINJAMAN_ID], agunanMapping.getPinjamanId());
         object.put(PstAgunanMapping.fieldNames[PstAgunanMapping.FLD_PROSENTASE_PINJAMAN], agunanMapping.getProsentasePinjaman());
      }catch(Exception exc){}
      return object;
   }
    public static long syncExc(JSONObject jSONObject){
      long oid = 0;
      if (jSONObject != null){
       oid = jSONObject.optLong(PstAgunanMapping.fieldNames[PstAgunanMapping.FLD_AGUNAN_MAPPING_ID],0);
         if (oid > 0){
          AgunanMapping agunanMapping = new AgunanMapping();
          agunanMapping.setOID(jSONObject.optLong(PstAgunanMapping.fieldNames[PstAgunanMapping.FLD_AGUNAN_MAPPING_ID],0));
          agunanMapping.setAgunanId(jSONObject.optLong(PstAgunanMapping.fieldNames[PstAgunanMapping.FLD_AGUNAN_ID],0));
          agunanMapping.setPinjamanId(jSONObject.optLong(PstAgunanMapping.fieldNames[PstAgunanMapping.FLD_PINJAMAN_ID],0));
          agunanMapping.setProsentasePinjaman(jSONObject.optDouble(PstAgunanMapping.fieldNames[PstAgunanMapping.FLD_PROSENTASE_PINJAMAN],0));
         boolean checkOidAgunanMapping = PstAgunanMapping.checkOID(oid);
          try{
            if(checkOidAgunanMapping){
               PstAgunanMapping.updateExc(agunanMapping);
            }else{
               PstAgunanMapping.insertByOid(agunanMapping);
            }
         }catch(Exception exc){}
         }
      }
   ...
   public static long insertByOid(AgunanMapping agunanMapping) throws DBException {
      try {
         PstAgunanMapping pstAgunanMapping = new PstAgunanMapping(0);
         pstAgunanMapping.setLong(FLD_AGUNAN_MAPPING_ID, agunanMapping.getOID());
         pstAgunanMapping.setLong(FLD_AGUNAN_ID, agunanMapping.getAgunanId());
         pstAgunanMapping.setLong(FLD_PINJAMAN_ID, agunanMapping.getPinjamanId());
         pstAgunanMapping.setDouble(FLD_PROSENTASE_PINJAMAN, agunanMapping.getProsentasePinjaman());
         pstAgunanMapping.insertByOid(agunanMapping.getOID());
      } catch (DBException dbe) {
         throw dbe;
      } catch (Exception e) {
         throw new DBException(new PstAgunanMapping(0), DBException.UNKNOWN);
      }
      return agunanMapping.getOID();
   }
}
