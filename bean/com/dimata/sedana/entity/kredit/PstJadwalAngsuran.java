...
public class PstJadwalAngsuran extends DBHandler implements I_DBInterface, I_DBType, I_PersintentExc, I_Language {
    ...
    public static double sumJadwalAngsuran(long pinjamanId, String inJenisAngsuran){
        double total = 0;
        DBResultSet dbrs = null;
        try {
            String sql = "SELECT SUM("+fieldNames[FLD_JUMLAH_ANGSURAN]+") AS JUMLAH "
                    + " FROM " + TBL_JADWALANGSURAN
                    + " WHERE " + fieldNames[FLD_PINJAMAN_ID] + " = " + pinjamanId
                    + " AND " + fieldNames[FLD_JENIS_ANGSURAN] + " IN ("+inJenisAngsuran+")";
            dbrs = DBHandler.execQueryResult(sql);
            ResultSet rs = dbrs.getResultSet();
            while (rs.next()) {
    ...
    public static double sumJadwalAngsuranDibayar(long pinjamanId, String inJenisAngsuran){
        double total = 0;
        DBResultSet dbrs = null;
        try {
            String sql = "SELECT SUM(ags."+PstAngsuran.fieldNames[PstAngsuran.FLD_JUMLAH_ANGSURAN]+") AS DIBAYAR "
                    + " FROM " + TBL_JADWALANGSURAN +" as j "
                    + " INNER JOIN " + PstAngsuran.TBL_ANGSURAN+" as ags "
                    + " ON j."+fieldNames[FLD_JADWAL_ANGSURAN_ID]+" = ags."+PstAngsuran.fieldNames[PstAngsuran.FLD_JADWAL_ANGSURAN_ID]
                    + " WHERE j." + fieldNames[FLD_PINJAMAN_ID] + " = " + pinjamanId
                    + " AND j." + fieldNames[FLD_JENIS_ANGSURAN] + " IN ("+inJenisAngsuran+")";
            dbrs = DBHandler.execQueryResult(sql);
            ResultSet rs = dbrs.getResultSet();
            while (rs.next()) {
                total = rs.getDouble("DIBAYAR");
            }
            rs.close();
        } catch (Exception e) {
            System.out.println(e);
    ...
    public static int countAngsuranDibayar(long pinjamanId){
        int count = 0;
        DBResultSet dbrs = null;
        try {
            String sql = "SELECT COUNT(TANGGAL_ANGSURAN) AS JML FROM "
                    + "(SELECT "
                    + " j."+fieldNames[FLD_JADWAL_ANGSURAN_ID]
                    + ", j."+fieldNames[FLD_TANGGAL_ANGSURAN]
                    + ", SUM(j."+fieldNames[FLD_JUMLAH_ANGSURAN]+") AS JUMLAH_ANGSURAN"
                    + ", SUM(ags."+PstAngsuran.fieldNames[PstAngsuran.FLD_JUMLAH_ANGSURAN]+") AS dibayar"
                    + " FROM " + TBL_JADWALANGSURAN +" as j "
                    + " INNER JOIN " + PstAngsuran.TBL_ANGSURAN+" as ags "
                        + " ON j."+fieldNames[FLD_JADWAL_ANGSURAN_ID]+" = ags."+PstAngsuran.fieldNames[PstAngsuran.FLD_JADWAL_ANGSURAN_ID]
                    + " WHERE j." + fieldNames[FLD_PINJAMAN_ID] + " = " + pinjamanId
                    + " GROUP BY j." + fieldNames[FLD_TANGGAL_ANGSURAN]
                    + " HAVING ("+fieldNames[FLD_JUMLAH_ANGSURAN]+" - dibayar) = 0) as data";
            dbrs = DBHandler.execQueryResult(sql);
            ResultSet rs = dbrs.getResultSet();
            while (rs.next()) {
                count = rs.getInt("JML");
