...
public class PstKategoriKredit extends DBHandler implements I_DBInterface, I_DBType, I_PersintentExc, I_Language {
  ...
  public static int findLimitCommand(int start, int recordToGet, int vectSize) {
   ...
   public static JSONObject fetchJSON(long oid){
      ...
      try {
         KategoriKredit kategoriKredit = PstKategoriKredit.fetchExc(oid);
         object.put(PstKategoriKredit.fieldNames[PstKategoriKredit.FLD_KATEGORI_KREDIT_ID], kategoriKredit.getOID());
         object.put(PstKategoriKredit.fieldNames[PstKategoriKredit.FLD_KATEGORY_KREDIT], kategoriKredit.getKategoryKredit());
         object.put(PstKategoriKredit.fieldNames[PstKategoriKredit.FLD_KETERANGAN], kategoriKredit.getKeterangan());
      }catch(Exception exc){}
      return object;
   }
      ...
      long oid = 0;
      if (jSONObject != null){
       oid = jSONObject.optLong(PstKategoriKredit.fieldNames[PstKategoriKredit.FLD_KATEGORI_KREDIT_ID],0);
         if (oid > 0){
          KategoriKredit kategoriKredit = new KategoriKredit();
          kategoriKredit.setOID(jSONObject.optLong(PstKategoriKredit.fieldNames[PstKategoriKredit.FLD_KATEGORI_KREDIT_ID],0));
          kategoriKredit.setKategoryKredit(jSONObject.optString(PstKategoriKredit.fieldNames[PstKategoriKredit.FLD_KATEGORY_KREDIT], ""));
          kategoriKredit.setKeterangan(jSONObject.optString(PstKategoriKredit.fieldNames[PstKategoriKredit.FLD_KETERANGAN], ""));
         boolean checkOidKategoriKredit = PstKategoriKredit.checkOID(oid);
          try{
            if(checkOidKategoriKredit){
               PstKategoriKredit.updateExc(kategoriKredit);
            }else{
               PstKategoriKredit.insertByOid(kategoriKredit);
            }
         }catch(Exception exc){}
         }
      }
   ...
   public static long insertByOid(KategoriKredit kategoriKredit) throws DBException {
      try {
         PstKategoriKredit pstKategoriKredit = new PstKategoriKredit(0);
         pstKategoriKredit.setLong(FLD_KATEGORI_KREDIT_ID, kategoriKredit.getOID());
         pstKategoriKredit.setString(FLD_KATEGORY_KREDIT, kategoriKredit.getKategoryKredit());
         pstKategoriKredit.setString(FLD_KETERANGAN, kategoriKredit.getKeterangan());
         pstKategoriKredit.insertByOid(kategoriKredit.getOID());
      } catch (DBException dbe) {
         throw dbe;
      } catch (Exception e) {
         throw new DBException(new PstKategoriKredit(0), DBException.UNKNOWN);
      }
      return kategoriKredit.getOID();
   }
}
