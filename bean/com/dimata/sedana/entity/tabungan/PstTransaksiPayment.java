  ...
  public static int findLimitCommand(int start, int recordToGet, int vectSize) {
    ...
    return cmd;
     ...
     public static JSONObject fetchJSON(long oid){
      ...
      try {
         TransaksiPayment transaksiPayment = PstTransaksiPayment.fetchExc(oid);
         object.put(PstTransaksiPayment.fieldNames[PstTransaksiPayment.FLD_TRANSAKSI_PAYMENT_ID], transaksiPayment.getOID());
         object.put(PstTransaksiPayment.fieldNames[PstTransaksiPayment.FLD_PAYMENT_SYSTEM_ID], transaksiPayment.getPaymentSystemId());
         object.put(PstTransaksiPayment.fieldNames[PstTransaksiPayment.FLD_TRANSAKSI_ID], transaksiPayment.getTransaksiId());
         object.put(PstTransaksiPayment.fieldNames[PstTransaksiPayment.FLD_JUMLAH], transaksiPayment.getJumlah());
         object.put(PstTransaksiPayment.fieldNames[PstTransaksiPayment.FLD_CARD_NAME], transaksiPayment.getCardName());
         object.put(PstTransaksiPayment.fieldNames[PstTransaksiPayment.FLD_CARD_NO], transaksiPayment.getCardNo());
         object.put(PstTransaksiPayment.fieldNames[PstTransaksiPayment.FLD_BANK_NAME], transaksiPayment.getBankName());
         object.put(PstTransaksiPayment.fieldNames[PstTransaksiPayment.FLD_VALIDATE_DATE], transaksiPayment.getValidateDate());
      }catch(Exception exc){}
      return object;
   }
   ...
   public static long syncExc(JSONObject jSONObject){
      long oid = 0;
      if (jSONObject != null){
       oid = jSONObject.optLong(PstTransaksiPayment.fieldNames[PstTransaksiPayment.FLD_TRANSAKSI_PAYMENT_ID],0);
         if (oid > 0){
          TransaksiPayment transaksiPayment = new TransaksiPayment();
          transaksiPayment.setOID(jSONObject.optLong(PstTransaksiPayment.fieldNames[PstTransaksiPayment.FLD_TRANSAKSI_PAYMENT_ID],0));
          transaksiPayment.setPaymentSystemId(jSONObject.optLong(PstTransaksiPayment.fieldNames[PstTransaksiPayment.FLD_PAYMENT_SYSTEM_ID],0));
          transaksiPayment.setTransaksiId(jSONObject.optLong(PstTransaksiPayment.fieldNames[PstTransaksiPayment.FLD_TRANSAKSI_ID],0));
          transaksiPayment.setJumlah(jSONObject.optLong(PstTransaksiPayment.fieldNames[PstTransaksiPayment.FLD_JUMLAH],0));
          transaksiPayment.setCardName(jSONObject.optString(PstTransaksiPayment.fieldNames[PstTransaksiPayment.FLD_CARD_NAME], ""));
          transaksiPayment.setCardNo(jSONObject.optString(PstTransaksiPayment.fieldNames[PstTransaksiPayment.FLD_CARD_NO], ""));
          transaksiPayment.setBankName(jSONObject.optString(PstTransaksiPayment.fieldNames[PstTransaksiPayment.FLD_BANK_NAME], ""));
          transaksiPayment.setValidateDate(Formater.formatDate(jSONObject.optString(PstTransaksiPayment.fieldNames[PstTransaksiPayment.FLD_VALIDATE_DATE],"0000-00-00"),"yyyy-MM-dd"));
         boolean checkOidTransaksiPayment = PstTransaksiPayment.checkOID(oid);
          try{
            if(checkOidTransaksiPayment){
               PstTransaksiPayment.updateExc(transaksiPayment);
            }else{
               PstTransaksiPayment.insertByOid(transaksiPayment);
            }
         }catch(Exception exc){}
         }
      }
   ...
   public static long insertByOid(TransaksiPayment transaksiPayment) throws DBException {
      try {
         PstTransaksiPayment pstTransaksiPayment = new PstTransaksiPayment(0);
         pstTransaksiPayment.setLong(FLD_TRANSAKSI_PAYMENT_ID, transaksiPayment.getOID());
         pstTransaksiPayment.setLong(FLD_PAYMENT_SYSTEM_ID, transaksiPayment.getPaymentSystemId());
         pstTransaksiPayment.setLong(FLD_TRANSAKSI_ID, transaksiPayment.getTransaksiId());
         pstTransaksiPayment.setDouble(FLD_JUMLAH, transaksiPayment.getJumlah());
         pstTransaksiPayment.setString(FLD_CARD_NAME, transaksiPayment.getCardName());
         pstTransaksiPayment.setString(FLD_CARD_NO, transaksiPayment.getCardNo());
         pstTransaksiPayment.setString(FLD_BANK_NAME, transaksiPayment.getBankName());
         pstTransaksiPayment.setDate(FLD_VALIDATE_DATE, transaksiPayment.getValidateDate());
         pstTransaksiPayment.insertByOid(transaksiPayment.getOID());
      } catch (DBException dbe) {
         throw dbe;
      } catch (Exception e) {
         throw new DBException(new PstTransaksiPayment(0), DBException.UNKNOWN);
      }
      return transaksiPayment.getOID();
   }
}
