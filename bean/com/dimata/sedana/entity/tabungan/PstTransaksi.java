...
public class PstTransaksi extends DBHandler implements I_DBInterface, I_DBType, I_PersintentExc, I_Language {
    ...
    public static final int FLD_APP_USER_ID = 13;
    ...
    public static Transaksi fetchExc(long oid) throws DBException {
        try {
            ...
            entTransaksi.setAppUserId(pstTransaksi.getlong(FLD_APP_USER_ID));
            return entTransaksi;
    ...
    public static synchronized long updateExc(Transaksi entTransaksi) throws DBException {
        try {
            if (entTransaksi.getOID() != 0) {
                ...
                if(entTransaksi.getAppUserId() != 0){
                    pstTransaksi.setLong(FLD_APP_USER_ID, entTransaksi.getAppUserId());
                }
                pstTransaksi.update();
                return entTransaksi.getOID();
            }
    ...
    public static void resultToObject(ResultSet rs, Transaksi entTransaksi) {
        try {
            ...
            entTransaksi.setAppUserId(rs.getLong(fieldNames[FLD_APP_USER_ID]));
        } catch (Exception e) {
        }
    }
    ...
    public static ArrayList getDetaiBungaTabunganPerTanggal(String date) {
     ...
     public static JSONObject fetchJSON(long oid){
      ...
      try {
         Transaksi transaksi = PstTransaksi.fetchExc(oid);
         object.put(PstTransaksi.fieldNames[PstTransaksi.FLD_TRANSAKSI_ID], transaksi.getOID());
         object.put(PstTransaksi.fieldNames[PstTransaksi.FLD_TANGGAL_TRANSAKSI], transaksi.getTanggalTransaksi());
         object.put(PstTransaksi.fieldNames[PstTransaksi.FLD_KODE_BUKTI_TRANSAKSI], transaksi.getKodeBuktiTransaksi());
         object.put(PstTransaksi.fieldNames[PstTransaksi.FLD_ID_ANGGOTA], transaksi.getIdAnggota());
         object.put(PstTransaksi.fieldNames[PstTransaksi.FLD_TGL_PRINT_TERAKHIR], transaksi.getTglPrintTerakhir());
         object.put(PstTransaksi.fieldNames[PstTransaksi.FLD_TELLER_SHIFT_ID], transaksi.getTellerShiftId());
         object.put(PstTransaksi.fieldNames[PstTransaksi.FLD_KETERANGAN], transaksi.getKeterangan());
         object.put(PstTransaksi.fieldNames[PstTransaksi.FLD_STATUS], transaksi.getStatus());
         object.put(PstTransaksi.fieldNames[PstTransaksi.FLD_TIPE_ARUS_KAS], transaksi.getTipeArusKas());
         object.put(PstTransaksi.fieldNames[PstTransaksi.FLD_PINJAMAN_ID], transaksi.getPinjamanId());
         object.put(PstTransaksi.fieldNames[PstTransaksi.FLD_ID_DEPOSITO], transaksi.getIdDeposito());
         object.put(PstTransaksi.fieldNames[PstTransaksi.FLD_USECASE_TYPE], transaksi.getUsecaseType());
         object.put(PstTransaksi.fieldNames[PstTransaksi.FLD_TRANSAKSI_PARENT_ID], transaksi.getTransaksiParentId());
         object.put(PstTransaksi.fieldNames[PstTransaksi.FLD_APP_USER_ID], transaksi.getAppUserId());
      }catch(Exception exc){}
      return object;
   }
   ...
   public static long syncExc(JSONObject jSONObject){
      long oid = 0;
      if (jSONObject != null){
       oid = jSONObject.optLong(PstTransaksi.fieldNames[PstTransaksi.FLD_TRANSAKSI_ID],0);
         if (oid > 0){
          Transaksi transaksi = new Transaksi();
          transaksi.setOID(jSONObject.optLong(PstTransaksi.fieldNames[PstTransaksi.FLD_TRANSAKSI_ID],0));
          transaksi.setTanggalTransaksi(Formater.formatDate(jSONObject.optString(PstTransaksi.fieldNames[PstTransaksi.FLD_TANGGAL_TRANSAKSI],"0000-00-00"),"yyyy-MM-dd"));
          transaksi.setKodeBuktiTransaksi(jSONObject.optString(PstTransaksi.fieldNames[PstTransaksi.FLD_KODE_BUKTI_TRANSAKSI], ""));
          transaksi.setIdAnggota(jSONObject.optLong(PstTransaksi.fieldNames[PstTransaksi.FLD_ID_ANGGOTA],0));
          transaksi.setTglPrintTerakhir(Formater.formatDate(jSONObject.optString(PstTransaksi.fieldNames[PstTransaksi.FLD_TGL_PRINT_TERAKHIR],"0000-00-00"),"yyyy-MM-dd"));
          transaksi.setTellerShiftId(jSONObject.optLong(PstTransaksi.fieldNames[PstTransaksi.FLD_TELLER_SHIFT_ID],0));
          transaksi.setKeterangan(jSONObject.optString(PstTransaksi.fieldNames[PstTransaksi.FLD_KETERANGAN], ""));
          transaksi.setStatus(jSONObject.optInt(PstTransaksi.fieldNames[PstTransaksi.FLD_STATUS],0));
          transaksi.setTipeArusKas(jSONObject.optInt(PstTransaksi.fieldNames[PstTransaksi.FLD_TIPE_ARUS_KAS],0));
          transaksi.setPinjamanId(jSONObject.optLong(PstTransaksi.fieldNames[PstTransaksi.FLD_PINJAMAN_ID],0));
          transaksi.setIdDeposito(jSONObject.optLong(PstTransaksi.fieldNames[PstTransaksi.FLD_ID_DEPOSITO],0));
          transaksi.setUsecaseType(jSONObject.optInt(PstTransaksi.fieldNames[PstTransaksi.FLD_USECASE_TYPE],0));
          transaksi.setTransaksiParentId(jSONObject.optLong(PstTransaksi.fieldNames[PstTransaksi.FLD_TRANSAKSI_PARENT_ID],0));
          transaksi.setAppUserId(jSONObject.optLong(PstTransaksi.fieldNames[PstTransaksi.FLD_APP_USER_ID],0));
         boolean checkOidTransaksi = PstTransaksi.checkOID(oid);
          try{
            if(checkOidTransaksi){
               PstTransaksi.updateExc(transaksi);
            }else{
               PstTransaksi.insertByOid(transaksi);
            }
         }catch(Exception exc){}
         }
      }
   ...
   public static long insertByOid(Transaksi transaksi) throws DBException {
      try {
         PstTransaksi pstTransaksi = new PstTransaksi(0);
         pstTransaksi.setLong(FLD_TRANSAKSI_ID, transaksi.getOID());
         pstTransaksi.setDate(FLD_TANGGAL_TRANSAKSI, transaksi.getTanggalTransaksi());
         pstTransaksi.setString(FLD_KODE_BUKTI_TRANSAKSI, transaksi.getKodeBuktiTransaksi());
         pstTransaksi.setLong(FLD_ID_ANGGOTA, transaksi.getIdAnggota());
         pstTransaksi.setDate(FLD_TGL_PRINT_TERAKHIR, transaksi.getTglPrintTerakhir());
         pstTransaksi.setLong(FLD_TELLER_SHIFT_ID, transaksi.getTellerShiftId());
         pstTransaksi.setString(FLD_KETERANGAN, transaksi.getKeterangan());
         pstTransaksi.setInt(FLD_STATUS, transaksi.getStatus());
         pstTransaksi.setInt(FLD_TIPE_ARUS_KAS, transaksi.getTipeArusKas());
         pstTransaksi.setLong(FLD_PINJAMAN_ID, transaksi.getPinjamanId());
         pstTransaksi.setLong(FLD_ID_DEPOSITO, transaksi.getIdDeposito());
         pstTransaksi.setInt(FLD_USECASE_TYPE, transaksi.getUsecaseType());
         pstTransaksi.setLong(FLD_TRANSAKSI_PARENT_ID, transaksi.getTransaksiParentId());
         pstTransaksi.setLong(FLD_APP_USER_ID, transaksi.getAppUserId());
         pstTransaksi.insertByOid(transaksi.getOID());
      } catch (DBException dbe) {
         throw dbe;
      } catch (Exception e) {
         throw new DBException(new PstTransaksi(0), DBException.UNKNOWN);
      }
      return transaksi.getOID();
   }
   
   public static boolean checkExisting(Transaksi transaksi){
       boolean isExisting = false;
       DBResultSet dbrs = null;
       String sql = "SELECT * "
                + " FROM "
                + "  " + TBL_TRANSAKSI
                + " WHERE " + fieldNames[FLD_USECASE_TYPE] + " = '" + transaksi.getUsecaseType() + "' "
                + "  AND " + fieldNames[FLD_TANGGAL_TRANSAKSI] + " = '" + Formater.formatDate(transaksi.getTanggalTransaksi(), "yyyy-MM-dd") + "' "
                + "  AND " + fieldNames[FLD_PINJAMAN_ID] + " = '" + transaksi.getPinjamanId() + "'"
                + "  AND " + fieldNames[FLD_TELLER_SHIFT_ID] + " = '" + transaksi.getTellerShiftId()+ "'";
        try {
            dbrs = DBHandler.execQueryResult(sql);
            try (ResultSet rs = dbrs.getResultSet()) {
                while (rs.next()) {
