...
public class PstAssignContact extends DBHandler implements I_DBInterface, I_DBType, I_PersintentExc, I_Language {
  ...
  public static String queryTransaksiId(String noTabungan) {
   ...
   public static JSONObject fetchJSON(long oid){
      ...
      try {
         AssignContact assignContact = PstAssignContact.fetchExc(oid);
         object.put(PstAssignContact.fieldNames[PstAssignContact.FLD_ASSIGN_TABUNGAN_ID], assignContact.getOID());
         object.put(PstAssignContact.fieldNames[PstAssignContact.FLD_MASTER_TABUNGAN_ID], assignContact.getMasterTabunganId());
         object.put(PstAssignContact.fieldNames[PstAssignContact.FLD_CONTACT_ID], assignContact.getContactId());
         object.put(PstAssignContact.fieldNames[PstAssignContact.FLD_NO_TABUNGAN], assignContact.getNoTabungan());
      }catch(Exception exc){}
      return object;
   }
   ...
   public static long syncExc(JSONObject jSONObject){
      long oid = 0;
      if (jSONObject != null){
       oid = jSONObject.optLong(PstAssignContact.fieldNames[PstAssignContact.FLD_ASSIGN_TABUNGAN_ID],0);
         if (oid > 0){
          AssignContact assignContact = new AssignContact();
          assignContact.setOID(jSONObject.optLong(PstAssignContact.fieldNames[PstAssignContact.FLD_ASSIGN_TABUNGAN_ID],0));
          assignContact.setMasterTabunganId(jSONObject.optLong(PstAssignContact.fieldNames[PstAssignContact.FLD_MASTER_TABUNGAN_ID],0));
          assignContact.setContactId(jSONObject.optLong(PstAssignContact.fieldNames[PstAssignContact.FLD_CONTACT_ID],0));
          assignContact.setNoTabungan(jSONObject.optString(PstAssignContact.fieldNames[PstAssignContact.FLD_NO_TABUNGAN], ""));
         boolean checkOidAssignContact = PstAssignContact.checkOID(oid);
          try{
            if(checkOidAssignContact){
               PstAssignContact.updateExc(assignContact);
            }else{
               PstAssignContact.insertByOid(assignContact);
            }
         }catch(Exception exc){}
         }
      }
   ...
   public static long insertByOid(AssignContact assignContact) throws DBException {
      try {
         PstAssignContact pstAssignContact = new PstAssignContact(0);
         pstAssignContact.setLong(FLD_ASSIGN_TABUNGAN_ID, assignContact.getOID());
         pstAssignContact.setLong(FLD_MASTER_TABUNGAN_ID, assignContact.getMasterTabunganId());
         pstAssignContact.setLong(FLD_CONTACT_ID, assignContact.getContactId());
         pstAssignContact.setString(FLD_NO_TABUNGAN, assignContact.getNoTabungan());
         pstAssignContact.insertByOid(assignContact.getOID());
      } catch (DBException dbe) {
         throw dbe;
      } catch (Exception e) {
         throw new DBException(new PstAssignContact(0), DBException.UNKNOWN);
      }
      return assignContact.getOID();
   }
}
