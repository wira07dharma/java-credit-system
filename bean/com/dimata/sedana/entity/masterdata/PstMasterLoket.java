...
public class PstMasterLoket extends DBHandler implements I_DBInterface, I_DBType, I_PersintentExc, I_Language {
    ...
    //Use For Raditya Loket Type
    public static int LOKET_TYPE_SALES = 0; 
    public static int LOKET_TYPE_KOLEKTOR = 1;
    public static int LOKET_TYPE_OPERASIONAL = 2;

    //Use For Raditya Loket Type
    public static String[] loketRadityaTypeTitle = {"Sales", "Kolektor", "Operasional"};
   ...
   public static JSONObject fetchJSON(long oid){
      ...
      try {
         MasterLoket masterLoket = PstMasterLoket.fetchExc(oid);
         object.put(PstMasterLoket.fieldNames[PstMasterLoket.FLD_MASTER_LOKET_ID], masterLoket.getMasterLoketId());
         object.put(PstMasterLoket.fieldNames[PstMasterLoket.FLD_LOKET_NUMBER], masterLoket.getLoketNumber());
         object.put(PstMasterLoket.fieldNames[PstMasterLoket.FLD_LOCATION_ID], masterLoket.getLocationId());
         object.put(PstMasterLoket.fieldNames[PstMasterLoket.FLD_LOKET_NAME], masterLoket.getLoketName());
         object.put(PstMasterLoket.fieldNames[PstMasterLoket.FLD_LOKET_TYPE], masterLoket.getLoketType());
      }catch(Exception exc){}
      return object;
   }
      ...
      long oid = 0;
      if (jSONObject != null){
       oid = jSONObject.optLong(PstMasterLoket.fieldNames[PstMasterLoket.FLD_MASTER_LOKET_ID],0);
         if (oid > 0){
          MasterLoket masterLoket = new MasterLoket();
          masterLoket.setMasterLoketId(jSONObject.optLong(PstMasterLoket.fieldNames[PstMasterLoket.FLD_MASTER_LOKET_ID],0));
          masterLoket.setLoketNumber(jSONObject.optInt(PstMasterLoket.fieldNames[PstMasterLoket.FLD_LOKET_NUMBER],0));
          masterLoket.setLocationId(jSONObject.optLong(PstMasterLoket.fieldNames[PstMasterLoket.FLD_LOCATION_ID],0));
          masterLoket.setLoketName(jSONObject.optString(PstMasterLoket.fieldNames[PstMasterLoket.FLD_LOKET_NAME], ""));
          masterLoket.setLoketType(jSONObject.optInt(PstMasterLoket.fieldNames[PstMasterLoket.FLD_LOKET_TYPE],0));
         boolean checkOidMasterLoket = PstMasterLoket.checkOID(oid);
          try{
            if(checkOidMasterLoket){
               PstMasterLoket.updateExc(masterLoket);
            }else{
               PstMasterLoket.insertByOid(masterLoket);
            }
         }catch(Exception exc){}
         }
      }
   ...
   public static long insertByOid(MasterLoket masterLoket) throws DBException {
      try {
         PstMasterLoket pstMasterLoket = new PstMasterLoket(0);
         pstMasterLoket.setLong(FLD_MASTER_LOKET_ID, masterLoket.getMasterLoketId());
         pstMasterLoket.setInt(FLD_LOKET_NUMBER, masterLoket.getLoketNumber());
         pstMasterLoket.setLong(FLD_LOCATION_ID, masterLoket.getLocationId());
         pstMasterLoket.setString(FLD_LOKET_NAME, masterLoket.getLoketName());
         pstMasterLoket.setInt(FLD_LOKET_TYPE, masterLoket.getLoketType());
         pstMasterLoket.insertByOid(masterLoket.getOID());
      } catch (DBException dbe) {
         throw dbe;
      } catch (Exception e) {
         throw new DBException(new PstMasterLoket(0), DBException.UNKNOWN);
      }
      return masterLoket.getOID();
   }
}
