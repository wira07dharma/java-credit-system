...
public class PstSedanaShift extends DBHandler implements I_DBInterface, I_DBType, I_PersintentExc, I_Language {
  ...
  public static int findLimitCommand(int start, int recordToGet, int vectSize) {
   ...
   public static JSONObject fetchJSON(long oid){
      ...
      try {
         SedanaShift sedanaShift = PstSedanaShift.fetchExc(oid);
         object.put(PstSedanaShift.fieldNames[PstSedanaShift.FLD_SHIFT_ID], sedanaShift.getOID());
         object.put(PstSedanaShift.fieldNames[PstSedanaShift.FLD_NAME], sedanaShift.getName());
         object.put(PstSedanaShift.fieldNames[PstSedanaShift.FLD_END_TIME], sedanaShift.getEndTime());
         object.put(PstSedanaShift.fieldNames[PstSedanaShift.FLD_START_TIME], sedanaShift.getStartTime());
         object.put(PstSedanaShift.fieldNames[PstSedanaShift.FLD_REMARK], sedanaShift.getRemark());
      }catch(Exception exc){}
      return object;
   }
      ...
      long oid = 0;
      if (jSONObject != null){
       oid = jSONObject.optLong(PstSedanaShift.fieldNames[PstSedanaShift.FLD_SHIFT_ID],0);
         if (oid > 0){
          SedanaShift sedanaShift = new SedanaShift();
          sedanaShift.setOID(jSONObject.optLong(PstSedanaShift.fieldNames[PstSedanaShift.FLD_SHIFT_ID],0));
          sedanaShift.setName(jSONObject.optString(PstSedanaShift.fieldNames[PstSedanaShift.FLD_NAME], ""));
          sedanaShift.setEndTime(Formater.formatDate(jSONObject.optString(PstSedanaShift.fieldNames[PstSedanaShift.FLD_END_TIME],"0000-00-00"),"yyyy-MM-dd"));
          sedanaShift.setStartTime(Formater.formatDate(jSONObject.optString(PstSedanaShift.fieldNames[PstSedanaShift.FLD_START_TIME],"0000-00-00"),"yyyy-MM-dd"));
          sedanaShift.setRemark(jSONObject.optString(PstSedanaShift.fieldNames[PstSedanaShift.FLD_REMARK], ""));
         boolean checkOidSedanaShift = PstSedanaShift.checkOID(oid);
          try{
            if(checkOidSedanaShift){
               PstSedanaShift.updateExc(sedanaShift);
            }else{
               PstSedanaShift.insertByOid(sedanaShift);
            }
         }catch(Exception exc){}
         }
      }
   ...
   public static long insertByOid(SedanaShift sedanaShift) throws DBException {
      try {
         PstSedanaShift pstSedanaShift = new PstSedanaShift(0);
         pstSedanaShift.setLong(FLD_SHIFT_ID, sedanaShift.getOID());
         pstSedanaShift.setString(FLD_NAME, sedanaShift.getName());
         pstSedanaShift.setDate(FLD_END_TIME, sedanaShift.getEndTime());
         pstSedanaShift.setDate(FLD_START_TIME, sedanaShift.getStartTime());
         pstSedanaShift.setString(FLD_REMARK, sedanaShift.getRemark());
         pstSedanaShift.insertByOid(sedanaShift.getOID());
      } catch (DBException dbe) {
         throw dbe;
      } catch (Exception e) {
         throw new DBException(new PstSedanaShift(0), DBException.UNKNOWN);
      }
      return sedanaShift.getOID();
   }
}
