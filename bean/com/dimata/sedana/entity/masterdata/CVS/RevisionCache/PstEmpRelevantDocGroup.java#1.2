...
public class PstEmpRelevantDocGroup extends DBHandler implements I_DBInterface, I_DBType, I_PersintentExc, I_Language {
   ...
   public static JSONObject fetchJSON(long oid){
      ...
      try {
         EmpRelevantDocGroup empRelevantDocGroup = PstEmpRelevantDocGroup.fetchExc(oid);
         object.put(PstEmpRelevantDocGroup.fieldNames[PstEmpRelevantDocGroup.FLD_EMP_RELVT_DOC_GRP_ID], empRelevantDocGroup.getOID());
         object.put(PstEmpRelevantDocGroup.fieldNames[PstEmpRelevantDocGroup.FLD_DOC_GROUP], empRelevantDocGroup.getDocGroup());
         object.put(PstEmpRelevantDocGroup.fieldNames[PstEmpRelevantDocGroup.FLD_DOC_GROUP_DESC], empRelevantDocGroup.getDocGroupDesc());
      }catch(Exception exc){}
      return object;
   }
      ...
      long oid = 0;
      if (jSONObject != null){
       oid = jSONObject.optLong(PstEmpRelevantDocGroup.fieldNames[PstEmpRelevantDocGroup.FLD_EMP_RELVT_DOC_GRP_ID],0);
         if (oid > 0){
          EmpRelevantDocGroup empRelevantDocGroup = new EmpRelevantDocGroup();
          empRelevantDocGroup.setOID(jSONObject.optLong(PstEmpRelevantDocGroup.fieldNames[PstEmpRelevantDocGroup.FLD_EMP_RELVT_DOC_GRP_ID],0));
          empRelevantDocGroup.setDocGroup(jSONObject.optString(PstEmpRelevantDocGroup.fieldNames[PstEmpRelevantDocGroup.FLD_DOC_GROUP], ""));
          empRelevantDocGroup.setDocGroupDesc(jSONObject.optString(PstEmpRelevantDocGroup.fieldNames[PstEmpRelevantDocGroup.FLD_DOC_GROUP_DESC], ""));
         boolean checkOidEmpRelevantDocGroup = PstEmpRelevantDocGroup.checkOID(oid);
          try{
            if(checkOidEmpRelevantDocGroup){
               PstEmpRelevantDocGroup.updateExc(empRelevantDocGroup);
            }else{
               PstEmpRelevantDocGroup.insertByOid(empRelevantDocGroup);
            }
         }catch(Exception exc){}
         }
      }
   ...
   public static long insertByOid(EmpRelevantDocGroup empRelevantDocGroup) throws DBException {
      try {
         PstEmpRelevantDocGroup pstEmpRelevantDocGroup = new PstEmpRelevantDocGroup(0);
         pstEmpRelevantDocGroup.setLong(FLD_EMP_RELVT_DOC_GRP_ID, empRelevantDocGroup.getOID());
         pstEmpRelevantDocGroup.setString(FLD_DOC_GROUP, empRelevantDocGroup.getDocGroup());
         pstEmpRelevantDocGroup.setString(FLD_DOC_GROUP_DESC, empRelevantDocGroup.getDocGroupDesc());
         pstEmpRelevantDocGroup.insertByOid(empRelevantDocGroup.getOID());
      } catch (DBException dbe) {
         throw dbe;
      } catch (Exception e) {
         throw new DBException(new PstEmpRelevantDocGroup(0), DBException.UNKNOWN);
      }
      return empRelevantDocGroup.getOID();
   }
   

    public static Hashtable<String, Long> listToHashTableAll() {
        return listToHashTable(0, 0, "", "");
    }

    public static Hashtable<String, Long> listToHashTable(int limitStart, int recordToGet, String whereClause, String order) {
        Hashtable<String, Long> lists = new Hashtable<>();
        DBResultSet dbrs = null;
        try {
            String sql = "SELECT * FROM " + TBL_EMP_RELVT_DOC_GRP;
            if (whereClause != null && whereClause.length() > 0) {
            ...
            while (rs.next()) {
                EmpRelevantDocGroup empRelevantDocGroup = new EmpRelevantDocGroup();
                resultToObject(rs, empRelevantDocGroup);
                lists.put(empRelevantDocGroup.getDocGroup(), empRelevantDocGroup.getOID());
            }
            rs.close();
        } catch (Exception e) {
            System.out.println(e);
