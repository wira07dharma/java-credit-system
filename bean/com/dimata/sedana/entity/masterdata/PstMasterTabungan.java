...
public class PstMasterTabungan extends DBHandler implements I_DBInterface, I_DBType, I_PersintentExc, I_Language {
  ...
  public static int findLimitCommand(int start, int recordToGet, int vectSize) {
   ...
   public static JSONObject fetchJSON(long oid){
      ...
      try {
         MasterTabungan masterTabungan = PstMasterTabungan.fetchExc(oid);
         object.put(PstMasterTabungan.fieldNames[PstMasterTabungan.FLD_MASTER_TABUNGAN_ID], masterTabungan.getMasterTabunganId());
         object.put(PstMasterTabungan.fieldNames[PstMasterTabungan.FLD_KODE_TABUNGAN], masterTabungan.getKodeTabungan());
         object.put(PstMasterTabungan.fieldNames[PstMasterTabungan.FLD_NAMA_TABUNGAN], masterTabungan.getNamaTabungan());
         object.put(PstMasterTabungan.fieldNames[PstMasterTabungan.FLD_KET], masterTabungan.getKet());
         object.put(PstMasterTabungan.fieldNames[PstMasterTabungan.FLD_DENDA], masterTabungan.getDenda());
         object.put(PstMasterTabungan.fieldNames[PstMasterTabungan.FLD_JENIS_DENDA], masterTabungan.getJenisDenda());
         object.put(PstMasterTabungan.fieldNames[PstMasterTabungan.FLD_MIN_SALDO_BUNGA], masterTabungan.getMinSaldoBunga());
      }catch(Exception exc){}
      return object;
   }
     ...
     public static long syncExc(JSONObject jSONObject){
      long oid = 0;
      if (jSONObject != null){
       oid = jSONObject.optLong(PstMasterTabungan.fieldNames[PstMasterTabungan.FLD_MASTER_TABUNGAN_ID],0);
         if (oid > 0){
          MasterTabungan masterTabungan = new MasterTabungan();
          masterTabungan.setMasterTabunganId(jSONObject.optLong(PstMasterTabungan.fieldNames[PstMasterTabungan.FLD_MASTER_TABUNGAN_ID],0));
          masterTabungan.setKodeTabungan(jSONObject.optString(PstMasterTabungan.fieldNames[PstMasterTabungan.FLD_KODE_TABUNGAN], ""));
          masterTabungan.setNamaTabungan(jSONObject.optString(PstMasterTabungan.fieldNames[PstMasterTabungan.FLD_NAMA_TABUNGAN], ""));
          masterTabungan.setKet(jSONObject.optString(PstMasterTabungan.fieldNames[PstMasterTabungan.FLD_KET], ""));
          masterTabungan.setDenda(jSONObject.optDouble(PstMasterTabungan.fieldNames[PstMasterTabungan.FLD_DENDA],0));
          masterTabungan.setJenisDenda(jSONObject.optInt(PstMasterTabungan.fieldNames[PstMasterTabungan.FLD_JENIS_DENDA],0));
          masterTabungan.setMinSaldoBunga(jSONObject.optDouble(PstMasterTabungan.fieldNames[PstMasterTabungan.FLD_MIN_SALDO_BUNGA],0));
         boolean checkOidMasterTabungan = PstMasterTabungan.checkOID(oid);
          try{
            if(checkOidMasterTabungan){
               PstMasterTabungan.updateExc(masterTabungan);
            }else{
               PstMasterTabungan.insertByOid(masterTabungan);
            }
         }catch(Exception exc){}
         }
      }
   ...
   public static long insertByOid(MasterTabungan masterTabungan) throws DBException {
      try {
         PstMasterTabungan pstMasterTabungan = new PstMasterTabungan(0);
         pstMasterTabungan.setLong(FLD_MASTER_TABUNGAN_ID, masterTabungan.getMasterTabunganId());
         pstMasterTabungan.setString(FLD_KODE_TABUNGAN, masterTabungan.getKodeTabungan());
         pstMasterTabungan.setString(FLD_NAMA_TABUNGAN, masterTabungan.getNamaTabungan());
         pstMasterTabungan.setString(FLD_KET, masterTabungan.getKet());
         pstMasterTabungan.setDouble(FLD_DENDA, masterTabungan.getDenda());
         pstMasterTabungan.setInt(FLD_JENIS_DENDA, masterTabungan.getJenisDenda());
         pstMasterTabungan.setDouble(FLD_MIN_SALDO_BUNGA, masterTabungan.getMinSaldoBunga());
         pstMasterTabungan.insertByOid(masterTabungan.getOID());
      } catch (DBException dbe) {
         throw dbe;
      } catch (Exception e) {
         throw new DBException(new PstMasterTabungan(0), DBException.UNKNOWN);
