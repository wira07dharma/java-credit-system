...
public class PstAssignTabungan extends DBHandler implements I_DBInterface, I_DBType, I_PersintentExc, I_Language {
  ...
  public static String getIdJenisSimpananByTabungan(Tabungan tabungan) {
   ...
   public static JSONObject fetchJSON(long oid){
      ...
      try {
         AssignTabungan assignTabungan = PstAssignTabungan.fetchExc(oid);
         object.put(PstAssignTabungan.fieldNames[PstAssignTabungan.FLD_ASSIGN_MASTER_TABUNGAN_ID], assignTabungan.getOID());
         object.put(PstAssignTabungan.fieldNames[PstAssignTabungan.FLD_MASTER_TABUNGAN], assignTabungan.getMasterTabungan());
         object.put(PstAssignTabungan.fieldNames[PstAssignTabungan.FLD_ID_JENIS_SIMPANAN], assignTabungan.getIdJenisSimpanan());
      }catch(Exception exc){}
      return object;
   } 
   public static long syncExc(JSONObject jSONObject){
      long oid = 0;
      if (jSONObject != null){
       oid = jSONObject.optLong(PstAssignTabungan.fieldNames[PstAssignTabungan.FLD_ASSIGN_MASTER_TABUNGAN_ID],0);
         if (oid > 0){
          AssignTabungan assignTabungan = new AssignTabungan();
          assignTabungan.setOID(jSONObject.optLong(PstAssignTabungan.fieldNames[PstAssignTabungan.FLD_ASSIGN_MASTER_TABUNGAN_ID],0));
          assignTabungan.setMasterTabungan(jSONObject.optLong(PstAssignTabungan.fieldNames[PstAssignTabungan.FLD_MASTER_TABUNGAN],0));
          assignTabungan.setIdJenisSimpanan(jSONObject.optLong(PstAssignTabungan.fieldNames[PstAssignTabungan.FLD_ID_JENIS_SIMPANAN],0));
         boolean checkOidAssignTabungan = PstAssignTabungan.checkOID(oid);
          try{
            if(checkOidAssignTabungan){
               PstAssignTabungan.updateExc(assignTabungan);
            }else{
               PstAssignTabungan.insertByOid(assignTabungan);
            }
         }catch(Exception exc){}
         }
      }
   ...
   public static long insertByOid(AssignTabungan assignTabungan) throws DBException {
      try {
         PstAssignTabungan pstAssignTabungan = new PstAssignTabungan(0);
         pstAssignTabungan.setLong(FLD_ASSIGN_MASTER_TABUNGAN_ID, assignTabungan.getOID());
         pstAssignTabungan.setLong(FLD_MASTER_TABUNGAN, assignTabungan.getMasterTabungan());
         pstAssignTabungan.setLong(FLD_ID_JENIS_SIMPANAN, assignTabungan.getIdJenisSimpanan());
         pstAssignTabungan.insertByOid(assignTabungan.getOID());
      } catch (DBException dbe) {
         throw dbe;
      } catch (Exception e) {
         throw new DBException(new PstAssignTabungan(0), DBException.UNKNOWN);
      }
      return assignTabungan.getOID();
   }
}
