 ...
 * @author Regen
 */
public class AjaxPenarikan extends HTTPTabungan {
  
  public static final int FORM_SEARCH = 2;
  public static final int FORM_PENARIKAN = 1;
  public static final int SAVE_PENARIKAN = 0;
  ...
  public void formSearch() {
    this.isJSON = false;
    this.redirect = false;
    this.req.setAttribute("dataFor", FORM_PENARIKAN);
  }
  
  public void formPenarikan() {
    ...
    for (JenisTransaksi jt : jts) {
      selected = (jt.getOID() == idPenarikanCash) ? "selected":"";
      jenisTransaksi += "<option "+selected+" value='" + jt.getOID() + "'>" + jt.getJenisTransaksi() + "</option>";
    }
    this.isJSON = false;
    this.redirect = false;
    this.req.setAttribute("dataFor", SAVE_PENARIKAN);
  ...
  public synchronized void savePenarikan() {
    this.redirect = true;
    String kodeTransaksi = I_Sedana.KODE_TRANSAKSI_TABUNGAN_PENARIKAN;
    int useCaseType = I_Sedana.USECASE_TYPE_TABUNGAN_PENARIKAN;
