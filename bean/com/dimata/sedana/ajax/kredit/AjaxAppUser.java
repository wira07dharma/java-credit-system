 ...
 * @author arise
 */
public class AjaxAppUser extends HttpServlet {
//DATATABLES
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        this.oidLocation = FRMQueryString.requestLong(request, "FRM_USER_LOCATION");
...
//				commandNone(request);
    ...
    public void commandList(HttpServletRequest request, HttpServletResponse response) {
        if (this.dataFor.equals("listAppUser")) {
            String[] cols = {
                "",
    ...
    public JSONObject listDataTables(HttpServletRequest request, HttpServletResponse response, String[] cols, String dataFor, JSONObject result) {
        ...
        if (dataFor.equals("listAppUser")) {
            whereClause += "("
                    + PstAppUser.fieldNames[PstAppUser.FLD_LOGIN_ID] + " LIKE '%" + searchTerm + "%' "
                    + " OR " + PstAppUser.fieldNames[PstAppUser.FLD_FULL_NAME] + " LIKE '%" + searchTerm + "%'"
                    + " OR " + PstAppUser.fieldNames[PstAppUser.FLD_REG_DATE] + " LIKE '%" + searchTerm + "%'"
                    + " OR " + PstAppUser.fieldNames[PstAppUser.FLD_UPDATE_DATE] + " LIKE '%" + searchTerm + "%'"
                    + ")";
            if(this.oidLocation != 0){
                whereClause += " AND " + PstAppUser.fieldNames[PstAppUser.FLD_ASSIGN_LOCATION_ID] + " = " + oidLocation;
            }
        }
        ...
        if (dataFor.equals("listAppUser")) {
            total = PstAppUser.getCount(whereClause);
        }
    ...
    public JSONObject getData(int total, HttpServletRequest request, String datafor, String whereClause) {
        ...
        if (datafor.equals("listAppUser")) {
            listData = PstAppUser.listPartObj(start, amount, whereClause, order);
        }

        for (int i = 0; i <= listData.size() - 1; i++) {
            JSONArray ja = new JSONArray();
            if (datafor.equals("listAppUser")) {
                AppUser au = (AppUser) listData.get(i);
                ja.put("<div class='text-center'>" + (this.start + i + 1) + "</div>");
                ja.put("<div class='text-left'>" + au.getLoginId() + "</div>");
                ja.put("<div class='text-left'>" + au.getFullName() + "</div>");
                ja.put("<div class='text-left'>" + AppUser.getStatusTxt(au.getUserStatus()) + "</div>");
                ja.put("<div class='text-center'>" + Formater.formatDate(au.getRegDate(), dateFormat) + "</div>");
                ja.put("<div class='text-center'>" + Formater.formatDate(au.getUpdateDate(), dateFormat) + "</div>");
                String button = "";
                if (this.privUpdate) {
                    button = "<button type='button' value='" + au.getOID() + "'"
                            + "class='btn btn-warning create-edit-user'>"
                            + "<i class='fa fa-pencil'></i>"
                            + "</button>";
                } else {
                    button = "<p>No Access.</p>";
                }
