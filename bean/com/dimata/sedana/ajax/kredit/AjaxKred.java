...
public class AjaxKred {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        if (sessLogin == true) {
            switch (iCommand) {
                case Command.SAVE:
                    objValue = commandSave(request, dataFor);
                    break;

                case Command.DELETE:
                    objValue = commandDelete(request, dataFor);
                    break;

                case Command.LIST:
                    objValue = commandList(request, response, dataFor);
                    break;

                default:
                    objValue = commandNone(request, dataFor);
                    break;
            }
        } else {
        ...
        try {
            ...
            this.jSONObject.put("RETURN_DATA_JENIS_KREDIT", this.jsonArrayJenisKredit);
            this.jSONObject.put("RETURN_DATA_KREDIT", this.jsonArrayDataKredit);
            ...
            this.jSONObject.put("RETURN_DATA_OID_PINJAMAN", "" + this.oidPinjaman);
            this.jSONObject.put("RETURN_DATA_OID_ANGGOTA", "" + this.oidAnggota);
            this.jSONObject.put("RETURN_ERROR_CODE", "" + this.iErrCode);
            this.jSONObject.put("RETURN_MESSAGE", "" + this.message);
            this.jSONObject.put("RETURN_OID_PAYMENT_TYPE", "" + this.oidPaymentType);
            this.jSONObject.put("RETURN_NOMOR_TRANSAKSI", "" + this.noTransaksi);
            this.jSONObject.put("RETURN_PAYMENT_TYPE", "" + this.paymentType);
            this.jSONObject.put("RETURN_DATA_OPEN_TRANSACTION", "" + this.openTransaction);
            this.jSONObject.put("RETURN_APPROVAL", "" + this.approve);
            this.jSONObject.put("RETURN_BILL_MAIN_ID", "" + this.oidBillMain);
    ...
    //COMMAND SAVE==============================================================
    public JSONObject commandSave(HttpServletRequest request, String dataFor) throws JSONException{
        JSONObject objValue = new JSONObject();
...
//                    JSONObject objStatus = WebServices.postAPI("", url);
                    ...
                    dataSend.put("USER_LOCATION_ID", userLocationId);
...
//            message = ex.getMessage();
    ...
    //COMMAND DELETE============================================================
    public synchronized JSONObject commandDelete(HttpServletRequest request, String dataFor) throws JSONException{
        JSONObject objValue = new JSONObject();
    ...
    //COMMAND LIST==============================================================
    public JSONObject commandList(HttpServletRequest request, HttpServletResponse response, String dataFor) throws JSONException{
        String typeOfCredit = PstSystemProperty.getValueByName("TYPE_OF_CREDIT");
        JSONObject objValue = new JSONObject();
    ...
    public JSONObject listDataTables(HttpServletRequest request, HttpServletResponse response, String[] cols, String dataFor, JSONObject result) throws JSONException{
        ...
        if (dataFor.equals("listPinjamanAll")) {
            ...
            if ((start_date != null && start_date.length() > 0) && (end_date != null && end_date.length() > 0)) {
                whereClause += " AND ("
                        + "(TO_DAYS( r." + PstReturnKredit.fieldNames[PstReturnKredit.FLD_TANGGAL_RETURN] + ") "
                        + ">= TO_DAYS('" + Formater.formatDate(start_date, "yyyy-MM-DD") + "')) AND "
                        + "(TO_DAYS( r." + PstReturnKredit.fieldNames[PstReturnKredit.FLD_TANGGAL_RETURN] + ") "
                        + "<= TO_DAYS('" + Formater.formatDate(end_date, "yyyy-MM-DD") + "'))"
                        + ")";
            }
        } else if (dataFor.equals("listKreditKolektabilitas")) {
            whereClause = " ("
...
//                total = this.listIdPinjaman.size();
            ...
            if (typeOfCredit.equals("1")) {
                if (userSession.getAppUser().getAssignLocationId() != 0) {
                    total = PstPinjaman.getCountJoinKredit(whereClause + " AND " + PstPinjaman.fieldNames[PstPinjaman.FLD_STATUS_PINJAMAN] + " = '" + Pinjaman.STATUS_DOC_CAIR + "' AND bm." + PstBillMain.fieldNames[PstBillMain.FLD_LOCATION_ID] + " = " + userSession.getAppUser().getAssignLocationId());
                } else {
...
//                }
            ...
            whereClause += " AND " + PstPinjaman.fieldNames[PstPinjaman.FLD_STATUS_PINJAMAN] + " = '" + Pinjaman.STATUS_DOC_CAIR + "'"
    ...
    public JSONObject getData(int total, HttpServletRequest request, String datafor, String addSql, JSONObject objValue) throws JSONException{
        ...
        if (datafor.equals("listPinjamanFinal")) {
            ...
            if (typeOfCredit.equals("1")) {
                if (userSession.getAppUser().getAssignLocationId() != 0) {
                    listData = PstPinjaman.listJoinKredit(objValue.optInt("start",0), objValue.optInt("amount",0), whereClause + " AND " + PstPinjaman.fieldNames[PstPinjaman.FLD_STATUS_PINJAMAN] + " = '" + Pinjaman.STATUS_DOC_CAIR + "' AND bm." + PstBillMain.fieldNames[PstBillMain.FLD_LOCATION_ID] + " = " + userSession.getAppUser().getAssignLocationId(), order);
                } else {
...
//                }
            ...
            if (statusDoc.equals("all")) {
                if (typeOfCredit.equals("1")) {
                    if (userSession.getAppUser().getAssignLocationId() != 0) {
                        listData = PstPinjaman.listJoinKredit(objValue.optInt("start",0), objValue.optInt("amount",0), whereClause + " AND bm." + PstBillMain.fieldNames[PstBillMain.FLD_LOCATION_ID] + " = " + userSession.getAppUser().getAssignLocationId(), order);
                    } else {
                        listData = PstPinjaman.listJoinKredit(objValue.optInt("start",0), objValue.optInt("amount",0), whereClause, order);
                    }
                } else {
                ...
                if (typeOfCredit.equals("1")) {
                    if (userSession.getAppUser().getAssignLocationId() != 0) {
                        listData = PstPinjaman.listJoinKredit(objValue.optInt("start",0), objValue.optInt("amount",0), whereClause + " AND " + PstPinjaman.fieldNames[PstPinjaman.FLD_STATUS_PINJAMAN] + " = '" + statusDoc + "' AND bm." + PstBillMain.fieldNames[PstBillMain.FLD_LOCATION_ID] + " = " + userSession.getAppUser().getAssignLocationId(), order);
                    } else {
...
//                array.put(ja);
    ...
    //COMMAND NONE==============================================================
    public synchronized JSONObject commandNone(HttpServletRequest request, String dataFor) throws JSONException{
        JSONObject objValue = new JSONObject();
    ...
    public JSONObject getDataKreditForReturn(HttpServletRequest request, JSONObject objValue) throws JSONException{
        ...
        for (Billdetail bd : listItem) {
            ...
            html += ""
                    ...
                    + "</td>"
                    + " <td class='text-center'>" 
                    + jo.optString(PstMaterial.fieldNames[PstMaterial.FLD_SKU],"")
                    + "</td>"
                    + " <td class='text-center'>" 
                    + jo.optString(PstMaterial.fieldNames[PstMaterial.FLD_NAME],"")
                    + "</td>"
