...
//        SimpleDateFormat simpledateformat = new SimpleDateFormat("yyyy.MM.dd G 'at' hh:mm:ss a zzz");
    ...
    public String freeConnection(Connection connection, int conInUse, int maxCon)
    {
        String s = "";
        if(conInUse <= (maxCon))
        {
	    for(int i = conInUse; i > -1; i--){
		if(i > 2){
		    connStatus[(i-2)] = 0;
		}else{
		    if(i == 0){
		...
		System.out.println("Currenct Connection :::: "+idOfConnection(connection));
		if(i > 4){
		   currConnections--;
		}
		//s = "freed " + connection.toString();
