...
public class PstAnggota extends DBHandler implements I_Language, I_DBType, I_DBInterface, I_PersintentExc {
    ...
    public static final int FLD_MARITAL_ID = 29;
    public static final int FLD_LATITUDE = 30;
    public static final int FLD_LONGITUDE = 31;
...
//            anggota.setNoRekening(pstAnggota.getString(FLD_NO_REKENING));
            ...
            anggota.setMaritalId(pstAnggota.getlong(FLD_MARITAL_ID));
            anggota.setAddrLat(pstAnggota.getdouble(FLD_LATITUDE));
            anggota.setAddrLong(pstAnggota.getdouble(FLD_LONGITUDE));
...
//            pstAnggota.setString(FLD_NO_REKENING, anggota.getNoRekening());
            ...
            pstAnggota.setLong(FLD_MARITAL_ID, anggota.getMaritalId());
            pstAnggota.setDouble(FLD_LATITUDE, anggota.getAddrLat());
            pstAnggota.setDouble(FLD_LONGITUDE, anggota.getAddrLong());
...
//                pstAnggota.setString(FLD_NO_REKENING, anggota.getNoRekening());
   ...
   public static int getCountJoin(String whereClause) {
    ...
    public static void resultToObject(ResultSet rs, Anggota anggota) {
        try {
...
//            anggota.setNoRekening(rs.getString(PstAnggota.fieldNames[PstAnggota.FLD_NO_REKENING]));
            ...
            anggota.setMaritalId(rs.getLong(PstAnggota.fieldNames[PstAnggota.FLD_MARITAL_ID]));
            anggota.setAddrLat(rs.getDouble(PstAnggota.fieldNames[PstAnggota.FLD_LATITUDE]));
            anggota.setAddrLong(rs.getDouble(PstAnggota.fieldNames[PstAnggota.FLD_LONGITUDE]));
        } catch (Exception e) {
        }
    }
    ...
    public static Hashtable fetchExcHashtable(long oid) throws DBException {
        ...
        try {
            ...
            if(anggota.getBirthDate() != null){
                age = getAge(anggota.getBirthDate());
            }
            hashtableAnggota.put("MEMBER_AGE", age == 0 ? "-" : age);
            
            EmpRelevantDoc empRelevantDoc = new EmpRelevantDoc();
            try {
                empRelevantDoc = PstEmpRelevantDoc.getObjEmpPicture(anggota.getOID());
            } catch (Exception e) {
                System.out.println(e.toString());
            }
            String imgSrc = "";
            if(!empRelevantDoc.getFileName().equals("") && empRelevantDoc.getFileName().length() > 0){
               String dir = PstSystemProperty.getValueByName("IMGDOC");
               imgSrc = "<img src='" + dir + "/" + empRelevantDoc.getFileName() + "'/>";
            }
            hashtableAnggota.put("FOTO_KTP", imgSrc);
            
            String penanggungName = "";
            String penanggungRelation = "";
            String penanggungAddress = "";
            String penanggungHandphone = "";
            String penanggungTelephone = "";
            try {
                whereClause = PstAnggotaKeluarga.fieldNames[PstAnggotaKeluarga.FLD_CONTACT_ANGGOTA_ID] + " = " + anggota.getOID();
                order  = PstAnggotaKeluarga.fieldNames[PstAnggotaKeluarga.FLD_STATUS_RELASI] + " ASC ";
                Vector<AnggotaKeluarga> listKeluarga = PstAnggotaKeluarga.list(0, 0, whereClause, order);
                if(!listKeluarga.isEmpty()){
                    AnggotaKeluarga ak = listKeluarga.get(0);
                    Anggota keluarga = PstAnggota.fetchExc(ak.getContactKeluargaId());
                    
                    penanggungName = keluarga.getName();
                    penanggungRelation = PstAnggotaKeluarga.statusKeluarga[ak.getStatusRelasi()];
                    penanggungAddress = keluarga.getAddressPermanent();
                    penanggungHandphone = keluarga.getHandPhone();
                    penanggungTelephone = keluarga.getTelepon();
            ...
            hashtableAnggota.put("PENANGGUNG_NAME", (penanggungName.length() > 0 ? penanggungName : "-"));
            hashtableAnggota.put("PENANGGUNG_RELATION", (penanggungRelation.length() > 0 ? penanggungRelation : "-"));
            hashtableAnggota.put("PENANGGUNG_ADDRESS", (penanggungAddress.length() > 0 ? penanggungAddress : "-"));
            hashtableAnggota.put("PENANGGUNG_HANDPHONE", (penanggungHandphone.length() > 0 ? penanggungHandphone : "-"));
            hashtableAnggota.put("PENANGGUNG_TELEPHONE", (penanggungTelephone.length() > 0 ? penanggungTelephone : "-"));
