...
public class Formater {
    ...
    public static String formatShortMonthyYear(Date date) {
        String strRet = new String();
        
        try {
            strRet= YearMonth.getShortEngMonthName(date.getMonth()+1) + " " + String.valueOf(date.getYear()+1900).substring(2,4);
        } catch(Exception e) {
            strRet = "ERROR::" + e.toString();
        }
        return strRet;
    ...
    public static String formatNumber(double dnumber, String format) {
        DecimalFormat df = new DecimalFormat(format);
        String number=df.format(dnumber).toString();  

        //System.out.println("<br>number : "+number);
        ...
        if(format.indexOf(".")==-1){
                ...
                String decNum = "";
                while (strToken.hasMoreTokens()){
                        decNum=strToken.nextToken();
                }

                //System.out.println("<br>decNum : "+decNum);

                String afterComma = "";
                String beforeComma = "";
                if(number.indexOf(".")==-1){
                        if(decNum!=null && decNum.length()>0){
                                for(int i=0; i<decNum.length(); i++){
                                        afterComma = afterComma + "0";
                                }
                                number = number +"."+afterComma;
                        }			
                }
                else{
                        strToken=new StringTokenizer(number,".");

                        while (strToken.hasMoreTokens()){
                                beforeComma=strToken.nextToken();
                                afterComma=strToken.nextToken();
                        }

                        if(afterComma.length()<decNum.length()){
                                int loop = decNum.length() - afterComma.length();
    ...
    public static void main(String args[]){
        ...
        String str = Formater.formatShortMonthyYear(dts);
