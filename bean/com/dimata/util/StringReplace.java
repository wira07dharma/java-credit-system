...
public class StringReplace {
     ...
     */
    public static String replaceChar(String source, String oldString, String newString){
           String result = ""; 
           String tmpResult = "";
    ...
    {
           String result = "";            
           String[] strOldStringSplit = source.split(oldString);
           int maxSentence = strOldStringSplit.length;
           for(int i=0; i<maxSentence; i++)
           {
               result = result + " " + strOldStringSplit[i].trim() + " " + newString;               
           }           
           
           if( result.length() > (newString.length()+1) )
           {
               result = result.substring(0, result.length()-newString.length()-1);
           }
