...
public class PstEmployee extends DBHandler implements I_DBInterface, I_DBType, I_PersintentExc, I_Language { 
    ...
    public static final  int FLD_LOCATION_ID = 30;
	...
	public static Vector listEmpJoinDept(int limitStart,int recordToGet, String whereClause, String order){
		Vector lists = new Vector(); 
		DBResultSet dbrs = null;
		try {
			String sql = "SELECT * FROM " + TBL_HR_EMPLOYEE +" INNER JOIN "
					+ PstDepartment.TBL_HR_DEPARTMENT + " ON "
			...
			while(rs.next()) {
				...
				resultToObject(rs, employee);
				//PstDepartment.resultToObject(rs, dept);
				temp.add(employee);
				temp.add(dept);
				lists.add(temp);
			}
    ...
    **/
    ...
    public static Vector getListFromApiWithResigned(int limitStart, int recordToGet, String whereClause, String order) {
        JSONArray lists = new JSONArray();
        String hrApiUrl = PstSystemProperty.getValueByName("HARISMA_URL");
        String param = "limitStart=" + WebServices.encodeUrl("" + limitStart) + "&recordToGet=" + WebServices.encodeUrl("" + recordToGet)
                + "&whereClause=" + WebServices.encodeUrl(whereClause) + "&order=" + WebServices.encodeUrl("" + order);
        JSONObject jo = WebServices.getAPIWithParam("", hrApiUrl + "/employee/employee-list-with-resigned", param);
        Vector listData = new Vector(1, 1);
        try {
            lists = jo.getJSONArray("DATA");
            for (int i = 0; i < lists.length(); i++) {
    ...
    */
    ...
    public static Employee fetchFromApiWResigned(long oid) {
        String whereClause = "emp." + PstEmployee.fieldNames[PstEmployee.FLD_EMPLOYEE_ID] + " = " + oid;
        Vector listData = getListFromApiWithResigned(0, 0, whereClause, "");
        Employee emp = new Employee();
        if (!listData.isEmpty()) {
            emp = (Employee) listData.get(0);
        }
    ...
    public static Hashtable getHashTableEmployee(AppUser user, Pinjaman pinjaman){
        Hashtable result = new Hashtable();
        try {
            String managerOidStr = PstSystemProperty.getValueByName("SEDANA_MANAGER_OPERASIONAL_OID");
            long managerPosId = 0;
            if(!managerOidStr.equals("") && managerOidStr.length() > 0 && !managerOidStr.equals("Not initialized")){
                managerPosId = Long.parseLong(managerOidStr);
            }
            String whereClause = " emp." + PstEmployee.fieldNames[PstEmployee.FLD_POSITION_ID] + " = " + managerPosId
                    + " AND dv." + PstDivision.fieldNames[PstDivision.FLD_LOCATION_ID] + " = " + user.getAssignLocationId();
            Vector<Employee> listManagerOpr = PstEmployee.getListFromApi(0, 0, whereClause, "");
            Employee managerObj = new Employee();
            Hashtable managerHash = new Hashtable();
            Hashtable analisHash = new Hashtable();
            Hashtable kolektorHash = new Hashtable();
            if(!listManagerOpr.isEmpty()){
                managerObj = listManagerOpr.get(0);
            }
            Employee analisObj = PstEmployee.fetchFromApi(pinjaman.getAccountOfficerId());
            Employee kolektorObj = PstEmployee.fetchFromApi(pinjaman.getCollectorId());
                
            objectToHashtable(managerHash, managerObj);
            objectToHashtable(analisHash, analisObj);
            objectToHashtable(kolektorHash, kolektorObj);
                       
            result.put("MANAGER_OPR", managerHash);
            result.put("ANALIS_KREDIT", managerHash);
            result.put("KOLEKTOR_KREDIT", managerHash);
                        
        } catch (Exception e) {
            System.out.println("Exception getHashTableEmployee " + e.getMessage());
        }
