...
public class PstEmpDoc extends DBHandler implements I_DBInterface, I_DBType, I_PersintentExc, I_Language {
    ...
    public static Hashtable fetchExcHashtable(long oid) throws DBException {
        try {
            ...
            String noSpp = "";
            try {
                SessAutoCode codeSpp = new SessAutoCode();
                codeSpp.setSysProp("CODE_SPP");
                codeSpp.setDb(TBL_HR_EMP_DOC);
                codeSpp.setDbFieldToCheck(fieldNames[FLD_DOC_MASTER_ID]);
                
                Pinjaman p = PstPinjaman.fetchExc(empDoc.getPinjamanId());
                String posApiUrl = PstSystemProperty.getValueByName("POS_API_URL");
                JSONObject jo = WebServices.getAPI("", posApiUrl + "/bill/billmain/" + p.getBillMainId());
                long oidLocation = jo.optLong("LOCATION_ID", 0);
                noSpp = codeSpp.generate(oidLocation);                
            } catch (Exception e) {
            }
            empDocH.put("SPP_DOC_NUMBER", noSpp);
