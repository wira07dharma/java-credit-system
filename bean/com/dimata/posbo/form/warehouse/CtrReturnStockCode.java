...
public class CtrReturnStockCode extends Control implements I_Language {
    ...
    public String requestBarcode(int cnt, long oid, HttpServletRequest req) {
        try {
            for (int k = 0; k < cnt; k++) {
                long sts_chk = FRMQueryString.requestInt(req, "" + FrmSourceStockCode.fieldNames[FrmSourceStockCode.FRM_FIELD_STOCK_CODE] + "_" + k);
                //System.out.println("sts_chk : " + k + " - " + sts_chk);
                if (sts_chk != 0) {
                    ...
                    try {
                        ...
                        materialStockCode.setStockStatus(PstMaterialStockCode.FLD_STOCK_STATUS_RETURN);
                        PstMaterialStockCode.updateExc(materialStockCode);
                    } catch (Exception e) {
                    }
                    ...
                    try {
                        ...
                        System.out.println("EXC requestBarcode : " + e.toString());
                    }
                }
            }
        } catch (Exception e) {
            strError = "Data tidak dapat disimpan.";
            System.out.println("EXC requestBarcode retur : " + e.toString());
        }
        return "";
    }
}
