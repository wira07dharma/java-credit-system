    ...
    public static void insertExc(StockCardReport stockCardReport) throws DBException {
        try {
            ...
            String sql = "INSERT INTO "+TBL_POS_STOCK_CARD_REPORT+
                    "("+fieldNames[FLD_TANGGAL]+","+fieldNames[FLD_DOC_TYPE]+
                    ","+fieldNames[FLD_DOC_CODE]+","+fieldNames[FLD_QTY]+","+fieldNames[FLD_KETERANGAN]+","+fieldNames[FLD_TRANSACTION_TYPE]+")"+
                    "VALUES("+s+","+stockCardReport.getDocType()+
                    ",'"+stockCardReport.getDocCode()+"',"+stockCardReport.getQty()+",'"+stockCardReport.getKeterangan()+"',"+stockCardReport.getTransaction_type()+")";
