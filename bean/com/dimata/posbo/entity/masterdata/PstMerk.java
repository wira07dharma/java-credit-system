...
public class PstMerk extends DBHandler implements I_DBInterface, I_DBType, I_PersintentExc, I_Language {
   ...
   public static long syncExc(JSONObject jSONObject){
      long oid = 0;
      if (jSONObject != null){
       oid = jSONObject.optLong(PstMerk.fieldNames[PstMerk.FLD_MERK_ID],0);
         if (oid > 0){
          Merk merk = new Merk();
          merk.setOID(jSONObject.optLong(PstMerk.fieldNames[PstMerk.FLD_MERK_ID],0));
          merk.setName(jSONObject.optString(PstMerk.fieldNames[PstMerk.FLD_NAME], ""));
          merk.setCode(jSONObject.optString(PstMerk.fieldNames[PstMerk.FLD_CODE], ""));
         boolean checkOidMerk = PstMerk.checkOID(oid);
          try{
            if(checkOidMerk){
               PstMerk.updateExc(merk);
            }else{
               PstMerk.insertByOid(merk);
            }
         }catch(Exception exc){}
         }
      }
   ...
   public static long insertByOid(Merk merk) throws DBException {
      try {
         PstMerk pstMerk = new PstMerk(0);
         pstMerk.setLong(FLD_MERK_ID, merk.getOID());
         pstMerk.setString(FLD_NAME, merk.getName());
         pstMerk.setString(FLD_CODE, merk.getCode());
         pstMerk.insertByOid(merk.getOID());
      } catch (DBException dbe) {
         throw dbe;
      } catch (Exception e) {
         throw new DBException(new PstMerk(0), DBException.UNKNOWN);
