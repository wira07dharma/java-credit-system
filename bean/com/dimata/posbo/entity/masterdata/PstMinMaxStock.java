...
public class PstMinMaxStock extends DBHandler implements I_DBInterface, I_DBType, I_PersintentExc, I_Language {
   ...
   public static long syncExc(JSONObject jSONObject){
      long oid = 0;
      if (jSONObject != null){
       oid = jSONObject.optLong(PstMinMaxStock.fieldNames[PstMinMaxStock.FLD_MINMAXSTOCK_ID],0);
         if (oid > 0){
          MinMaxStock minMaxStock = new MinMaxStock();
          minMaxStock.setOID(jSONObject.optLong(PstMinMaxStock.fieldNames[PstMinMaxStock.FLD_MINMAXSTOCK_ID],0));
          minMaxStock.setMaterialId(jSONObject.optLong(PstMinMaxStock.fieldNames[PstMinMaxStock.FLD_MATERIAL_ID],0));
          minMaxStock.setLocationId(jSONObject.optLong(PstMinMaxStock.fieldNames[PstMinMaxStock.FLD_LOCATION_ID],0));
          minMaxStock.setQtyMin(jSONObject.optDouble(PstMinMaxStock.fieldNames[PstMinMaxStock.FLD_QTY_MIN],0));
          minMaxStock.setQtyMax(jSONObject.optDouble(PstMinMaxStock.fieldNames[PstMinMaxStock.FLD_QTY_MAX],0));
      boolean checkOidMinMaxStock = PstMinMaxStock.checkOID(oid);
          try{
            if(checkOidMinMaxStock){
               PstMinMaxStock.updateExc(minMaxStock);
            }else{
               PstMinMaxStock.insertByOid(minMaxStock);
            }
         }catch(Exception exc){}
         }
      }
   return oid;
     ...
     public static long insertByOid(MinMaxStock minMaxStock) throws DBException {
      try {
         PstMinMaxStock pstMinMaxStock = new PstMinMaxStock(0);
         pstMinMaxStock.setLong(FLD_MINMAXSTOCK_ID, minMaxStock.getOID());
         pstMinMaxStock.setLong(FLD_MATERIAL_ID, minMaxStock.getMaterialId());
         pstMinMaxStock.setLong(FLD_LOCATION_ID, minMaxStock.getLocationId());
         pstMinMaxStock.setDouble(FLD_QTY_MIN, minMaxStock.getQtyMin());
         pstMinMaxStock.setDouble(FLD_QTY_MAX, minMaxStock.getQtyMax());
         pstMinMaxStock.insertByOid(minMaxStock.getOID());
      } catch (DBException dbe) {
         throw dbe;
      } catch (Exception e) {
         throw new DBException(new PstMinMaxStock(0), DBException.UNKNOWN);
