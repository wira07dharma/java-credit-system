...
public class PstCosting extends DBHandler implements I_DBInterface, I_DBType, I_PersintentExc, I_Language {
   ...
   public static long syncExc(JSONObject jSONObject){
      long oid = 0;
      if (jSONObject != null){
       oid = jSONObject.optLong(PstCosting.fieldNames[PstCosting.FLD_COSTING_ID],0);
         if (oid > 0){
          Costing costing = new Costing();
          costing.setOID(jSONObject.optLong(PstCosting.fieldNames[PstCosting.FLD_COSTING_ID],0));
          costing.setName(jSONObject.optString(PstCosting.fieldNames[PstCosting.FLD_NAME], ""));
          costing.setDescription(jSONObject.optString(PstCosting.fieldNames[PstCosting.FLD_DESCRIPTION], ""));
          boolean checkOidCosting = PstCosting.checkOID(oid);
          try{
            if(checkOidCosting){
               PstCosting.updateExc(costing);
            }else{
               PstCosting.insertByOid(costing);
            }
         }catch(Exception exc){}
         }
      }
   ...
   public static long insertByOid(Costing costing) throws DBException {
      try {
         PstCosting pstCosting = new PstCosting(0);
         pstCosting.setLong(FLD_COSTING_ID, costing.getOID());
         pstCosting.setString(FLD_NAME, costing.getName());
         pstCosting.setString(FLD_DESCRIPTION, costing.getDescription());
         pstCosting.insertByOid(costing.getOID());
      } catch (DBException dbe) {
         throw dbe;
      } catch (Exception e) {
         throw new DBException(new PstCosting(0), DBException.UNKNOWN);
