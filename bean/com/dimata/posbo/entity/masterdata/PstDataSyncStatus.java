...
//public static final  String TBL_P2_LOCATION = "LOCATION";
   ...
   public static long syncExc(JSONObject jSONObject){
      long oid = 0;
      if (jSONObject != null){
       oid = jSONObject.optLong(PstDataSyncStatus.fieldNames[PstDataSyncStatus.FLD_ID_DATA_SYNC_STATUS],0);
         if (oid > 0){
          DataSyncStatus dataSyncStatus = new DataSyncStatus();
          dataSyncStatus.setId_data_sync_status(jSONObject.optLong(PstDataSyncStatus.fieldNames[PstDataSyncStatus.FLD_ID_DATA_SYNC_STATUS],0));
          dataSyncStatus.setId_data_sync(jSONObject.optLong(PstDataSyncStatus.fieldNames[PstDataSyncStatus.FLD_ID_DATA_SYNC],0));
          dataSyncStatus.setId_dbconnection(jSONObject.optLong(PstDataSyncStatus.fieldNames[PstDataSyncStatus.FLD_ID_DBCONNECTION],0));
          dataSyncStatus.setStatus(jSONObject.optString(PstDataSyncStatus.fieldNames[PstDataSyncStatus.FLD_STATUS], ""));
         boolean checkOidDataSyncStatus = PstDataSyncStatus.checkOID(oid);
          try{
            if(checkOidDataSyncStatus){
               PstDataSyncStatus.updateExc(dataSyncStatus);
            }else{
               PstDataSyncStatus.insertByOid(dataSyncStatus);
            }
         }catch(Exception exc){}
         }
      }
   ...
   public static long insertByOid(DataSyncStatus dataSyncStatus) throws DBException {
      try {
         PstDataSyncStatus pstDataSyncStatus = new PstDataSyncStatus(0);
         pstDataSyncStatus.setLong(FLD_ID_DATA_SYNC_STATUS, dataSyncStatus.getId_data_sync_status());
         pstDataSyncStatus.setLong(FLD_ID_DATA_SYNC, dataSyncStatus.getId_data_sync());
         pstDataSyncStatus.setLong(FLD_ID_DBCONNECTION, dataSyncStatus.getId_dbconnection());
         pstDataSyncStatus.setString(FLD_STATUS, dataSyncStatus.getStatus());
         pstDataSyncStatus.insertByOid(dataSyncStatus.getOID());
      } catch (DBException dbe) {
         throw dbe;
      } catch (Exception e) {
         throw new DBException(new PstDataSyncStatus(0), DBException.UNKNOWN);
      }
      return dataSyncStatus.getOID();
   }
}
