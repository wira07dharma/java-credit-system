...
public class PstDataSyncSql extends DBHandler implements I_DBInterface, I_DBType, I_PersintentExc, I_Language {
   ...
   public static long syncExc(JSONObject jSONObject){
      long oid = 0;
      if (jSONObject != null){
       oid = jSONObject.optLong(PstDataSyncSql.fieldNames[PstDataSyncSql.FLD_DATA_SYNC_ID],0);
         if (oid > 0){
          DataSyncSql dataSyncSql = new DataSyncSql();
          dataSyncSql.setId_data_sync(jSONObject.optLong(PstDataSyncSql.fieldNames[PstDataSyncSql.FLD_DATA_SYNC_ID],0));
          dataSyncSql.setQuery(jSONObject.optString(PstDataSyncSql.fieldNames[PstDataSyncSql.FLD_QUERY], ""));
          boolean checkOidDataSyncSql = PstDataSyncSql.checkOID(oid);
          try{
            if(checkOidDataSyncSql){
               PstDataSyncSql.updateExc(dataSyncSql);
            }else{
               PstDataSyncSql.insertByOid(dataSyncSql);
            }
         }catch(Exception exc){}
         }
      }
   ...
   public static long insertByOid(DataSyncSql dataSyncSql) throws DBException {
      try {
         PstDataSyncSql pstDataSyncSql = new PstDataSyncSql(0);
         pstDataSyncSql.setLong(FLD_DATA_SYNC_ID, dataSyncSql.getId_data_sync());
         pstDataSyncSql.setString(FLD_QUERY, dataSyncSql.getQuery());
         pstDataSyncSql.insertByOid(dataSyncSql.getOID());
      } catch (DBException dbe) {
         throw dbe;
      } catch (Exception e) {
         throw new DBException(new PstDataSyncSql(0), DBException.UNKNOWN);
