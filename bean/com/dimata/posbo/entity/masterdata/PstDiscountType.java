...
public class PstDiscountType extends DBHandler implements I_DBInterface, I_DBType, I_PersintentExc, I_Language {
   ...
   public static long syncExc(JSONObject jSONObject){
      long oid = 0;
      if (jSONObject != null){
       oid = jSONObject.optLong(PstDiscountType.fieldNames[PstDiscountType.FLD_DISCOUNT_TYPE_ID],0);
         if (oid > 0){
          DiscountType discountType = new DiscountType();
          discountType.setOID(jSONObject.optLong(PstDiscountType.fieldNames[PstDiscountType.FLD_DISCOUNT_TYPE_ID],0));
          discountType.setCode(jSONObject.optString(PstDiscountType.fieldNames[PstDiscountType.FLD_CODE], ""));
          discountType.setName(jSONObject.optString(PstDiscountType.fieldNames[PstDiscountType.FLD_NAME], ""));
         boolean checkOidDiscountType = PstDiscountType.checkOID(oid);
          try{
            if(checkOidDiscountType){
               PstDiscountType.updateExc(discountType);
            }else{
               PstDiscountType.insertByOid(discountType);
            }
         }catch(Exception exc){}
         }
      }
   ...
   public static long insertByOid(DiscountType discountType) throws DBException {
      try {
         PstDiscountType pstDiscountType = new PstDiscountType(0);
         pstDiscountType.setLong(FLD_DISCOUNT_TYPE_ID, discountType.getOID());
         pstDiscountType.setString(FLD_CODE, discountType.getCode());
         pstDiscountType.setString(FLD_NAME, discountType.getName());
         pstDiscountType.insertByOid(discountType.getOID());
      } catch (DBException dbe) {
         throw dbe;
      } catch (Exception e) {
         throw new DBException(new PstDiscountType(0), DBException.UNKNOWN);
