...
public class PstDiscountMapping extends DBHandler implements I_DBInterface, I_DBType, I_PersintentExc, I_Language {
   ...
   public static long syncExc(JSONObject jSONObject){
      ...
      if (jSONObject != null){
       discountTypeId   = jSONObject.optLong(PstDiscountMapping.fieldNames[PstDiscountMapping.FLD_DISCOUNT_TYPE_ID],0);
       materialId       = jSONObject.optLong(PstDiscountMapping.fieldNames[PstDiscountMapping.FLD_MATERIAL_ID],0);
       currencyTypeId   = jSONObject.optLong(PstDiscountMapping.fieldNames[PstDiscountMapping.FLD_CURRENCY_TYPE_ID],0);
         if (discountTypeId > 0 && materialId >0 && currencyTypeId >0  ){
          DiscountMapping discountMapping = new DiscountMapping();
          discountMapping.setOID(jSONObject.optLong(PstDiscountMapping.fieldNames[PstDiscountMapping.FLD_DISCOUNT_TYPE_ID],0));
          discountMapping.setMaterialId(jSONObject.optLong(PstDiscountMapping.fieldNames[PstDiscountMapping.FLD_MATERIAL_ID],0));
          discountMapping.setCurrencyTypeId(jSONObject.optLong(PstDiscountMapping.fieldNames[PstDiscountMapping.FLD_CURRENCY_TYPE_ID],0));
          discountMapping.setDiscountPct(jSONObject.optDouble(PstDiscountMapping.fieldNames[PstDiscountMapping.FLD_DISCOUNT_PCT],0));
          discountMapping.setDiscountValue(jSONObject.optDouble(PstDiscountMapping.fieldNames[PstDiscountMapping.FLD_DISCOUNT_VALUE],0));
         boolean checkOidDiscountMapping = PstDiscountMapping.checkOID(discountTypeId, materialId, currencyTypeId);
          try{
            if(checkOidDiscountMapping){
               PstDiscountMapping.updateExc(discountMapping);
            }else{
               PstDiscountMapping.insertByOid(discountMapping);
            }
         }catch(Exception exc){}
         }
      }
   ...
   public static long insertByOid(DiscountMapping discountMapping) throws DBException {
      try {
         PstDiscountMapping pstDiscountMapping = new PstDiscountMapping(0);
         pstDiscountMapping.setLong(FLD_DISCOUNT_TYPE_ID, discountMapping.getOID());
         pstDiscountMapping.setLong(FLD_MATERIAL_ID, discountMapping.getDiscountType());
         pstDiscountMapping.setLong(FLD_CURRENCY_TYPE_ID, discountMapping.getDiscountTypeId());
         pstDiscountMapping.setDouble(FLD_DISCOUNT_PCT, discountMapping.getDiscountPct());
         pstDiscountMapping.setDouble(FLD_DISCOUNT_VALUE, discountMapping.getDiscountValue());
         pstDiscountMapping.insertByOid(discountMapping.getOID());
      } catch (DBException dbe) {
         throw dbe;
      } catch (Exception e) {
         throw new DBException(new PstDiscountMapping(0), DBException.UNKNOWN);
