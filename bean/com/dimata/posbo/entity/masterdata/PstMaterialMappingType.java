...
public class PstMaterialMappingType extends DBHandler implements I_DBInterface, I_DBType, I_PersintentExc, I_Language {
    ...
    public static long syncExc(JSONObject jSONObject){
      long oid = 0;
      if (jSONObject != null){
       oid = jSONObject.optLong(PstMaterialMappingType.fieldNames[PstMaterialMappingType.FLD_MATERIAL_TYPE_MAPPING_ID],0);
         if (oid > 0){
          MaterialTypeMapping materialTypeMapping = new MaterialTypeMapping();
          materialTypeMapping.setOID(jSONObject.optLong(PstMaterialMappingType.fieldNames[PstMaterialMappingType.FLD_MATERIAL_TYPE_MAPPING_ID],0));
          materialTypeMapping.setMaterialId(jSONObject.optLong(PstMaterialMappingType.fieldNames[PstMaterialMappingType.FLD_MATERIAL_ID],0));
          materialTypeMapping.setNote(jSONObject.optString(PstMaterialMappingType.fieldNames[PstMaterialMappingType.FLD_NOTE], ""));
          materialTypeMapping.setTypeId(jSONObject.optLong(PstMaterialMappingType.fieldNames[PstMaterialMappingType.FLD_TYPE_ID],0));

          boolean chekOidMaterial = PstMaterialMappingType.checkOID(oid);
          try{
              if(chekOidMaterial){
                  PstMaterialMappingType.updateExc(materialTypeMapping);
              }else{
                  PstMaterialMappingType.insertByOid(materialTypeMapping);
}
          }catch(Exception exc){}
         }
      }
   return oid;
    ...
    public static long insertByOid(MaterialTypeMapping entMaterialTypeMapping) throws DBException {
        try {
      PstMaterialMappingType pstMaterialTypeMapping = new PstMaterialMappingType(0);
      pstMaterialTypeMapping.setLong(FLD_MATERIAL_ID, entMaterialTypeMapping.getMaterialId());
      pstMaterialTypeMapping.setLong(FLD_TYPE_ID, entMaterialTypeMapping.getTypeId());
      pstMaterialTypeMapping.setString(FLD_NOTE, entMaterialTypeMapping.getNote());
      pstMaterialTypeMapping.insertByOid(entMaterialTypeMapping.getOID());
    } catch (DBException dbe) {
      throw dbe;  
