...
public class PstUnit extends DBHandler implements I_DBInterface, I_DBType, I_PersintentExc, I_Language {
   ...
   public static long syncExc(JSONObject jSONObject){
      long oid = 0;
      if (jSONObject != null){
       oid = jSONObject.optLong(PstCategory.fieldNames[PstUnit.FLD_UNIT_ID],0);
         if (oid > 0){
          Unit unit = new Unit();
          unit.setOID(jSONObject.optLong(PstUnit.fieldNames[PstUnit.FLD_UNIT_ID],0));
          unit.setCode(jSONObject.optString(PstUnit.fieldNames[PstUnit.FLD_CODE], ""));
          unit.setName(jSONObject.optString(PstUnit.fieldNames[PstUnit.FLD_NAME], ""));
          unit.setBaseUnitId(jSONObject.optLong(PstUnit.fieldNames[PstUnit.FLD_BASE_UNIT_ID],0));
          unit.setQtyPerBaseUnit(jSONObject.optDouble(PstUnit.fieldNames[PstUnit.FLD_QTY_PER_BASE_UNIT],0));
         boolean checkOidUnit = PstUnit.checkOID(oid);
          try{
            if(checkOidUnit){
               PstUnit.updateExc(unit);
            }else{
               PstUnit.insertByOid(unit);
            }
         }catch(Exception exc){}
         }
      }
   ...
   public static long insertByOid(Unit unit) throws DBException {
      try {
         PstUnit pstUnit = new PstUnit(0);
         pstUnit.setLong(FLD_UNIT_ID, unit.getOID());
         pstUnit.setString(FLD_CODE, unit.getCode());
         pstUnit.setString(FLD_NAME, unit.getName());
         pstUnit.setDouble(FLD_QTY_PER_BASE_UNIT, unit.getQtyPerBaseUnit());
         pstUnit.setLong(FLD_BASE_UNIT_ID, unit.getBaseUnitId());
         pstUnit.insertByOid(unit.getOID());
      } catch (DBException dbe) {
         throw dbe;
      } catch (Exception e) {
         throw new DBException(new PstUnit(0), DBException.UNKNOWN);
