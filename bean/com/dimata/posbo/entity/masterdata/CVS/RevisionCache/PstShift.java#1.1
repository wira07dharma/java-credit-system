...
public class PstShift extends DBHandler implements I_DBInterface, I_DBType, I_PersintentExc, I_Language {
   ...
   public static long syncExc(JSONObject jSONObject){
      long oid = 0;
      if (jSONObject != null){
       oid = jSONObject.optLong(PstShift.fieldNames[PstShift.FLD_SHIFT_ID],0);
         if (oid > 0){
          Shift shift = new Shift();
          shift.setOID(jSONObject.optLong(PstShift.fieldNames[PstShift.FLD_SHIFT_ID],0));
          shift.setName(jSONObject.optString(PstShift.fieldNames[PstShift.FLD_NAME], ""));
          shift.setRemark(jSONObject.optString(PstShift.fieldNames[PstShift.FLD_REMARK], ""));
          shift.setStartTime(Formater.formatDate(jSONObject.optString(PstShift.fieldNames[PstShift.FLD_START_TIME],"0000-00-00"),"yyyy-MM-dd"));
          shift.setEndTime(Formater.formatDate(jSONObject.optString(PstShift.fieldNames[PstShift.FLD_END_TIME],"0000-00-00"),"yyyy-MM-dd"));
          
          boolean checkOidShift = PstShift.checkOID(oid);
          try{
              if(checkOidShift){
                  PstShift.updateExc(shift);
              }else{
                  PstShift.insertByOid(shift);
              }
          }catch(Exception exc){}
         }
      }
   return oid;
     ...
     public static long insertByOid(Shift shift) throws DBException {
      try {
         PstShift pstShift = new PstShift(0);
         pstShift.setLong(FLD_SHIFT_ID, shift.getOID());
         pstShift.setString(FLD_NAME, shift.getName());
         pstShift.setString(FLD_REMARK, shift.getRemark());
         pstShift.setDate(FLD_START_TIME, shift.getStartTime());
         pstShift.setDate(FLD_END_TIME, shift.getEndTime());
         pstShift.insertByOid(shift.getOID());
      } catch (DBException dbe) {
         throw dbe;
      } catch (Exception e) {
         throw new DBException(new PstShift(0), DBException.UNKNOWN);
