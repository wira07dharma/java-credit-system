...
public class PstKsg extends DBHandler implements I_DBInterface, I_DBType, I_PersintentExc, I_Language {
   ...
   public static long syncExc(JSONObject jSONObject){
      long oid = 0;
      if (jSONObject != null){
       oid = jSONObject.optLong(PstKsg.fieldNames[PstKsg.FLD_KSG_ID],0);
         if (oid > 0){
          Ksg ksg = new Ksg();
          ksg.setOID(jSONObject.optLong(PstKsg.fieldNames[PstKsg.FLD_KSG_ID],0));
          ksg.setName(jSONObject.optString(PstKsg.fieldNames[PstKsg.FLD_NAME], ""));
          ksg.setCode(jSONObject.optString(PstKsg.fieldNames[PstKsg.FLD_CODE], ""));
         boolean checkOidKsg = PstKsg.checkOID(oid);
          try{
            if(checkOidKsg){
               PstKsg.updateExc(ksg);
            }else{
               PstKsg.insertByOid(ksg);
            }
         }catch(Exception exc){}
         }
      }
   ...
   public static long insertByOid(Ksg ksg) throws DBException {
      try {
         PstKsg pstKsg = new PstKsg(0);
         pstKsg.setLong(FLD_KSG_ID, ksg.getOID());
         pstKsg.setString(FLD_NAME, ksg.getName());
         pstKsg.setString(FLD_CODE, ksg.getCode());
         pstKsg.insertByOid(ksg.getOID());
      } catch (DBException dbe) {
         throw dbe;
      } catch (Exception e) {
         throw new DBException(new PstKsg(0), DBException.UNKNOWN);
