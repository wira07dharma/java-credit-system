...
public class PstMemberGroup extends DBHandler implements I_DBInterface, I_DBType, I_PersintentExc, I_Language {
   ...
   public static long syncExc(JSONObject jSONObject){
      long oid = 0;
      if (jSONObject != null){
       oid = jSONObject.optLong(PstMemberGroup.fieldNames[PstMemberGroup.FLD_MEMBER_GROUP_ID],0);
         if (oid > 0){
          MemberGroup memberGroup = new MemberGroup();
          memberGroup.setOID(jSONObject.optLong(PstMemberGroup.fieldNames[PstMemberGroup.FLD_MEMBER_GROUP_ID],0));
          memberGroup.setDiscountTypeId(jSONObject.optLong(PstMemberGroup.fieldNames[PstMemberGroup.FLD_DISCOUNT_TYPE_ID],0));
          memberGroup.setPriceTypeId(jSONObject.optLong(PstMemberGroup.fieldNames[PstMemberGroup.FLD_PRICE_TYPE_ID],0));
          memberGroup.setCode(jSONObject.optString(PstMemberGroup.fieldNames[PstMemberGroup.FLD_CODE], ""));
          memberGroup.setName(jSONObject.optString(PstMemberGroup.fieldNames[PstMemberGroup.FLD_NAME], ""));
          memberGroup.setDescription(jSONObject.optString(PstMemberGroup.fieldNames[PstMemberGroup.FLD_DESCRIPTION], ""));
          memberGroup.setGroupType(jSONObject.optInt(PstMemberGroup.fieldNames[PstMemberGroup.FLD_GROUP_TYPE],0));
         boolean checkOidMemberGroup = PstMemberGroup.checkOID(oid);
          try{
            if(checkOidMemberGroup){
               PstMemberGroup.updateExc(memberGroup);
            }else{
               PstMemberGroup.insertByOid(memberGroup);
            }
         }catch(Exception exc){}
         }
      }
   return oid;
    ...
    public static long insertByOid(MemberGroup memberGroup) throws DBException {
      try {
         PstMemberGroup pstMemberGroup = new PstMemberGroup(0);
         pstMemberGroup.setLong(FLD_MEMBER_GROUP_ID, memberGroup.getOID());
         pstMemberGroup.setLong(FLD_DISCOUNT_TYPE_ID, memberGroup.getDiscountTypeId());
         pstMemberGroup.setLong(FLD_PRICE_TYPE_ID, memberGroup.getPriceTypeId());
         pstMemberGroup.setString(FLD_CODE, memberGroup.getCode());
         pstMemberGroup.setString(FLD_NAME, memberGroup.getName());
         pstMemberGroup.setString(FLD_DESCRIPTION, memberGroup.getDescription());
         pstMemberGroup.setInt(FLD_GROUP_TYPE, memberGroup.getGroupType());
         pstMemberGroup.insertByOid(memberGroup.getOID());
      } catch (DBException dbe) {
         throw dbe;
      } catch (Exception e) {
         throw new DBException(new PstMemberGroup(0), DBException.UNKNOWN);
