...
public class PstColor extends DBHandler implements I_DBInterface, I_DBType, I_PersintentExc, I_Language {
    ...
    public static long syncExc(JSONObject jSONObject){
      long oid = 0;
      if (jSONObject != null){
       oid = jSONObject.optLong(PstColor.fieldNames[PstColor.FLD_COLOR_ID],0);
         if (oid > 0){
          Color enColor = new Color();
          enColor.setOID(jSONObject.optLong(PstColor.fieldNames[PstColor.FLD_COLOR_ID],0));
          enColor.setColorCode(jSONObject.optString(PstColor.fieldNames[PstColor.FLD_COLOR_CODE],""));
          enColor.setColorName(jSONObject.optString(PstColor.fieldNames[PstColor.FLD_COLOR_NAME], ""));

          boolean chekOidMaterial = PstColor.checkOID(oid);
          try{
              if(chekOidMaterial){
                  PstColor.updateExc(enColor);
              }else{
                  PstColor.insertByOid(enColor);
              }
          }catch(Exception exc){}
         }
      }
   return oid;
    ...
    public static long insertByOid(Color entColor) throws DBException {
        try {
      PstColor pstColor = new PstColor(0);
      pstColor.setString(FLD_COLOR_NAME, entColor.getColorName());
      pstColor.setString(FLD_COLOR_CODE, entColor.getColorCode());
      pstColor.insertByOid(entColor.getOID());
    } catch (DBException dbe) { 
      throw dbe;  
