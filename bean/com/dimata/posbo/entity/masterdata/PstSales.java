...
public class PstSales extends DBHandler implements I_DBInterface, I_DBType, I_PersintentExc, I_Language {
	...
	public long insertExcWithOid(Entity ent) throws Exception {
        return insertExcWithOid((Sales) ent);
    }
	...
	public static long insertByOid(Sales matSales) throws DBException {
        try {
            ...
            pstSales.insertByOid(matSales.getOID());
        } catch (DBException dbe) {
            throw dbe;
        } catch (Exception e) {
            throw new DBException(new PstSales(0), DBException.UNKNOWN);
	...
	public static long syncExc(JSONObject jSONObject){
		long oid = 0;
		if (jSONObject != null){
			oid = jSONObject.optLong(PstSales.fieldNames[PstSales.FLD_SALES_ID], 0); 
			if (oid > 0){
				Sales sales = new Sales();
				sales.setOID(jSONObject.optLong(PstSales.fieldNames[PstSales.FLD_SALES_ID],0));
				sales.setCode(jSONObject.optString(PstSales.fieldNames[PstSales.FLD_CODE],""));
				sales.setName(jSONObject.optString(PstSales.fieldNames[PstSales.FLD_NAME],""));
				sales.setRemark(jSONObject.optString(PstSales.fieldNames[PstSales.FLD_REMARK],""));
				sales.setCommision(jSONObject.optDouble(PstSales.fieldNames[PstSales.FLD_COMMISION],0));
				sales.setLoginId(jSONObject.optString(PstSales.fieldNames[PstSales.FLD_LOGIN_ID],""));
				sales.setPassword(jSONObject.optString(PstSales.fieldNames[PstSales.FLD_PASSWORD],""));
				sales.setLocationId(jSONObject.optLong(PstSales.fieldNames[PstSales.FLD_ASSIGN_LOCATION_WAREHOUSE],0));
				sales.setDefaultCurrencyId(jSONObject.optLong(PstSales.fieldNames[PstSales.FLD_STANDARD_CURRENCY_ID],0));
				sales.setEmployeeId(jSONObject.optLong(PstSales.fieldNames[PstSales.FLD_EMPLOYEE_ID],0));
				sales.setStatus(jSONObject.optInt(PstSales.fieldNames[PstSales.FLD_STATUS],0));

				boolean checkOidSales = PstSales.checkOID(oid);
				try {
					if (checkOidSales){
						PstSales.updateExc(sales);
					} else {
						PstSales.insertByOid(sales);
					}
				} catch (Exception exc){}
			}
		}
