...
public class PstMatCurrency extends DBHandler implements I_DBInterface, I_DBType, I_PersintentExc, I_Language {
   ...
   public static long syncExc(JSONObject jSONObject){
      long oid = 0;
      if (jSONObject != null){
       oid = jSONObject.optLong(PstMatCurrency.fieldNames[PstMatCurrency.FLD_CURRENCY_ID],0);
         if (oid > 0){
          MatCurrency matCurrency = new MatCurrency();
          matCurrency.setOID(jSONObject.optLong(PstMatCurrency.fieldNames[PstMatCurrency.FLD_CURRENCY_ID],0));
          matCurrency.setCode(jSONObject.optString(PstMatCurrency.fieldNames[PstMatCurrency.FLD_CODE], ""));
          matCurrency.setName(jSONObject.optString(PstMatCurrency.fieldNames[PstMatCurrency.FLD_NAME], ""));
          matCurrency.setSellingRate(jSONObject.optDouble(PstMatCurrency.fieldNames[PstMatCurrency.FLD_SELLING_RATE],0));
          matCurrency.setBuyingRate(jSONObject.optDouble(PstMatCurrency.fieldNames[PstMatCurrency.FLD_BUYING_RATE],0));
         boolean checkOidMatCurrency = PstMatCurrency.checkOID(oid);
          try{
            if(checkOidMatCurrency){
               PstMatCurrency.updateExc(matCurrency);
            }else{
               PstMatCurrency.insertByOid(matCurrency);
            }
         }catch(Exception exc){}
         }
      }
   ...
   public static long insertByOid(MatCurrency matCurrency) throws DBException {
      try {
         PstMatCurrency pstMatCurrency = new PstMatCurrency(0);
         pstMatCurrency.setLong(FLD_CURRENCY_ID, matCurrency.getOID());
         pstMatCurrency.setString(FLD_CODE, matCurrency.getCode());
         pstMatCurrency.setString(FLD_NAME, matCurrency.getName());
         pstMatCurrency.setDouble(FLD_SELLING_RATE, matCurrency.getSellingRate());
         pstMatCurrency.setDouble(FLD_BUYING_RATE, matCurrency.getBuyingRate());
         pstMatCurrency.insertByOid(matCurrency.getOID());
      } catch (DBException dbe) {
         throw dbe;
      } catch (Exception e) {
         throw new DBException(new PstMatCurrency(0), DBException.UNKNOWN);
