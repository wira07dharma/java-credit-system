...
public class PstCodeRange extends DBHandler implements I_DBInterface, I_DBType, I_PersintentExc, I_Language {
   ...
   public static long syncExc(JSONObject jSONObject){
      long oid = 0;
      if (jSONObject != null){
       oid = jSONObject.optLong(PstCodeRange.fieldNames[PstCodeRange.FLD_RANGE_CODE_ID],0);
         if (oid > 0){
          CodeRange codeRange = new CodeRange();
          codeRange.setOID(jSONObject.optLong(PstCodeRange.fieldNames[PstCodeRange.FLD_RANGE_CODE_ID],0));
          codeRange.setFromRangeCode(jSONObject.optString(PstCodeRange.fieldNames[PstCodeRange.FLD_FROM_RANGE_CODE], ""));
          codeRange.setToRangeCode(jSONObject.optString(PstCodeRange.fieldNames[PstCodeRange.FLD_TO_RANGE_NAME], ""));
        boolean checkOidCodeRange = PstCodeRange.checkOID(oid);
          try{
            if(checkOidCodeRange){
               PstCodeRange.updateExc(codeRange);
            }else{
               PstCodeRange.insertByOid(codeRange);
            }
         }catch(Exception exc){}
         }
      }
   ...
   public static long insertByOid(CodeRange codeRange) throws DBException {
      try {
         PstCodeRange pstCodeRange = new PstCodeRange(0);
         pstCodeRange.setLong(FLD_RANGE_CODE_ID, codeRange.getOID());
         pstCodeRange.setString(FLD_FROM_RANGE_CODE, codeRange.getFromRangeCode());
         pstCodeRange.setString(FLD_TO_RANGE_NAME, codeRange.getToRangeCode());
         pstCodeRange.insertByOid(codeRange.getOID());
      } catch (DBException dbe) {
         throw dbe;
      } catch (Exception e) {
         throw new DBException(new PstCodeRange(0), DBException.UNKNOWN);
