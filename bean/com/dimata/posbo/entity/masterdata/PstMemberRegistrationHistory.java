...
public class PstMemberRegistrationHistory extends DBHandler implements I_DBInterface, I_DBType, I_PersintentExc, I_Language { 
   ...
   public static long syncExc(JSONObject jSONObject){
      long oid = 0;
      if (jSONObject != null){
       oid = jSONObject.optLong(PstMemberRegistrationHistory.fieldNames[PstMemberRegistrationHistory.FLD_MEMBER_REGISTRATION_HISTORY_ID],0);
         if (oid > 0){
          MemberRegistrationHistory memberRegistrationHistory = new MemberRegistrationHistory();
          memberRegistrationHistory.setOID(jSONObject.optLong(PstMemberRegistrationHistory.fieldNames[PstMemberRegistrationHistory.FLD_MEMBER_REGISTRATION_HISTORY_ID],0));
          memberRegistrationHistory.setMemberId(jSONObject.optLong(PstMemberRegistrationHistory.fieldNames[PstMemberRegistrationHistory.FLD_MEMBER_ID],0));
          memberRegistrationHistory.setRegistrationDate(Formater.formatDate(jSONObject.optString(PstMemberRegistrationHistory.fieldNames[PstMemberRegistrationHistory.FLD_REGISTRATION_DATE],"0000-00-00"),"yyyy-MM-dd"));
          memberRegistrationHistory.setValidStartDate(Formater.formatDate(jSONObject.optString(PstMemberRegistrationHistory.fieldNames[PstMemberRegistrationHistory.FLD_VALID_START_DATE],"0000-00-00"),"yyyy-MM-dd"));
          memberRegistrationHistory.setValidExpiredDate(Formater.formatDate(jSONObject.optString(PstMemberRegistrationHistory.fieldNames[PstMemberRegistrationHistory.FLD_VALID_EXPIRED_DATE],"0000-00-00"),"yyyy-MM-dd"));
         boolean checkOidMemberRegistrationHistory = PstMemberRegistrationHistory.checkOID(oid);
          try{
            if(checkOidMemberRegistrationHistory){
               PstMemberRegistrationHistory.updateExc(memberRegistrationHistory);
            }else{
               PstMemberRegistrationHistory.insertByOid(memberRegistrationHistory);
            }
         }catch(Exception exc){}
         }
      }
   ...
   public static long insertByOid(MemberRegistrationHistory memberRegistrationHistory) throws DBException {
      try {
         PstMemberRegistrationHistory pstMemberRegistrationHistory = new PstMemberRegistrationHistory(0);
         pstMemberRegistrationHistory.setLong(FLD_MEMBER_REGISTRATION_HISTORY_ID, memberRegistrationHistory.getOID());
         pstMemberRegistrationHistory.setLong(FLD_MEMBER_ID, memberRegistrationHistory.getMemberId());
         pstMemberRegistrationHistory.setDate(FLD_REGISTRATION_DATE, memberRegistrationHistory.getRegistrationDate());
         pstMemberRegistrationHistory.setDate(FLD_VALID_START_DATE, memberRegistrationHistory.getValidStartDate());
         pstMemberRegistrationHistory.setDate(FLD_VALID_EXPIRED_DATE, memberRegistrationHistory.getValidExpiredDate());
         pstMemberRegistrationHistory.insertByOid(memberRegistrationHistory.getOID());
      } catch (DBException dbe) {
         throw dbe;
      } catch (Exception e) {
         throw new DBException(new PstMemberRegistrationHistory(0), DBException.UNKNOWN);
