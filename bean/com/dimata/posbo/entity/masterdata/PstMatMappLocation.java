...
public class PstMatMappLocation extends DBHandler implements I_DBInterface, I_DBType, I_PersintentExc, I_Language {
   ...
   public static long syncExc(JSONObject jSONObject){
      ...
      long materialOid  = 0 ;
      if (jSONObject != null){
       locationOid = jSONObject.optLong(PstMatMappLocation.fieldNames[PstMatMappLocation.FLD_LOCATION_ID],0);
       materialOid  = jSONObject.optLong(PstMatMappLocation.fieldNames[PstMatMappLocation.FLD_MATERIAL_ID],0);
         if (locationOid > 0){
          MatMappLocation matMappLocation = new MatMappLocation();
          matMappLocation.setMaterialId(jSONObject.optLong(PstMatMappLocation.fieldNames[PstMatMappLocation.FLD_MATERIAL_ID],0));
          matMappLocation.setLocationId(jSONObject.optLong(PstMatMappLocation.fieldNames[PstMatMappLocation.FLD_LOCATION_ID],0));
         boolean checkOidMatMappLocation = PstMatMappLocation.checkOID(locationOid , materialOid);
          try{
            if(checkOidMatMappLocation){
               PstMatMappLocation.updateExc(matMappLocation);
            }else{
               PstMatMappLocation.insertByOid(matMappLocation);
            }
         }catch(Exception exc){}
         }
      }
   ...
   public static long insertByOid(MatMappLocation matMappLocation) throws DBException {
      try {
         PstMatMappLocation pstMatMappLocation = new PstMatMappLocation(0);
         pstMatMappLocation.setLong(FLD_MATERIAL_ID, matMappLocation.getMaterialId());
         pstMatMappLocation.setLong(FLD_LOCATION_ID, matMappLocation.getLocationId());
         pstMatMappLocation.insertByOid(matMappLocation.getOID());
      } catch (DBException dbe) {
         throw dbe;
      } catch (Exception e) {
         throw new DBException(new PstMatMappLocation(0), DBException.UNKNOWN);
