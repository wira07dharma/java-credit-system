...
public class SessMatDispatch extends Control {
    ...
    public static Vector listMatDispatch(SrcMatDispatch srcMatDispatch, int start, int limit) {
	...
	if (srcMatDispatch != null) {
		...
		status = "(DF." + PstMatDispatch.fieldNames[PstMatDispatch.FLD_DISPATCH_STATUS] + " = " + (srcMatDispatch.getStatus()) + ")";
            ...
            //+location type
            String locationType = "";
                locationType = " DF." + PstMatDispatch.fieldNames[PstMatDispatch.FLD_LOCATION_TYPE] + " != 2";
	...
	try {
		...
		if (whereClause.length() > 0) {
		    whereClause = whereClause + " AND R." + PstMatDispatch.fieldNames[PstMatDispatch.FLD_DISPATCH_DATE] + " BETWEEN '" + Formater.formatDate(srcStockCard.getStardDate(), "yyyy-MM-dd 00:00:01") + "' AND '" + Formater.formatDate(srcStockCard.getEndDate(), "yyyy-MM-dd 23:59:59") + "'";
		} else {
		...
		if (whereClause.length() > 0) {
		    whereClause = whereClause + " AND R." + PstMatDispatch.fieldNames[PstMatDispatch.FLD_DISPATCH_DATE] + " <= '" + Formater.formatDate(srcStockCard.getStardDate(), "yyyy-MM-dd 00:00:00") + "'";
		} else {
