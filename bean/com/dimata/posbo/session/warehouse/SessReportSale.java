    ...
    public static Vector getReportSaleRekapTanggal(SrcReportSale srcReportSale) {
        ...
        try {
            ...
            String sql2 = " union SELECT 0 AS SUM_PRICE" +
                    ",SUM(CBM." + PstBillMain.fieldNames[PstBillMain.FLD_DISCOUNT] + ") AS SUM_DISC " +
                    ",SUM(CBM." + PstBillMain.fieldNames[PstBillMain.FLD_SERVICE_VALUE] + ") AS SUM_SERVICE " +
                    ",SUM(CBM." + PstBillMain.fieldNames[PstBillMain.FLD_TAX_VALUE] + ") AS SUM_TAX " +
                    ", 0 AS HPP" +
                    ", CBM." + PstBillMain.fieldNames[PstBillMain.FLD_BILL_DATE] + // BILL_DATE+
                    " FROM " + PstBillMain.TBL_CASH_BILL_MAIN + " CBM" + // cash_bill_main
                    " INNER JOIN " + PstBillDetail.TBL_CASH_BILL_DETAIL + " CBD" + // cash_bill_detail
            ...
            if (strLocationId.length() > 0) {
                ...
                sql2 += " GROUP BY LEFT(CBM." + PstBillMain.fieldNames[PstBillMain.FLD_BILL_DATE]+",10)";
            }
            ...
            sql = "select sum(SUM_PRICE) as SUM_PRICE,sum(SUM_DISC) as SUM_DISC,sum(SUM_SERVICE) as SUM_SERVICE,sum(SUM_TAX) as SUM_TAX, sum(HPP) as HPP," + PstBillMain.fieldNames[PstBillMain.FLD_BILL_DATE] +
                    " from (" + sql + " " + sql2 + ") as tbl group by " + PstBillMain.fieldNames[PstBillMain.FLD_BILL_DATE];
    ...
    public static Vector getDataMaterial(SrcStockCard srcStockCard) {
        ...
        try {
            ...
            if (srcStockCard.getStardDate() != null && srcStockCard.getEndDate() != null) {
                if (whereClause.length() > 0) {
                    whereClause = whereClause + " AND R." + PstBillMain.fieldNames[PstBillMain.FLD_BILL_DATE] + " BETWEEN '" + Formater.formatDate(srcStockCard.getStardDate(), "yyyy-MM-dd 00:00:01") + "' AND '" + Formater.formatDate(srcStockCard.getEndDate(), "yyyy-MM-dd 23:59:59") + "'";
                } else {
                ...
                if (whereClause.length() > 0) {
                    whereClause = whereClause + " AND R." + PstBillMain.fieldNames[PstBillMain.FLD_BILL_DATE] + " <= '" + Formater.formatDate(srcStockCard.getStardDate(), "yyyy-MM-dd 00:00:00") + "'";
                } else {
    ...
    public static Vector ReportBrand(SrcReportSale srcReportSale) {
        ...
        try {    
            String sql = "select pmr.code, sum(bd.qty) as total_qty ,pmr.name from cash_bill_detail as bd " +
                    ...
                    " inner join pos_ksg pmr on pm.gondola_code=pmr.code " +
    ...
    public static Vector ReportRekapKategori(SrcReportSale srcReportSale) {
        ...
        try {
            String sql = "select pm.sku , pm.name as prod_name ,pmr.name, " +
                    " sum(bd.qty) as total_qty ,pmr.name, bd.total_price, " +
            ...
            while (rs.next()) {
                ...
                litem = new Vector(1, 1);
                litem.add(String.valueOf(i));
                litem.add(String.valueOf(rs.getString("sku")));
                litem.add(String.valueOf(rs.getString("prod_name")));
                litem.add(String.valueOf(rs.getInt("total_qty")));
                litem.add(String.valueOf(rs.getDouble("total_price")));
                litem.add(String.valueOf(rs.getString("name")));
                litem.add(String.valueOf(rs.getDouble("cost")));
                list.add(litem);
            }
        } catch (Exception e) {
            return new Vector();
