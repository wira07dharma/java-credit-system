...
import com.dimata.pos.cashier.CashBalanceDialog;
    ...
    public void run() {
		...
		vecCashCashier.add(cashCashier);
            ...
            for(int i=0;i<vecCashCashier.size();i++){
                    ...
                    sql="select count(" +PstBillMain.fieldNames[PstBillMain.FLD_BILL_MAIN_ID] + ") as jumlah from " + PstBillMain.TBL_CASH_BILL_MAIN + " where ((" + PstBillMain.fieldNames[PstBillMain.FLD_DOC_TYPE] + " <> 0) or  (" + PstBillMain.fieldNames[PstBillMain.FLD_DOC_TYPE] + "=0 and " + PstBillMain.fieldNames[PstBillMain.FLD_TRANSCATION_TYPE] + "=0 and " + PstBillMain.fieldNames[PstBillMain.FLD_TRANSACTION_STATUS] + "= 1 )) and " + PstBillMain.fieldNames[PstBillMain.FLD_CASH_CASHIER_ID] + " = " + cashCashier.getOID() ;
                    rsCount = statement.executeQuery(sql);
                    while(rsCount.next()){
...
public void transferBillMainTransCredit(long oidCashCashier){
    ...
    public void transferCashPayment(long oidBillMain){
		...
		CashPayments cashPayment = new CashPayments();
                PstCashPayment.resultToObject(rs, cashPayment);
		vecCashPayment.add(cashPayment);
            ...
            while(isRunThread() && vecCashPayment!=null && vecCashPayment.size() > 0  ){
                ...
                for(int idx=0;idx<vecCashPayment.size();idx++){
                    try{
                    CashPayments cashpayment = (CashPayments) vecCashPayment.get(idx);
                        try{
                            PstCashPayment.insertExcByOid(cashpayment);
                        }catch(Exception exc){
                            System.out.println(exc);
                        }
    ...
    public void transferCreditPayment(long oidCreditPaymentMain){
		...
		CashCreditPayments cashCreditPayments = new CashCreditPayments();
                PstCashCreditPayment.resultToObject(rs, cashCreditPayments);
		vecCreditPayment.add(cashCreditPayments);
            ...
            while(isRunThread() && vecCreditPayment!=null && vecCreditPayment.size() > 0  ){
                ...
                for(int idx=0;idx<vecCreditPayment.size();idx++){
                    try{
                    CashCreditPayments cashCreditPayment = (CashCreditPayments) vecCreditPayment.get(idx);
                    PstCashCreditPayment.insertExcByOid(cashCreditPayment);
