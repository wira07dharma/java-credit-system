 ...
 * Exchange files object between online and backoffine system
 */
 ...
 * @version
 */
public class ObjSynchFTPAPI implements Serializable {
    ...
    public ObjSynchFTPAPI(ObjSynchNode objSynch ) {
        ...
        RemotePathDelim=this.objSynch.RemotePathDelim;
        
        FTPMode      = this.objSynch.FTPMode;//args[5];
        FTPConnMode  = this.objSynch.FTPConnMode;//args[6];
    }
    
    public void putAllObject(boolean deleteLocalFile){
        try{
            ...
            if( (fileslist == null) || (fileslist.size()<1)){
                putAllObjectTargetInStatus(deleteLocalFile); // if there are a rest of status
                return;
            }
            
            String remoteDir = RemoteInDirURL+RemotePathDelim+this.objSynch.getLocalNodeName()+
            ...
            for(int i = 0; i < fileslist.size(); i++) {
                try{
                    ...
                    if( (putFile(file.getAbsolutePath(), file.getName())==DSJ_ClassMsg.OK) &&
                    deleteLocalFile){
                        file.delete();
                    }
                } catch(Exception exc){
            ...
            putAllObjectTargetInStatus(deleteLocalFile);
    ...
    public void putAllObjectTargetInStatus(boolean deleteLocalFile){
        try{
            ...
            if(oNd!=null){
               ...
               // + LOCAL_PATH_DELIM + this.objSynch.LocalObjectSubFolder ;
                ...
                for(int idx=0; idx< oNd.size();idx++){
                    ...
                    String remoteDir = RemoteInDirURL+RemotePathDelim+this.objSynch.getLocalNodeName()+
                    RemotePathDelim + RemoteStatusSubFolder + RemotePathDelim + oNd.get(idx) ;
     ...
     */
    public void getAllObject(boolean deleteRemoteDataFile, boolean deleteRemoteStatusFile){
        try{
            Vector nodes = this.objSynch.getOtherNodesNames();
            
            if(nodes!=null){
                System.out.println(">>>>> getAllObject : nodes.size ="+nodes.size());
                for(int idx=0;idx<nodes.size();idx++){
                    System.out.println("                 idx ="+idx+"  "+objSynch.getOtherNodeName(idx));
                    String localDir  = this.LocalInDirURL + LOCAL_PATH_DELIM +
