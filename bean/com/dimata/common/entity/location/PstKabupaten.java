...
WHERE prov.NAMA_PROP='Seoul';
	...
	public static ArrayList getListFromApi(int limitStart, int recordToGet, String whereClause, String order){
		...
		JSONObject jo = WebServices.getAPIWithParam("", posApiUrl + "/master/kabupaten-list?", param);
		...
		try {
			...
			for(int i = 0; i < lists.length(); i++){
				...
				convertJsonToObject(tempObj, kab);
				listData.add(kab);
			}
		} catch (Exception e) {
			System.out.println(e);
	...
	public static Kabupaten fetchFromApi(long oid) {
		String whereClause = fieldNames[FLD_ID_KABUPATEN] + "=" + oid;
		ArrayList<Kabupaten> listData = getListFromApi(0, 0, whereClause, "");
		Kabupaten kab = new Kabupaten();
		if(!listData.isEmpty()){
			kab = listData.get(0);
			try {
				if(checkOID(kab.getOID())){
					updateExc(kab);
				} else {
					syncExec(kab);
				}
	...
	public static long syncExec(Kabupaten kab)throws DBException{
		try {
			PstKabupaten pstKab = new PstKabupaten(0);
			pstKab.setString(FLD_KD_KABUPATEN, kab.getKdKabupaten());
			pstKab.setString(FLD_NM_KABUPATEN, kab.getNmKabupaten());
			pstKab.setLong(FLD_ID_PROPINSI, kab.getIdPropinsi());
			pstKab.insertByOid(kab.getOID());
		} catch (DBException dbe) {
            throw dbe;
        } catch (Exception e) {
            throw new DBException(new PstKabupaten(0), DBException.UNKNOWN);
	...
	public static void convertJsonToObject(JSONObject obj, Kabupaten kabupaten) {
		try {
			kabupaten.setOID(obj.optLong(PstKabupaten.fieldNames[PstKabupaten.FLD_ID_KABUPATEN], 0));
			kabupaten.setKdKabupaten(obj.optString(PstKabupaten.fieldNames[PstKabupaten.FLD_KD_KABUPATEN], ""));
			kabupaten.setNmKabupaten(obj.optString(PstKabupaten.fieldNames[PstKabupaten.FLD_NM_KABUPATEN], ""));
			kabupaten.setIdPropinsi(obj.optLong(PstKabupaten.fieldNames[PstKabupaten.FLD_ID_PROPINSI], 0));
		} catch (Exception e) {
		}
	}
}
