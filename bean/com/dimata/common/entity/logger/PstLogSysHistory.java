...
public class PstLogSysHistory extends DBHandler
  ...
  public long fetchExc(Entity ent) {
    ...
    try {
      logSysHistory = fetchExc(ent.getOID());
      ent = (Entity) logSysHistory;
    } catch (DBException ex) {
      Logger.getLogger(PstLogSysHistory.class.getName()).log(Level.SEVERE, null, ex);
    }
  ...
  public static long insertUpdateLog(LogSysHistory logHis) {
    try {
      PstLogSysHistory pstLogSysHistory = new PstLogSysHistory(0);
  ...
  public static long insertLog(LogSysHistory logHis) {
    try {
      PstLogSysHistory pstLogSysHistory = new PstLogSysHistory(0);
      pstLogSysHistory.insertExc(logHis);
    } catch (DBException dbe) {
  ...
  /**
   * Keterangan: untuk mengambil data dari database berdasarkan oid jenisItemsId
   * dan kemudaian di set objecknya
   *
   * @param oid : negaraId
   * @return
   * @throws DBException
  ...
  /**
   * KETERANGAN: Fungsi untuk melakukan list table logSysHistory , berdasarkan
   * parameter di bawah
   *
   * @param limitStart
   * @param recordToGet
  ...
  public static Vector listPurchaseOrder(int limitStart, int recordToGet, String whereClause, String order) {
    ...
    try {
      String sql = "SELECT " + fieldNames[PstLogSysHistory.FLD_LOG_LOGIN_NAME] + ", " + fieldNames[PstLogSysHistory.FLD_LOG_USER_ACTION]
              + ", " + fieldNames[PstLogSysHistory.FLD_LOG_UPDATE_DATE] + ", " + fieldNames[PstLogSysHistory.FLD_LOG_APPLICATION]
              + ", " + fieldNames[PstLogSysHistory.FLD_LOG_DETAIL] + " FROM " + TBL_LOG_HISTORY;
  ...
  /**
   * INSERT DATA KETIKA SUDAH 2 JAM, PROSES : 1. TAMPILKAN WAKTU UPDATE TERAKHIR
   * BERDASARKAN ID DOKUMEN TERTENTU 2. SELISIHKAN WAKTU UPDATE TERAKHIR DENGAN
   * WAKTU SEKARANG 3. JIKA LEBIH BESAR DARI 2 JAM,,MAKA INSERTKAN DATA
   */
  ...
  public static String getHistoryLogQuery(String params, String app) {

    org.json.JSONArray docs = null;
    String time = "";

    try {
      org.json.JSONObject o = (params != null) ? new org.json.JSONObject(params) : new org.json.JSONObject();
      time = (o.getString("time")!=null)?o.getString("time"):"";
      docs = o.getJSONArray("doc");
    ...
    boolean validTimes = (times != null && times.length == 2);
    String start = validTimes ? times[0] : "";
    String end = validTimes ? times[1] : "";

    String sql = "SELECT * FROM " + PstLogSysHistory.TBL_LOG_HISTORY + " "
            + "WHERE " + PstLogSysHistory.fieldNames[PstLogSysHistory.FLD_LOG_APPLICATION] + " = '" + app + "' " + ((!start.equals("") && !end.equals(""))
            ? "AND " + PstLogSysHistory.fieldNames[PstLogSysHistory.FLD_LOG_UPDATE_DATE] + " BETWEEN '" + start + "' AND '" + end + "' " : "") + (!sDocs.equals("")
            ? "AND " + PstLogSysHistory.fieldNames[PstLogSysHistory.FLD_LOG_DOCUMENT_TYPE] + " IN (" + sDocs + ") " : " ");
    sql += " ORDER BY " + PstLogSysHistory.fieldNames[PstLogSysHistory.FLD_LOG_UPDATE_DATE] + " DESC ";
