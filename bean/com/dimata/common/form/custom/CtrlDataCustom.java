				...
				if(dataCustom.getOID()==0){
					try{
						long oid = pstDataCustom.insertExc(this.dataCustom);
                                                if(oid != 0){
                                                    DSJ_ObjSynch.addObjToSynch(oid, dataCustom.getClass().getName(), Command.ADD);
                                                }
					...
					try {
						long oid = pstDataCustom.updateExc(this.dataCustom);
                                                if(oid != 0){
                                                    DSJ_ObjSynch.addObjToSynch(oid, dataCustom.getClass().getName(), Command.UPDATE);
                                                }
					}catch (DBException dbexc){
						excCode = dbexc.getErrorCode();
						msgString = getSystemMessage(excCode);
			...
			case Command.DELETE :
				if (oidDataCustom != 0){
					try{
						...
						if(oid!=0){
                                                        DSJ_ObjSynch.addObjToSynch(oid, dataCustom.getClass().getName(), Command.DELETE);
