...
import com.dimata.common.session.contact.SessContactClass;
...
public class CtrlContactClass extends Control implements I_Language 
{
	...
	public static String[][] resultText = {
		{"Berhasil", "Tidak dapat diproses", "Kelompok kontak sudah ada", "Data tidak lengkap"},
		{"Succes", "Can not process", "Contact class is exist", "Data incomplete"}
	};
		...
		switch(cmd){
			...
			case Command.SAVE :
				...
				frmContactClass.requestEntityObject(contactClass);
                                ...
                                boolean checkCntClass = SessContactClass.checkContactClass(contactClass.getClassType()); 
			...
			case Command.DELETE :
				if (oidContactClass != 0){
					try{
						...
						if(oid!=0){
                                                        int iUpdate = SessContactClass.updateSeries();
