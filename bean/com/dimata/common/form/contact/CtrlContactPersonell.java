...
import com.dimata.services.objsynchxnodes.DSJ_ObjSynch;
...
public class CtrlContactPersonell extends Control implements I_Language {
    ...
    //for prochain manufacture
    public void action(int cmd, long contactPslOID, int start, int vectSize, int recordToGet, Vector vectClassAssign) {
        msgString = "";
        long rsCode = 0;
        switch(cmd){
            case Command.ADD :
                    /*contactList.setCode("");
                    contactList.setPersonName("");
                    contactList.setPersonLastName("");
                    contactList.setTelpMobile("");
                    contactList.setHomeAddress("");
                    contactList.setHomeTown("");
                    contactList.setHomeProvince("");
                    contactList.setHomeCountry("");
                    contactList.setHomeTelp("");
                    contactList.setHomeFax("");
                     
                    contactList.setDirections("");
                    contactList.setBankAcc1("");
                    contactList.setBankAcc2("");
                    contactList.setNotes("");
                contactList.setEmail("");
                                //contactList.setDate(this.getDate(FRM_REG_DATE));   ==> under construction*/
            ...
            case Command.SAVE :
                ...
                frmContactPsl.requestEntityObject(contactList);
                //contactList.setOID(contactPslOID);
                
                boolean bool = PstContactList.cekCodeContact(contactList.getContactCode(),contactPslOID);
                if(bool)
                    frmContactPsl.addError(frmContactPsl.FRM_CODE,"Code is already defined for another company or person.");
                ...
                if (contactList.getOID() == 0)
                    try{
                        rsCode = PstContactList.insertExc(this.contactList);
                        if(rsCode != 0){
                            DSJ_ObjSynch.addObjToSynch(contactList.getOID(), contactList.getClass().getName(), Command.ADD);
                        }
                    }catch(Exception e){}
                else
                    try{
                        rsCode = PstContactList.updateExc(this.contactList);
                        if(rsCode != 0){
                            DSJ_ObjSynch.addObjToSynch(contactList.getOID(), contactList.getClass().getName(), Command.UPDATE);
                        }
                    }catch(Exception e){}
                if(rsCode == FRMMessage.NONE)       {  // error condition
                    msgString = FRMMessage.getErr(FRMMessage.ERR_SAVED);
                ...
                System.out.println("vectClassAssign "+vectClassAssign.size());                
                if((vectClassAssign!=null)&&(vectClassAssign.size()>0)){
                    for(int i=0;i<vectClassAssign.size();i++){
                        ContactClassAssign cntClsAssign = (ContactClassAssign)vectClassAssign.get(i);
                        cntClsAssign.setContactId(contactList.getOID());
            ...
            case Command.DELETE :
                ...
                if (contactPslOID > 0){
                    ...
                    if(rsCode == FRMMessage.NONE) {
                        ...
                        DSJ_ObjSynch.addObjToSynch(contactPslOID, contactList.getClass().getName(), Command.DELETE);
                        msgString = FRMMessage.getMsg(FRMMessage.MSG_DELETED);
                        iErrCode = FRMMessage.ERR_NONE;
                    }
                }
