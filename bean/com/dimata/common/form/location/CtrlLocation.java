...
public class CtrlLocation extends Control implements I_Language 
{
	...
	public FrmLocation getForm() { return frmLocation; }
		...
		switch(cmd){
					...
					try{
						long oid = pstLocation.insertExc(this.location);
                                                if(oid != 0){
                                                    ...
                                                    System.out.println("continue to insert outlet on hanoman");
                                                    ...
                                                    System.out.println("outlet on hanoman inserted successfull ... ");
                                                    
                                                    DSJ_ObjSynch.addObjToSynch(oid, location.getClass().getName(), Command.ADD);
                                                }
					...
					try {
						long oid = pstLocation.updateExc(this.location);
                                                if(oid != 0){
                                                    ...
                                                    DSJ_ObjSynch.addObjToSynch(oid, location.getClass().getName(), Command.UPDATE);
                                                }
					}catch (DBException dbexc){
						excCode = dbexc.getErrorCode();
						msgString = getSystemMessage(excCode);
			...
			case Command.DELETE :
					...
					try{
						...
						long oid = PstLocation.deleteExc(oidLocation);
						if(oid!=0){
                                                        DSJ_ObjSynch.addObjToSynch(oid, location.getClass().getName(), Command.DELETE);
