...
public String getContactName(ContactList objCntList, String compName){
...
public String drawList(int language, Vector objectClass, long oid, int start){
	...
	for (int i = 0; i < objectClass.size(); i++) {
		...
		String strContact = getContactName(contactList, contactList.getCompName());
		lstData.add(rowx);
		lstLinkData.add(String.valueOf(contactList.getOID())+"','"+strContact+
						"','"+cekNull(contactList.getTelpNr())+"','"+cekNull(contactList.getFax()));
	}
		...
		if(objectClass!=null && objectClass.size()>0){
			...
			for(int i=0; i<objectClass.size(); i++){
				...
				result ="<script language=\"JavaScript\"> cmdEdit('"+(contactList.getOID()+"','"+
							  compName+"','"+cekNull(contactList.getTelpNr())+"','"+cekNull(contactList.getFax())+"');</script>");
			}
		}else{
...
int invType = FRMQueryString.requestInt(request,"inv_type");
...
String strCntType = "";
try{
	strCntType = (String)session.getValue("CNT_TYPE");
}catch(Exception e){}

if(strCntType != null){
	cntType = Integer.parseInt(strCntType);
}

System.out.println("cntType list : "+cntType);
...
function cmdEdit(oid, name, telp, fax){
	<%switch(cntType){
		case 0:
	%>
    
	self.opener.document.forms.<%=FrmInvoiceMain.FRM_INVOICE_MAIN%>.<%=FrmInvoiceMain.fieldNames[FrmInvoiceMain.FRM_FIRST_CONTACT_ID]%>.value = oid;
	<%//if(invType == 0){%>
		//self.opener.document.forms.<%=FrmInvoiceMain.FRM_INVOICE_MAIN%>.<%=FrmInvoiceMain.fieldNames[FrmInvoiceMain.FRM_FIRST_CONTACT_ID]%>_TXT.value = name;
    <%//}else{%>
		self.opener.document.forms.<%=FrmInvoiceMain.FRM_INVOICE_MAIN%>.<%=FrmInvoiceMain.fieldNames[FrmInvoiceMain.FRM_FIRST_CONTACT_ID]%>_TEXT.value = name;
		self.opener.document.forms.<%=FrmInvoiceMain.FRM_INVOICE_MAIN%>.telpon.value = telp;
		self.opener.document.forms.<%=FrmInvoiceMain.FRM_INVOICE_MAIN%>.fax.value = fax;
	<%//}%>
	<% break;
	   case 1:	
	%>
	self.opener.document.forms.<%=FrmInvoiceMain.FRM_INVOICE_MAIN%>.<%=FrmInvoiceMain.fieldNames[FrmInvoiceMain.FRM_SECOND_CONTACT_ID]%>.value = oid;
    self.opener.document.forms.<%=FrmInvoiceMain.FRM_INVOICE_MAIN%>.<%=FrmInvoiceMain.fieldNames[FrmInvoiceMain.FRM_SECOND_CONTACT_ID]%>_TEXT.value = name;
	<% break;
	   case 2:	
	%>
	self.opener.document.forms.<%=FrmCosting.FRM_COSTING%>.<%=FrmCosting.fieldNames[FrmCosting.FRM_CONTACT_ID]%>.value = oid;
    self.opener.document.forms.<%=FrmCosting.FRM_COSTING%>.<%=FrmCosting.fieldNames[FrmCosting.FRM_CONTACT_ID]%>_TEXT.value = name;
	<% break;
	   case 3:	
	%>
	self.opener.document.forms.<%=FrmSrcInvoice.FRM_INVOICE%>.<%=FrmSrcInvoice.fieldNames[FrmSrcInvoice.FRM_CONTACT_ID]%>.value = oid;
    self.opener.document.forms.<%=FrmSrcInvoice.FRM_INVOICE%>.<%=FrmSrcInvoice.fieldNames[FrmSrcInvoice.FRM_CONTACT_ID]%>_TEXT.value = name;
	<%
		break;
	}
	%>
              ...
              <table width="100%" border="0" cellspacing="0" cellpadding="0">
                ...
                <tr> 
                  <td>
					...
					<%}else{%>
				  		<%out.println(drawList(SESS_LANGUAGE,records,oidContactList,start));%>
				  	<%}%>
				  </td>
                </tr>			  
                <%}else{%>
