...
String strCmdPosted = SESS_LANGUAGE==com.dimata.util.lang.I_Language.LANGUAGE_FOREIGN ? "Posted Data" : "Posting Data"; 
...
try{	
	vListInvoice = sessInvoice.listInvoice(start,recordToGet,srcInvoice,strOrderBy,I_DocStatus.DOCUMENT_STATUS_TO_BE_APPROVED);
}catch(Exception e){}  
...
if(iCommand == Command.POST){
	...
	if(vInvoiceId != null && vInvoiceId.size() > 0){
		for(int p = 0; p < vInvoiceId.size(); p++){
			...
			if(objInvoiceMain != null){
				try{
					SessInvoice.postedData(bookType, userOID, periodeOID, objInvoiceMain, lCurrType, lDepartmentId);
				}catch(Exception e){}
			}
		}
	}
...
function cmdPosted(){
	...
	document.frmsrcorderaktiva.action="post_invoice_list.jsp";
...
function cmdBack(){
	...
	document.frmsrcorderaktiva.action="post_invoice_search.jsp";
	document.frmsrcorderaktiva.submit();
}
</script>
<!-- #EndEditable -->
