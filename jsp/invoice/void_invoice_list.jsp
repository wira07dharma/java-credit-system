...
String strCmdPosted = SESS_LANGUAGE==com.dimata.util.lang.I_Language.LANGUAGE_FOREIGN ? "Voiding Invoice" : "Batalkan Invoice";
...
try{
	vectSize = sessInvoice.totalInvoice(srcInvoice,I_DocStatus.DOCUMENT_STATUS_FINAL);
}catch(Exception e){}
...
try{	
	vListInvoice = sessInvoice.listInvoice(start,recordToGet,srcInvoice,strOrderBy,I_DocStatus.DOCUMENT_STATUS_FINAL);
}catch(Exception e){}  
...
if(iCommand == Command.POST){
	...
	if(vInvoiceId != null && vInvoiceId.size() > 0){
		for(int p = 0; p < vInvoiceId.size(); p++){
			...
			if(objInvoiceMain != null){
				try{
					SessInvoice.voidingInvoice(lInvoiceMainId);
				}catch(Exception e){}
			}
		}
	}
...
function cmdVoiding(){
	...
	document.frmsrcorderaktiva.action="void_invoice_list.jsp";
...
function cmdBack(){
	...
	document.frmsrcorderaktiva.action="void_invoice_search.jsp";
	document.frmsrcorderaktiva.submit();
}
</script>
<!-- #EndEditable -->
            ...
            <form name="frmsrcorderaktiva" method="post" action="">
              ...
              <table width="100%" border="0" cellspacing="2" cellpadding="0">
					<tr>
					  ...
					  <%if(iStatus > 0){%>
					  		<a href="javascript:cmdVoiding()"><%=strCmdPosted%></a> | <a href="javascript:cmdBack()"><%=strBack%></a>
					  <%}else{%>
