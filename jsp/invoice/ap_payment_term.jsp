...
public Vector drawListCosting(int language, Vector objectClass, int iCommand){
	ControlList ctrlist = new ControlList();
	ctrlist.setAreaWidth("100%");
	ctrlist.setListStyle("listgen");
	ctrlist.setTitleStyle("listgentitle");
...
public String listApPayTerm(int language, Vector objectClass, int iCommand, long lApPaymentTermId, Costing objCosting, 
		InvoiceMain objInvoiceMain, double dApNetAmount, FrmPaymentTerm frmPaymentTerm){
	ControlList ctrlist = new ControlList();
	ctrlist.setAreaWidth("60%");
	ctrlist.setListStyle("listgen");
	ctrlist.setTitleStyle("listgentitle");
	...
	double totAmount = 0.0;
		for (int i = 0; i < objectClass.size(); i++) {
			...
			rowx = new Vector();
			if(lApPaymentTermId == objApPaymentTerm.getOID())
				index = i;
			
			if(index==i && (iCommand==Command.EDIT || iCommand==Command.ASK)){	
				rowx.add(""+(i+1));
				rowx.add("<input type=\"text\" onClick=\"ds_sh(this);\" name=\""+FrmPaymentTerm.fieldNames[FrmPaymentTerm.FRM_PLAN_DATE]+"\" readonly=\"readonly\" style=\"cursor: text\" value=\""+Formater.formatDate((objApPaymentTerm.getPlanDate() == null) ? new Date() : objApPaymentTerm.getPlanDate(), "dd-MM-yyyy")+"\" class=\"formElemenR\">"+
				...
				rowx.add("<input type=\"text\" name=\""+FrmPaymentTerm.fieldNames[FrmPaymentTerm.FRM_AMOUNT]+"\" value=\""+Formater.formatNumber(objApPaymentTerm.getAmount(),"###")+"\" class=\"formElemenR\">");
				rowx.add("<input type=\"text\" name=\""+FrmPaymentTerm.fieldNames[FrmPaymentTerm.FRM_NOTE]+"\" value=\""+objApPaymentTerm.getNote()+"\" class=\"formElemenR\">");			
			}else{
				rowx.add(""+(i+1));
		...
		if(iCommand==Command.ADD || (iCommand==Command.SAVE && frmPaymentTerm.errorSize()>0)){
			...
			if(objectClass.size() == 0){
				...
				rowx.add("<input type=\"text\" onClick=\"ds_sh(this);\" name=\""+FrmPaymentTerm.fieldNames[FrmPaymentTerm.FRM_PLAN_DATE]+"\" readonly=\"readonly\" style=\"cursor: text\" value=\""+Formater.formatDate((objApPaymentTerm.getPlanDate() == null) ? new Date() : dDueDate, "dd-MM-yyyy")+"\" class=\"formElemenR\">"+
					"<input type=\"hidden\" name=\""+FrmPaymentTerm.fieldNames[FrmPaymentTerm.FRM_PLAN_DATE]+"_mn\">"+
					"<input type=\"hidden\" name=\""+FrmPaymentTerm.fieldNames[FrmPaymentTerm.FRM_PLAN_DATE]+"_dy\">"+
					"<input type=\"hidden\" name=\""+FrmPaymentTerm.fieldNames[FrmPaymentTerm.FRM_PLAN_DATE]+"_yr\">"+
					"<script language=\"JavaScript\" type=\"text/JavaScript\">getThn();</script>");
				rowx.add("<input type=\"text\" name=\""+FrmPaymentTerm.fieldNames[FrmPaymentTerm.FRM_AMOUNT]+"\" value=\""+Formater.formatNumber(dApNetAmount,"###")+"\" class=\"formElemenR\">");
				...
				objApPaymentTerm.setPlanDate(new Date());
				rowx.add("<input type=\"text\" onClick=\"ds_sh(this);\" name=\""+FrmPaymentTerm.fieldNames[FrmPaymentTerm.FRM_PLAN_DATE]+"\" readonly=\"readonly\" style=\"cursor: text\" value=\""+Formater.formatDate((objApPaymentTerm.getPlanDate() == null) ? new Date() : objApPaymentTerm.getPlanDate(), "dd-MM-yyyy")+"\" class=\"formElemenR\">"+
					"<input type=\"hidden\" name=\""+FrmPaymentTerm.fieldNames[FrmPaymentTerm.FRM_PLAN_DATE]+"_mn\">"+
					"<input type=\"hidden\" name=\""+FrmPaymentTerm.fieldNames[FrmPaymentTerm.FRM_PLAN_DATE]+"_dy\">"+
					"<input type=\"hidden\" name=\""+FrmPaymentTerm.fieldNames[FrmPaymentTerm.FRM_PLAN_DATE]+"_yr\">"+
...
System.out.println("iInvoiceType on ap_payment_term ::::: "+iInvoiceType);
...
CtrlPaymentTerm ctrlApPaymentTerm = new CtrlPaymentTerm(request);
ctrlApPaymentTerm.setLanguage(SESS_LANGUAGE);
FrmPaymentTerm frmApPaymentTerm = ctrlApPaymentTerm.getForm();
int iErrCode = ctrlApPaymentTerm.action(iCommand, oidApPaymentTerm);
PaymentTerm objApPaymentTerm = ctrlApPaymentTerm.getPaymentTerm();
oidApPaymentTerm = objApPaymentTerm.getOID();
String msgString = ctrlApPaymentTerm.getMassage();
...
int iErrCodeCosting = ctrlCosting.action(Command.EDIT, oidCosting);
Costing objCosting = ctrlCosting.getCosting();
String msgStringCosting = ctrlCosting.getMassage();
...
Vector vTempListCosting = new Vector(1,1);
String strListCosting = "";
double dNetAmountAp = 0.0;
try{
	vTempListCosting = drawListCosting(SESS_LANGUAGE,listCosting,iCommand);
	strListCosting = vTempListCosting.get(0).toString();
	dNetAmountAp = Double.parseDouble(vTempListCosting.get(1).toString());
}catch(Exception e){}
...
if(objCosting.getIdPerkiraanHutang() != 0){
	try{
		objPerk = PstPerkiraan.fetchExc(objCosting.getIdPerkiraanHutang());
		if(objPerk != null){
			strNamaPerkiraan = getAccName(objPerk, SESS_LANGUAGE);
		}
	}catch(Exception e){objPerk = new Perkiraan();}
}
...
function cmdBackMain(){
    document.<%=FrmPaymentTerm.FRM_PAYMENT_TERM%>.payment_ap_term_id.value = 0;
...
function cmdBack(){
	<% switch(iQueryType){
		...
		case 1:%>
			document.<%=FrmPaymentTerm.FRM_PAYMENT_TERM%>.query_type.value="<%=iQueryType%>";
			document.<%=FrmPaymentTerm.FRM_PAYMENT_TERM%>.start.value="<%=start%>";
			<%if(iCommand == Command.ADD){%>
			document.<%=FrmPaymentTerm.FRM_PAYMENT_TERM%>.command.value="<%=Command.LIST%>";
			document.<%=FrmPaymentTerm.FRM_PAYMENT_TERM%>.action="ap_payment_term.jsp";
			<%}else{%>
			document.<%=FrmPaymentTerm.FRM_PAYMENT_TERM%>.command.value="<%=Command.LIST%>";
			document.<%=FrmPaymentTerm.FRM_PAYMENT_TERM%>.invoice_type.value="1";
		...
		case 2:%>
			...
			document.<%=FrmPaymentTerm.FRM_PAYMENT_TERM%>.command.value="<%=Command.LIST%>";
			document.<%=FrmPaymentTerm.FRM_PAYMENT_TERM%>.start.value="<%=start%>";
			document.<%=FrmPaymentTerm.FRM_PAYMENT_TERM%>.action="post_invoice_list.jsp";
			document.<%=FrmPaymentTerm.FRM_PAYMENT_TERM%>.submit();
		<%break;
		case 3:%>
			document.<%=FrmPaymentTerm.FRM_PAYMENT_TERM%>.query_type.value="<%=iQueryType%>";
		...
		case 4:%>
			...
			<%}else{%>
			document.<%=FrmPaymentTerm.FRM_PAYMENT_TERM%>.command.value="<%=Command.LIST%>";
			document.<%=FrmPaymentTerm.FRM_PAYMENT_TERM%>.invoice_type.value="2";
			document.<%=FrmPaymentTerm.FRM_PAYMENT_TERM%>.action="invoice_list.jsp";
			<%}%>
			document.<%=FrmPaymentTerm.FRM_PAYMENT_TERM%>.submit();
		<%break;
...
function cmdPosted(oid){
	document.<%=FrmPaymentTerm.FRM_PAYMENT_TERM%>.invoice_main_id.value=oid;
	document.<%=FrmPaymentTerm.FRM_PAYMENT_TERM%>.command.value="<%=Command.POST%>";
	document.<%=FrmPaymentTerm.FRM_PAYMENT_TERM%>.action="invoice_edit.jsp";
        ...
        <tr> 
          <td valign="top"><!-- #BeginEditable "content" -->
            <form name="<%=FrmPaymentTerm.FRM_PAYMENT_TERM%>" method="post" action="">
			  ...
			  <input type="hidden" name="<%=FrmPaymentTerm.fieldNames[FrmPaymentTerm.FRM_TYPE]%>" value="<%=PstPaymentTerm.PAY_TERM_AP%>">
                      ...
                      <tr>
                        <td width="100%" class="tabtitleactive" valign="top">
                          <table width="100%" border="0" cellpadding="0" cellspacing="0" height="25">
                              ...
                              <td width="94%">
                                <table width="100%">
								  ...
								  <tr>
								   ...
								   <tr>
                                    <td colspan="9"><%=strListCosting%></td>
                                  </tr>
								  <tr>
                                    <td colspan="9">&nbsp;
                                    </td>
								  ...
								  <tr>
                                    <td colspan="9">
										<%=listApPayTerm(SESS_LANGUAGE,listApPayTerm,iCommand, oidApPaymentTerm, objCosting, objInvoiceMain, dNetAmountAp,  frmApPaymentTerm)%>
									</td>
                                  </tr>
                                  <tr>
                                    <td colspan="9">&nbsp;</td>
							...
							String scomDel = "javascript:cmdAsk('"+oidApPaymentTerm+"')";
							String sconDelCom = "javascript:cmdDelete('"+oidApPaymentTerm+"')";
							String scancel = "javascript:cmdEdit('"+oidApPaymentTerm+"')";
							...
							%></td>
							<%if(oidInvoiceMain != 0 && iQueryType > 0 && allList){%>
                             	<%if(objInvoiceMain.getStatus() == I_DocStatus.DOCUMENT_STATUS_FINAL && iQueryType == 2){%>
									<td width="24%" nowrap><a href="javascript:cmdPosted('<%=oidInvoiceMain%>')" class="command"><%=strTitle[SESS_LANGUAGE][26]%></a></td>
							 	<%}else{%>
									<td width="24%" nowrap>
										<a href="javascript:cmdApproved('<%=oidInvoiceMain%>')" class="command"><%=strApproved%></a>
