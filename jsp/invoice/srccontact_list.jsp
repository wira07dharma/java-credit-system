...
int cntType = FRMQueryString.requestInt(request,"cnt_type");
...
if(iCommand == Command.NONE){
	if(session.getValue("CNT_TYPE") != null){
		session.removeValue("CNT_TYPE");
	}
	
	try{
		session.putValue("CNT_TYPE",""+cntType);
	}catch(Exception e){}
}
...
function cmdSearch(){
		document.frm_contactlist.inv_type.value="<%=invType%>";
        ...
        <tr> 
          <td valign="top"><!-- #BeginEditable "content" --> 
            <form name="frm_contactlist" method="post" action="">
			  ...
			  <input type="hidden" name="cnt_type" value="<%=cntType%>">
              ...
              <table width="100%" border="0" cellspacing="0" cellpadding="0">
				...
				<td>
					<table width="100%">
							  <%
							  Vector vectCntClass = PstContactClass.listAll();
							  int iCount = 0;						
							  int iMaxColumn = 4;
							  int iMaxRow = vectCntClass.size() / iMaxColumn;
							  if(vectCntClass.size() % iMaxColumn > 0)
								iMaxRow += 1;					   
								for(int r = 0; r < iMaxRow; r++){
								%>
								 <tr>						  
							  		<%for(int i=0; i<iMaxColumn; i++){
							  			ContactClass cntClass = (ContactClass)vectCntClass.get(iCount);
