<%-- 
    Document   : penutupan
    Created on : Aug 27, 2018, 10:03:00 AM
    Author     : Dimata 007
--%>
...
<%@page import="com.dimata.sedana.ajax.transaksi.AjaxPenutupan"%>
...
<%!
    public boolean cekTabunganDigunakanUntukKredit(long idAssignContactTabungan) {
        //cek apakah tabungan digunakan untuk transaksi kredit
        String whereCek = PstPinjaman.fieldNames[PstPinjaman.FLD_ASSIGN_TABUNGAN_ID] + " = " + idAssignContactTabungan
                + " AND " + PstPinjaman.fieldNames[PstPinjaman.FLD_STATUS_PINJAMAN]
                + " IN (" + Pinjaman.STATUS_DOC_DRAFT
                + "," + Pinjaman.STATUS_DOC_TO_BE_APPROVE
                + "," + Pinjaman.STATUS_DOC_APPROVED
                + "," + Pinjaman.STATUS_DOC_CAIR
                + "," + Pinjaman.STATUS_DOC_PENANGANAN_MACET
                + ")";
        
        Vector listPinjaman = PstPinjaman.list(0, 0, whereCek, "");
        if (!listPinjaman.isEmpty()) {
            return true;
        }

        //cek apakah data simpanan digunakan untuk pengendapan kredit
        String whereSimpanan = "SELECT " + PstDataTabungan.fieldNames[PstDataTabungan.FLD_ID_SIMPANAN]
                + " FROM " + PstDataTabungan.TBL_DATA_TABUNGAN
                + " WHERE " + PstDataTabungan.fieldNames[PstDataTabungan.FLD_ASSIGN_TABUNGAN_ID] + " = " + idAssignContactTabungan
                + "";
        
        whereCek = PstPinjaman.fieldNames[PstPinjaman.FLD_WAJIB_ID_JENIS_SIMPANAN] + " IN (" + whereSimpanan + ")"
                + " AND " + PstPinjaman.fieldNames[PstPinjaman.FLD_STATUS_PINJAMAN]
                + " IN (" + Pinjaman.STATUS_DOC_DRAFT
                + "," + Pinjaman.STATUS_DOC_TO_BE_APPROVE
                + "," + Pinjaman.STATUS_DOC_APPROVED
        ...
        listPinjaman = PstPinjaman.list(0, 0, whereCek, "");
        if (!listPinjaman.isEmpty()) {
            return true;
        }
        return false;
...
<%    
    ...
    Vector<JenisSimpanan> js = (Vector<JenisSimpanan>) request.getAttribute("js"); //sumber : com.dimata.sedana.ajax.transaksi (AjaxPenutupan.java)
    ...
    if (js != null) {
        for (JenisSimpanan j : js) {
            totalPenarikan += PstDetailTransaksi.getSimpananSaldo(tabungan.getOID(), tabungan.getAssignContactTabunganId(), j.getOID());
        }
    }
%>
...
<html>
        ...
        <div class="main-page">
            ...
            <section class="content">
                <div class="row">
                    <div class="col-xs-12">
                        <!-- Horizontal Form -->
                        <div class="box box-default" style="border-top-color: grey">
                            <div class="box-header with-border">
                                <h3 class="box-title">Form Pencarian</h3>
                            </div>
                            <!-- /.box-header -->
                            <!-- form start -->
                            <form id="form_saldo" action="Penutupan" class="form-horizontal" method="post">
                                ...
                                <div class="box-body">
                                    ...
                                    <style>
                                        ...
                                        .n.table-form thead tr, .n.table-form thead tr th { background-color: grey !important; color: white !important; padding: 10px; }
                                    ...
                                    <%
                                        //cek apakah tabungan digunakan untuk transaksi kredit
                                        boolean used = cekTabunganDigunakanUntukKredit(tabungan.getAssignContactTabunganId());
                                        
                                        if (used) {
                                            kredit = 1;
                                            out.print("<div class='label-danger text-center'>Tidak dapat menutup tabungan yang masih digunakan untuk pengajuan kredit !</div>");
                                        }
                                    %>
                                        ...
                                        <table class="table table-bordered table-condensed table-hover table-form n">
                                            ...
                                            <tbody id="payment-body">
                                                <tr>
                                                    ...
                                                    <td style="text-align: right;">
                                                        <input <%=(dataFor == AjaxSetoran.VIEW_TABUNGAN) ? "readonly" : ""%> type="text" style="text-align: right; font-size: 15px;" value="<%= totalPenarikan %>" class="form-control money input-payment" required data-cast-class="val-money" />
                                                        <input name="<%=HTTPTabungan.FRM_FIELD_PAYMENT_NOMINAL%>" type="hidden" value="<%= totalPenarikan %>" class="form-control hidden val-money" />
                                                    </td>
                                                </tr>
                                            </tbody>
                                        </table>                    
                                        <table class="table table-bordered table-condensed table-hover table-form n">
                                            ...
                                            <tbody id="saldo-body">
                                                ...
                                                <%
                                                    ...
                                                    MasterTabunganPenarikan masterPenarikan = PstMasterTabunganPenarikan.fetchExc(pentab.getIdTabunganRangePenarikan());
                                                    ...
                                                    for (JenisTransaksiMapping tm : vMapping) {
                                                        m += (!m.equals("") ? "," : "");
                                                        m += tm.getJenisTransaksiId();
                                                    }
                                                    JenisTransaksi jt = PstJenisTransaksi.fetchWhere(" PROSEDURE_UNTUK = 0 AND TIPE_ARUS_KAS = 0 AND TYPE_TRANSAKSI = 1 AND STATUS_AKTIF = 1 AND TIPE_DOC = 8");
                                                    long idJenisTransaksi = jt.getOID();
                                                    ...
                                                    //cek apakah punya periode penarikan
                                                    AssignContactTabungan act = PstAssignContactTabungan.fetchExc(tabungan.getAssignContactTabunganId());
                                                    Vector<AssignPenarikanTabungan> listPeriode = PstAssignPenarikanTabungan.list(0, 0, PstAssignPenarikanTabungan.fieldNames[PstAssignPenarikanTabungan.FLD_MASTER_TABUNGAN_ID] + " = " + act.getMasterTabunganId(), null);
                                                    for (AssignPenarikanTabungan apt : listPeriode) {
                                                        //cek tipe periode bulanan
                                                        MasterTabunganPenarikan mtp = PstMasterTabunganPenarikan.fetchExc(apt.getIdTabunganRangePenarikan());
                                                        if (mtp.getTipeRange() == I_Sedana.TIPE_PENARIKAN_TABUNGAN_BULAN) {
                                                            Vector<DataTabungan> listSimpanan = PstDataTabungan.list(0, 0, PstDataTabungan.fieldNames[PstDataTabungan.FLD_ASSIGN_TABUNGAN_ID] + " = " + tabungan.getAssignContactTabunganId(), null);
                                                            for (DataTabungan dt : listSimpanan) {
                                                                Date tglTutup = dt.getTanggalTutup();
                                                                if (tglTutup != null && tglTutup.after(new Date())) {
                                                                    idJenisTransaksi = PstJenisTransaksi.getIdJenisTransaksiByNamaJenisTransaksi("Denda Tabungan Deposito");
                                                                    MasterTabungan mt = PstMasterTabungan.fetchExc(act.getMasterTabunganId());
                                                                    if (mt.getJenisDenda() == MasterTabungan.DENDA_PROSENTASE) {
                                                                        double saldo = PstDetailTransaksi.getSimpananSaldo(dt.getOID());
                                                                        denda = saldo * mt.getDenda() / 100;
                                                                    } else {
                                                                        denda = mt.getDenda();
                                                                    }
                                                                }
                                                ...
                                                <tr data-denda="<%=denda%>" data-id="<%=mastab.getOID()%>">
                                                    ...
                                                    <td style="background-color:#f9f9f9;">
                                                        <input type="text" class="hidden" name="<%=AjaxSetoran.FRM_FIELD_JENIS_TRANSAKSI_DENDA%>" value="<%= idJenisTransaksi %>" class="hidden" />
                                                    ...
                                                    <td style="background-color:#f9f9f9;">
                                                        ...
                                                        <span style="float:right;" class="dendanya money">0</span>
                                                    </td>
                                                    <td>
                                                    ...
                                                    <td style="position:relative;">
                                                        <input style="position:absolute;bottom:1px;text-align: right;font-size: 15px;" type="text" data-max="" value="<%= saldo %>" class="form-control money input-saldo" data-fsetor="<%=j.getFrekuensiSimpanan()%>" data-ftarik="<%=j.getFrekuensiPenarikan()%>" data-cast-class="val-money">
                                                        <input name="<%=AjaxSetoran.FRM_FIELD_SALDO%>" type="hidden" value="<%= saldo %>" class="form-control hidden val-money">
                                                    </td>
                                                </tr>
                                                <%
                                            ...
                                            <tfoot class="footer-c">
                                                <tr>
                                                    ...
                                                    <td class="t" style="text-align: right;">
                                                        ...
                                                        <strong class="money" id="saldo-total" style="text-align: right;"><%= totalPenarikan %></strong>
                                                    </td>
                                                </tr>
                                                <tr>
                                                    <td class="" colspan="4" style="text-align: right; vertical-align: middle"><strong>Alasan Penutupan Tabungan :</strong></td>
                                                    <td class="" colspan="2">
                                                        <input type="text" style="width: 100%" id="alasanTutup" autocomplete="off" placeholder="..." name="<%= AjaxSetoran.FRM_FIELD_ALASAN_TUTUP%>">
                                                    </td>
                                                </tr>
                                            </tfoot>
                                            <% } %>
                                ...
                                <div class="box-footer">
                                    ...
                                    <button type="submit" id="submit" class="btn btn-success pull-right">Buat Penutupan</button>
                                    <% } else { %>
                                    <button type="button" class="btn-print btn btn-success pull-left">Cetak</button>
                                    <% } %>
                                </div>
    ...
    <print-area>
        ...
        <strong style="width: 100%; display: inline-block; padding-top: 0px;">JENIS TRANSAKSI&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;:&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;PENUTUPAN</strong>
    ...
    <script>
        ...
        function getConfirmation() {
            ...
            dataSend += "&FRM_FIELD_DATA_FOR=getKonfirmasiPenutupan";
            //alert(JSON.stringify(dataSend));
            onDone = function (data) {
