...
if(objSpecialJournalMain.getIdPerkiraan() != 0){
	try{
		Perkiraan objPerkiraan = PstPerkiraan.fetchExc(objSpecialJournalMain.getIdPerkiraan());
		strPerkiraan = getAccountName(objPerkiraan, SESS_LANGUAGE);
	}catch(Exception e){
		System.out.println("Exception on fetchobjPerkiraan"+e.toString());
	}
} 
...
if(session.getValue("PAYMENT") != null){
	session.removeValue("PAYMENT");
}
	session.putValue("PAYMENT",vDataToPdf);			 
%>
<!-- End of JSP Block -->
...
function cmdopenpayable(){
	closeWindow();
	var url = "payablelist.jsp?src_from=1&command=<%=Command.LIST%>";
...
function cmdopen(){
	...
	myWindow = window.open(url,"src_cnt_payment_main","height=500,width=800,status=yes,toolbars=no,menubar=no,location=no,scrollbars=yes");
}
...
function changeTotal()
{
	...
	switch(iCurrType){
		...
		case "<%=temp.get(0)%>" :
			 ...
			 document.all.tot_saldo_akhir.innerHTML = "<%=Formater.formatNumber(Double.parseDouble((String)temp.get(1)), "##,###.##")%>";
			 break;
        <%
            }
       }
...
function hideObjectForDate(){
  ...
  function printJournal(){
		var linkPage = "<%=reportroot%>report.specialJournal.PaymentJournalPdf";
        ...
        <tr> 
          <td valign="top"><!-- #BeginEditable "content" -->
            <form name="<%=FrmSpecialJournalMain.FRM_SPECIAL_JOURNAL_MAIN%>" method="post" action="">
              ...
              <input type="hidden" name="<%=FrmSpecialJournalMain.fieldNames[FrmSpecialJournalMain.FRM_JOURNAL_TYPE]%>" value="<%=I_JournalType.TIPE_SPECIAL_JOURNAL_PAYMENT%>" size="35">
                                    ...
                                    <td height="25" colspan="3"><div align="center">
                                        ...
                                        <tr>
                                          ...
                                          <td><input type="hidden" readonly name="<%=frmSpecialJournalMain.fieldNames[FrmSpecialJournalMain.FRM_CONTACT_ID]%>" value="<%=objSpecialJournalMain.getContactId()%>" size="25">
                                            <% if(updateJournal.equalsIgnoreCase("Y") || (iCommand != Command.EDIT)){%>
                                            		...
                                            		<a href="javascript:cmdopen()"><img alt="Open List Employee" border="0" src="<%=approot%>/dtree/img/folderopen.gif"></a> *)</span> 
                                            		<%}else{%>
                                                    <%=contactName%>
                                                    <%}%></td>
										...
										%> 
                                          ...
                                          <td width="17%">
											...
											<td width="23%" nowrap>
												<% if(updateJournal.equalsIgnoreCase("Y") || (iCommand != Command.EDIT)){%>
												<a href="javascript:cmdopenpayable()"><img alt="Open List Payable" border="0" src="<%=approot%>/dtree/img/folderopen.gif"></a>
											  <font face="Verdana, Arial, Helvetica, sans-serif" color="#FF0033">*)
											  <%if(ctrlErr != ctrlSpecialJournalMain.RSLT_FORM_INCOMPLETE || ctrlErr == ctrlSpecialJournalMain.RSLT_ERR_MEMO){%>
												<%=msgString%></font>
												<%}%>
