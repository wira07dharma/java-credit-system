...
public static final String subMasterTitle[] = {
	"Jurnal Setoran Bank","Bank Deposite Journal"
};

public static final String strCekAmountTransaction[][] = {
	{"Nilai transaksi melebihi saldo","Nilai transaksi tidak boleh nol","Saldo = ","Nilai detail melebihi nilai transaksi"},
	{"Transaction Amount Over than Balance Amount","Transaction value has to be greater than zero","Balance Amount is ","Detail amount over than transaction amount"}
};
 ...
 public String listJurnalDetail(int iCommand, int language,
        ...
        for(int i=0; i<objectClass.size(); i++){
			...
			if(vListAccount != null && vListAccount.size() > 0){
                ...
                rowx.add(ControlCombo.draw(FrmSpecialJournalDetail.fieldNames[FrmSpecialJournalDetail.FRM_ID_PERKIRAAN],"", null, sSelectedAccount,   vAccKey, vAccValue,"onChange=\"javascript:changeTotal()\""));
                ...
                rowx.add("<input type=\"text\" size=\"25\" name=\""+FrmSpecialJournalDetail.fieldNames[FrmSpecialJournalDetail.FRM_AMOUNT ]+"\" value=\""+FRMHandler.userFormatStringDecimal(specJournalDetail.getAmount()/specJournalDetail.getCurrencyRate())+"\" onKeyUp=\"javascript:getText(this)\" class=\"hiddenTextR\" maxlength=\""+(strBalanceLength + 3)+"\">"+
						"<input type=\"hidden\" size=\"25\" name=\"amount\" value=\""+specialJournalDetail.getAmount()/specialJournalDetail.getCurrencyRate()+"\">");
        ...
        if((iCommand==Command.ADD || (iCommand==Command.SAVE && frmSpecialJournalDetail.errorSize()>0)) && vDepartmentOid.size() > 0){
            ...
            rowx.add("<input type=\"text\" size=\"25\" name=\""+FrmSpecialJournalDetail.fieldNames[FrmSpecialJournalDetail.FRM_AMOUNT ]+"\" value=\""+FRMHandler.userFormatStringDecimal(specialJournalDetail.getAmount()/specialJournalDetail.getCurrencyRate())+"\" onKeyUp=\"javascript:getText(this)\" class=\"hiddenTextR\" maxlength=\""+(strBalanceLength + 2)+"\">"+
					"<input type=\"hidden\" size=\"25\" name=\"amount\" value=\""+specialJournalDetail.getAmount()/specialJournalDetail.getCurrencyRate()+"\">");
...
double drate = FRMQueryString.requestDouble(request, "rate");
double totBlc = FRMQueryString.requestDouble(request, "total_val_balance");
...
String msgErr = "";
    ...
    try{		
		...
		System.out.println("get main "+e.toString());
	}
...
Vector vEndingBalance = SessSpecialJurnal.getTotalSaldoAccountLink(periodeOID,0);
   ...
   // if(objSpecialJournalDetail.getOID()==0){
        if(iCommand==Command.SAVE){
            ...
            if(iCommand==Command.NONE){
                try{
                    System.out.println("Index Edit in here");
                }catch(Exception e){}
            }else{
                try{
                    switch(iCommand){
    ...
    try{
		...
		System.out.println("Exception on fetch objContact : "+e.toString());
	}
...
Vector vListAccount = PstAccountLink.getVectObjListAccountLink(0, PstPerkiraan.ACC_GROUP_CASH);
...
function cmdSave(){
	...
	var cekSaldo = parseFloat(document.<%=FrmSpecialJournalDetail.FRM_SPECIAL_JOURNAL_DETAIL%>.erramount.value);
	...
	if(parseFloat(amountBalance) < parseFloat(amountDet)){
		...
		if(parseFloat(amountDetail) == 0){
			alert("<%=strCekAmountTransaction[SESS_LANGUAGE][1]%>");
		}else{
			document.<%=FrmSpecialJournalDetail.FRM_SPECIAL_JOURNAL_DETAIL%>.submit();
		}
	}
...
function getText(element){
	...
	<%}%>
	var balance = parseFloat(document.<%=FrmSpecialJournalDetail.FRM_SPECIAL_JOURNAL_DETAIL%>.total_val_balance.value);
	var amountBalance = parseFloat(document.<%=FrmSpecialJournalDetail.FRM_SPECIAL_JOURNAL_DETAIL%>.balance_amount.value);
	var amountMain = Math.abs(document.<%=FrmSpecialJournalDetail.FRM_SPECIAL_JOURNAL_DETAIL%>.amount_main.value);
	var blc = 0;
	var msg = "";
		if(rate != 1){
			
			if(amountBalance < balance){
					msg = "<%=strCekAmountTransaction[SESS_LANGUAGE][3]%>\n<%=strCekAmountTransaction[SESS_LANGUAGE][2]%>";
				blc = amountBalance / rate;
			}else{
				blc = balance / rate;
				
					msg = "<%=strCekAmountTransaction[SESS_LANGUAGE][0]%>\n<%=strCekAmountTransaction[SESS_LANGUAGE][2]%>";
...
function cmdopen(){
	closeWindow();
	myWindow = window.open("activity.jsp","src_donor_comp_bank_dep","height=500,width=800,status=yes,toolbars=no,menubar=no,location=no,scrollbars=yes");
}

function cmdopencontact(){
	closeWindow();
	myWindow = window.open("donor_list.jsp?src_from=1&&command=<%=Command.LIST%>","src_cnt_bank_dep_det","height=500,width=800,status=yes,toolbars=no,menubar=no,location=no,scrollbars=yes");
}
...
function changeAccount(){
	var idAccount = 0;
	<%if(iCommand != Command.REFRESH){%>
		idAccount = document.<%=FrmSpecialJournalDetail.FRM_SPECIAL_JOURNAL_DETAIL%>.<%=FrmSpecialJournalDetail.fieldNames[FrmSpecialJournalDetail.FRM_ID_PERKIRAAN]%>.value;
	<%}%>
	switch(idAccount){
		<%
			if(vListAccount != null && vListAccount.size() > 0){
				for(int i = 0; i < vListAccount.size(); i++){
					Perkiraan objPerkiraan = (Perkiraan)vListAccount.get(i);
		%>	
		case "<%=objPerkiraan.getOID()%>":
...
function changeCurrType()
{
	...
	%>
		default :
			<%if(iCommand != Command.REFRESH){%>
			 document.<%=FrmSpecialJournalDetail.FRM_SPECIAL_JOURNAL_DETAIL%>.<%=FrmSpecialJournalDetail.fieldNames[FrmSpecialJournalDetail.FRM_CURRENCY_RATE]%>.value = "<%=1%>";
			 <%}%>
			 break;
	}
}
...
function changeTotal()
{
	var iCurrType = 0;
	<%if(iCommand != Command.REFRESH){%>
		iCurrType = document.<%=FrmSpecialJournalDetail.FRM_SPECIAL_JOURNAL_DETAIL%>.<%=FrmSpecialJournalDetail.fieldNames[FrmSpecialJournalDetail.FRM_ID_PERKIRAAN]%>.value;
	<%}%>
	switch(iCurrType){
        <%
		int iErrAmount = 0;
        if(vEndingBalance!=null && vEndingBalance.size()>0){
                for(int k=0; k < vEndingBalance.size(); k++){
                    Vector temp = (Vector) vEndingBalance.get(k);
					double dBlc = Double.parseDouble(temp.get(1).toString());                              
                %>
		case "<%=temp.get(0)%>" :
			 var balance = 0;
			 document.<%=FrmSpecialJournalDetail.FRM_SPECIAL_JOURNAL_DETAIL%>.total_val_balance.value = <%=Double.parseDouble((String)temp.get(1))%>;
			 var currCode = document.<%=FrmSpecialJournalDetail.FRM_SPECIAL_JOURNAL_DETAIL%>.curr_code.value;
			 var rate = parseFloat(document.<%=FrmSpecialJournalDetail.FRM_SPECIAL_JOURNAL_DETAIL%>.rate.value);
			 balance = parseFloat(document.<%=FrmSpecialJournalDetail.FRM_SPECIAL_JOURNAL_DETAIL%>.total_val_balance.value);
			 blc = 	document.<%=FrmSpecialJournalDetail.FRM_SPECIAL_JOURNAL_DETAIL%>.total_val_balance.value;		 
			 var amount = parseFloat(document.<%=FrmSpecialJournalDetail.FRM_SPECIAL_JOURNAL_DETAIL%>.amount.value);
			 var tAmount = 	document.<%=FrmSpecialJournalDetail.FRM_SPECIAL_JOURNAL_DETAIL%>.amount.value;		  
			 var amnt = 0;
			if(rate != 1){
				amnt = amount * rate;
			 }else{
			 ...
			 var strBalance = formatCurrency(blc,',','.');
			 var strAmount = formatCurrency(amnt,',','.');
			
			 if(amnt > balance){
			 	<% iErrAmount = 1; %>
				document.<%=FrmSpecialJournalDetail.FRM_SPECIAL_JOURNAL_DETAIL%>.erramount.value=<%=iErrAmount%>;
				alert("<%=strCekAmountTransaction[SESS_LANGUAGE][0]%>\n<%=strCekAmountTransaction[SESS_LANGUAGE][2]%>"+strBalance);
				document.<%=FrmSpecialJournalDetail.FRM_SPECIAL_JOURNAL_DETAIL%>.<%=FrmSpecialJournalDetail.fieldNames[FrmSpecialJournalDetail.FRM_AMOUNT]%>.value="<%=Formater.formatNumber(dBlc,"##,###.##")%>";
			 }else{
			 	<% iErrAmount = 0; %>
				document.<%=FrmSpecialJournalDetail.FRM_SPECIAL_JOURNAL_DETAIL%>.erramount.value=<%=iErrAmount%>;
				document.<%=FrmSpecialJournalDetail.FRM_SPECIAL_JOURNAL_DETAIL%>.<%=FrmSpecialJournalDetail.fieldNames[FrmSpecialJournalDetail.FRM_AMOUNT]%>.value=strAmount;
			 }
			 break;
        <%
            }
        ...
        <tr> 
          <td valign="top"><!-- #BeginEditable "content" -->
            <form name="<%=FrmSpecialJournalDetail.FRM_SPECIAL_JOURNAL_DETAIL%>" method="post" action="">
			  ...
			  <input type="hidden" name="amount_main" value="<%=objSpecialJournalMain.getAmount()%>">
			  <input type="hidden" name="rate" value="">
			  <input type="hidden" name="balance_amount" value="<%=dBalance%>">
			  <input type="hidden" name="erramount" value="<%=iErrAmount%>">
                ...
                <tr>
                  <td valign="top" height="372">
                    <table border="0" cellspacing="0" cellpadding="0" width="100%">
                      ...
                      <tr>
                        <td width="50%" class="tabtitleactive">
                            ...
                            <tr>                              
                              <td width="100%" valign="top">
								...
								<table width="100%" border="0" cellpadding="0" cellspacing="0" class="list">								
                                  ...
                                  <tr>
                                    <td colspan="3">&nbsp;&nbsp;<b><font face="Verdana, Arial, Helvetica, sans-serif" size="+1" color="#0000CC"><%=strTableName%></font></b></td>
                                  </tr>
                                  <tr>
                                  ...
                                  <tr>
                                    <td colspan="3"><%=listJurnalDetail(iCommand,SESS_LANGUAGE,frmSpecialJournalDetail,objectClass,vDepartmentOid,objSpecialJournalDetail,specialJournalDetailAssignt,vlistCurrencyType,indexEdit,approot,vListAccount,ctrlErrDetail,strBalanceLength)%></td>
                                  </tr>
                                  <tr>
                                    <td colspan="3" class="msgerror">
