...
public String selectOne(int language, Vector objectClass)
{
	String result = "";
		if(objectClass!=null && objectClass.size()>0){
			...
			for(int i=0; i<objectClass.size(); i++){
				...
				if(compName.length()==0)
					...
					compName = caracterReplace(compName);
				 String address = cekNull(contactList.getBussAddress());
				if(address.length()==0)
					address = cekNull(contactList.getHomeAddr());
					address = caracterReplace(address);
				
				String strTown = contactList.getTown()+"&nbsp;&nbsp;&nbsp;&nbsp;"+contactList.getPostalCodeCompany();
				String strCountry = contactList.getProvince()+", &nbsp;&nbsp;"+contactList.getCountry();
				
				strTown = caracterReplace(strTown);
				strCountry = caracterReplace(strCountry);	         	
				result ="<script language=\"JavaScript\"> cmdEdit('"+(contactList.getOID()+"','"+
							  compName+"','"+address+"','"+strTown+"','"+strCountry+"');</script>");
			}
		}else{
...
public String drawList(int language, Vector objectClass, long oid, int start){
	...
	for (int i = 0; i < objectClass.size(); i++) {
		...
		lstData.add(rowx);
		lstLinkData.add(String.valueOf(contactList.getOID())+"','"+compName+"','"+address+"','"+strTown+"','"+strCountry);
	}
...
long contactClass = FRMQueryString.requestLong(request, "cnt_type_cashier");
...
if(cHeckBoxContact.equalsIgnoreCase("N")){
	if(iCommand==Command.LIST || iCommand==Command.BACK || iCommand==Command.NONE){	
		 ...
		 long idContactClassCashier = 0;
		 if(srcFrom != 1){
		 	idContactClassCashier = PstContactClass.getIdContactType(PstContactClass.FLD_CLASS_CASHIER);
		 }else{
			idContactClassCashier = PstContactClass.getIdContactType(PstContactClass.CONTACT_TYPE_SUPPLIER);
		 }
...
function cmdAdd(){
		...
		document.frm_contactlist.action="donor_list.jsp.jsp";
...
function cmdEdit(oid, name, address, town, country){
    <%
        if(srcFrom==0){
    %>
        self.opener.document.forms.<%=FrmSpecialJournalMain.FRM_SPECIAL_JOURNAL_MAIN%>.<%=FrmSpecialJournalMain.fieldNames[FrmSpecialJournalMain.FRM_CONTACT_ID]%>.value = oid;
        self.opener.document.forms.<%=FrmSpecialJournalMain.FRM_SPECIAL_JOURNAL_MAIN%>.<%=FrmSpecialJournalMain.fieldNames[FrmSpecialJournalMain.FRM_CONTACT_ID]%>_TEXT.value = name;
		self.opener.document.all.address.innerHTML = address;
		self.opener.document.all.town.innerHTML = town;
		self.opener.document.all.country.innerHTML = country;
		self.opener.document.forms.<%=FrmSpecialJournalMain.FRM_SPECIAL_JOURNAL_MAIN%>.<%=FrmSpecialJournalMain.fieldNames[FrmSpecialJournalMain.FRM_REFERENCE]%>.focus();
    <%}else{%>
        self.opener.document.forms.<%=FrmSpecialJournalDetail.FRM_SPECIAL_JOURNAL_DETAIL%>.<%=FrmSpecialJournalDetail.fieldNames[FrmSpecialJournalDetail.FRM_CONTACT_ID]%>.value = oid;
            ...
            <form name="frm_contactlist" method="post" action="">
				  		...
				  		<table border="0" cellspacing="2" cellpadding="2" width="100%">
                            <tr align="left" valign="top"> 
                              ...
                              <td width="89%">
							   ...
							   String whClause = "";
							   if(srcFrom != 1){	
							   whClause = PstContactClass.fieldNames[PstContactClass.FLD_CLASS_TYPE]+" = "+PstContactClass.FLD_CLASS_CASHIER;
							  }else{
								whClause = PstContactClass.fieldNames[PstContactClass.FLD_CLASS_TYPE]+" = "+PstContactClass.CONTACT_TYPE_SUPPLIER;
							  }
							  ...
							  %>
							  <input name="<%=frmSrcContactList.fieldNames[frmSrcContactList.FRM_FIELD_TYPE]%>" type="checkbox" <%=disabled%> class="formElemen" value="<%=idxContactClass%>" <%if(getTruedFalse(srcContactList.getType(),idxContactClass)){%>checked<%}%> >
                            ...
                            <tr align="left" valign="top"> 
                              ...
                              <td width="1%">:</td>
                              <td width="89%">&nbsp; 
                                <%= ControlCombo.draw(frmSrcContactList.fieldNames[frmSrcContactList.FRM_FIELD_ORDER],"",null, ""+srcContactList.getOrderBy(), frmSrcContactList.getOrderVal(), frmSrcContactList.getOrdKey(SESS_LANGUAGE), " onkeydown=\"javascript:fnTrapKD()\"") %> </td>
                            </tr>
                            <tr align="left" valign="top"> 
                              <td width="10%">&nbsp;</td>
                              <td width="1%">&nbsp;</td>
			  ...
			  <table width="100%" border="0" cellspacing="0" cellpadding="0">
                ...
                <tr> 
                  ...
                  <td>
				  <%if(records.size() == 1){%>
						<%out.println(selectOne(SESS_LANGUAGE, records));%>
					<%}else{%>				  	
				  		<%=drawList(SESS_LANGUAGE,records,oidContactList,start)%>
					<%}%>
