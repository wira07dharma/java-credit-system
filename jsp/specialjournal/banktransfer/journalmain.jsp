...
public static final String subMasterTitle[] = {
	"Jurnal Transfer Bank","Bank Transfer Journal"
};
    ...
    Vector vSaldoAkhir = SessSpecialJurnal.getTotalSaldoAccountLink(periodeOID,0);
        if(ctrlErr ==0 && iCommand==Command.POST && 
			...
			objSpecialJournalMain.getReference().length()>0){
				...
				response.sendRedirect("journaldetail.jsp?command=Command.EDIT");  
             }
...
if(session.getValue("TRANSFER") != null){
	session.removeValue("TRANSFER");
}
	session.putValue("TRANSFER",vDataToPdf);
...
function cmdopen(){
	...
	myWindow = window.open(url,"src_cnt_bank_tran_main","height=500,width=800,status=yes,toolbars=no,menubar=no,location=no,scrollbars=yes");
}
function entContact(){
	if(event.keyCode == 13)
	cmdopen();
...
function printJournal(){
		var linkPage = "<%=reportroot%>report.specialJournal.TransferJournalPdf";
        ...
        <tr> 
          <td valign="top"><!-- #BeginEditable "content" -->
            <form name="<%=FrmSpecialJournalMain.FRM_SPECIAL_JOURNAL_MAIN%>" method="post" action="">
              ...
              <input type="hidden" name="<%=FrmSpecialJournalMain.fieldNames[FrmSpecialJournalMain.FRM_JOURNAL_TYPE]%>" value="<%=I_JournalType.TIPE_SPECIAL_JOURNAL_BANK_TRANSFER%>" size="35">
                                    ...
                                    <td height="25" colspan="3" ><div align="center">
										...
										%> <%= ControlCombo.draw(frmSpecialJournalMain.fieldNames[frmSpecialJournalMain.FRM_ID_PERKIRAAN],null, selectPerkiraan, key, val, "onChange=\"javascript:changeTotal()\"", "")%> 
                                          <%}else{
												Perkiraan objPerk = new Perkiraan();
												try{
													System.out.println("objSpecialJournalMain.getIdPerkiraan() ===> "+objSpecialJournalMain.getIdPerkiraan());
                                    ...
                                    <td height="25" colspan="3"><div align="center">
                                        ...
                                        <tr>
                                          ...
                                          <td width="17%">
                                            ...
                                            <%}%></td>
											<td width="23%" nowrap><% if(updateJournal.equalsIgnoreCase("Y") || (iCommand != Command.EDIT)){%>										    
											  <font face="Verdana, Arial, Helvetica, sans-serif" color="#FF0033">*)
