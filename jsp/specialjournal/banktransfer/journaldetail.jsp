...
public String listJurnalDetail(int iCommand, int language,
        ...
        for(int i=0; i<objectClass.size(); i++){
            ...
            rowx = new Vector();
            if(index==i && (iCommand==Command.EDIT || iCommand==Command.ASK) && vDepartmentOid != null){
				if(errSize > 0)
...
Vector vListAccount = PstAccountLink.getVectObjListAccountLink(0, PstPerkiraan.ACC_GROUP_BANK,objSpecialJournalMain.getIdPerkiraan());
...
function cmdopen(){
	window.open("activity.jsp","src_donor_comp_bank_tran","height=500,width=800,status=yes,toolbars=no,menubar=no,location=no,scrollbars=yes");
}

function cmdopencontact(){
	window.open("donor_list.jsp?src_from=1&&command=<%=Command.LIST%>","src_cnt_bank_tran_det","height=500,width=800,status=yes,toolbars=no,menubar=no,location=no,scrollbars=yes");
}
