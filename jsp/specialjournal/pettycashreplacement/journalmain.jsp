...
public static final String subMasterTitle[] = {
	"Jurnal Pengisian Kas Kecil","Petty Cash Replacement Journal"
};
...
if(session.getValue("REPLACEMENT") != null){
	session.removeValue("REPLACEMENT");
}
	session.putValue("REPLACEMENT",vDataToPdf);
%>
<!-- End of JSP Block -->
...
function cmdopen(){
	...
	myWindow = window.open(url,"src_cnt_pcr_main","height=500,width=800,status=yes,toolbars=no,menubar=no,location=no,scrollbars=yes");
}
...
function printJournal(){
		var linkPage = "<%=reportroot%>report.specialJournal.PCReplacementJournalPdf";
        ...
        <tr> 
          <td valign="top"><!-- #BeginEditable "content" -->
            <form name="<%=FrmSpecialJournalMain.FRM_SPECIAL_JOURNAL_MAIN%>" method="post" action="">
              ...
              <input type="hidden" name="<%=FrmSpecialJournalMain.fieldNames[FrmSpecialJournalMain.FRM_JOURNAL_TYPE]%>" value="<%=I_JournalType.TIPE_SPECIAL_JOURNAL_REPLACEMENT%>" size="35">
                                      ...
                                      <table width="99%" border="0" cellpadding="1" cellspacing="0" bgcolor="#D6EEFA">
                                        ...
                                        <tr>
										  ...
										  <%if(cekValue(objSpecialJournalMain.getAmount()) == true){%>											  
												<em>(<%=numberSpeller.spellNumberToEngLong(Long.parseLong(Formater.formatNumber((objSpecialJournalMain.getAmount() / objSpecialJournalMain.getStandarRate()),"###")))%>&nbsp;&nbsp;<%=currType.getName()%>)</em></font>
											<%}%>	
											</td>
                                        </tr>
                                        <tr>
