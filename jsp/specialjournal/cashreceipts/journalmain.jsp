...
public static final String strCekAmountTransaction[][] = {
	...
	{"Transaction Amount Over than Balance Amount","Transaction value has to be greater than zero"}
};
        ...
        for(int i=0; i<objectClass.size(); i++){
			...
			if(objPerk != null){
				if(language == com.dimata.util.lang.I_Language.LANGUAGE_FOREIGN){
					strAccName = objPerk.getAccountNameEnglish();
				}else{
					strAccName = objPerk.getNama();
				}
			}
            ...
            }catch(Exception e){objectCurrencyType =new CurrencyType();}
...
FrmSrcContactList frmSrcContactList = new FrmSrcContactList();
    ...
    if(objSpecialJournalMain.getOID()==0){
        ...
        if(iCommand==Command.POST){
			if(objSpecialJournalMain.getAmount() != 0 && 
				...
				objSpecialJournalMain.getFrcontactId() != 0 		 	
			){	
				try{ // ini digunakan untuk nyimpan data ke session saat tombol post
					...
					System.out.println("Exception : Failed insert to session");
				}
			}else{
				objSpecialJournalMain.setAmount(objSpecialJournalMain.getAmount() * objSpecialJournalMain.getStandarRate());
				//objSpecialJournalMain = new SpecialJournalMain();
	...
	String contactNameFr = "";
    ContactList cntList = new ContactList();
	try{
        cntList = PstContactList.fetchExc(objSpecialJournalMain.getFrcontactId());
        contactNameFr = cntList.getCompName();
        if(contactName.length()==0)
            contactNameFr = cntList.getPersonName()+" "+cntList.getPersonLastname();
    }catch(Exception e){
    ...
    }catch(Exception e){currPeriod = new Periode();}
...
long idContactClassCashier = PstContactClass.getIdContactType(PstContactClass.FLD_CLASS_CASHIER);
long idContactClassCollector = PstContactClass.getIdContactType(PstContactClass.FLD_CLASS_COLLECTOR);
...
vMain.add(contactNameFr); // Index 16 
vMain.add(cekNull(cntList.getBussAddress())); // Index 17
vMain.add(cekNull(cntList.getTown())); // Index 18
vMain.add(cekNull(cntList.getProvince())); // Index 19
vMain.add(cekNull(cntList.getCountry())); // Index 20

Vector vDetail =(Vector)listJurnalDetail(SESS_LANGUAGE,objectClass,approot,updateJournal,iCommand); 
...
if(session.getValue("CASH_RECEIPT") != null){
	session.removeValue("CASH_RECEIPT");
}
	session.putValue("CASH_RECEIPT",vDataToPdf);			 
%>
<!-- End of JSP Block -->
...
function cmdAddDetail(){
	var totTrans = parseFloat(document.<%=FrmSpecialJournalMain.FRM_SPECIAL_JOURNAL_MAIN%>.<%=FrmSpecialJournalMain.fieldNames[FrmSpecialJournalMain.FRM_AMOUNT]%>.value);
	document.<%=FrmSpecialJournalMain.FRM_SPECIAL_JOURNAL_MAIN%>.command.value="<%=Command.POST%>";
	document.<%=FrmSpecialJournalMain.FRM_SPECIAL_JOURNAL_MAIN%>.action="journalmain.jsp";
	if(totTrans != "0" || totTrans == null){
	document.<%=FrmSpecialJournalMain.FRM_SPECIAL_JOURNAL_MAIN%>.submit();
	}else{
...
function cmdopen(){
	...
	myWindow = window.open(url,"src_cnt_cash_rcv_main","height=500,width=800,status=yes,toolbars=no,menubar=no,location=no,scrollbars=yes");
}

function cmdOpenContact(){
	closeWindow();
	var url = "";
	var idContact = Math.abs(document.<%=FrmSpecialJournalMain.FRM_SPECIAL_JOURNAL_MAIN%>.<%=frmSpecialJournalMain.fieldNames[FrmSpecialJournalMain.FRM_FR_CONTACT_ID]%>.value);
	
	if(idContact == 0){
		url = "cnt_fr_list.jsp?command=<%=Command.LIST%>&"+
				"<%=frmSrcContactList.fieldNames[frmSrcContactList.FRM_FIELD_NAME] %>="+document.<%=FrmSpecialJournalMain.FRM_SPECIAL_JOURNAL_MAIN%>.<%=frmSpecialJournalMain.fieldNames[FrmSpecialJournalMain.FRM_FR_CONTACT_ID]%>_TEXT.value;				
	}else{
		url = "cnt_fr_list.jsp?command=<%=Command.LIST%>";
	}
	myWindow = window.open(url,"src_cnt_fr_cash_rcv_main","height=500,width=800,status=yes,toolbars=no,menubar=no,location=no,scrollbars=yes");
}
...
function printJournal(){
		var linkPage = "<%=reportroot%>report.specialJournal.CashReceiptsJournalPdf";
        ...
        <tr> 
          <td valign="top"><!-- #BeginEditable "content" -->
            <form name="<%=FrmSpecialJournalMain.FRM_SPECIAL_JOURNAL_MAIN%>" method="post" action="">
			  ...
			  <input type="hidden" name="cnt_type_cashier" value="<%=idContactClassCashier%>">
			  <input type="hidden" name="cnt_type_collector" value="<%=idContactClassCollector%>">
                                    ...
                                    <td height="25" colspan="3" ><div align="center">
                                        ...
                                        <tr >
										  ...
										  <% 	if(updateJournal.equalsIgnoreCase("Y") || (iCommand != Command.EDIT)){%>
												...
												%> 
												<%= ControlCombo.draw(frmSpecialJournalMain.fieldNames[frmSpecialJournalMain.FRM_ID_PERKIRAAN],null, selectPerkiraan, key, val, "onChange=\"javascript:changeTotal()\"", "")%> </strong>
												<%}else{
												Perkiraan objPerk = new Perkiraan();
												try{													
                                          ...
                                          <td width="30%"><table width="60%"  border="0" >
                                              <tr>
												...
												<% if(updateJournal.equalsIgnoreCase("Y") || (iCommand != Command.EDIT)){%>	
													...
													<%}else{%>
                                                    <%
													out.println(FRMHandler.userFormatStringDecimal(salakhir));%>
                                  ...
                                  <tr>
                                    <td colspan="3"><div align="center">
                                        ...
                                        <tr>
                                          ...
                                          <td colspan="2" >
										  		<% if(updateJournal.equalsIgnoreCase("Y") || (iCommand != Command.EDIT)){%>
                                          			...
                                          			<span class="style1">*) <font face="Verdana, Arial, Helvetica, sans-serif" >
													<%if(ctrlErr != ctrlSpecialJournalMain.RSLT_FORM_INCOMPLETE || ctrlErr == ctrlSpecialJournalMain.RSLT_ERR_MEMO){%>													
												 	...
												 	<%}%>
													</font><font face="Verdana, Arial, Helvetica, sans-serif" ></font></span>
												<%}else{%>
                                        ...
                                        <tr>
										  ...
										  <%if(ctrlErr != ctrlSpecialJournalMain.RSLT_FORM_INCOMPLETE || ctrlErr == ctrlSpecialJournalMain.RSLT_ERR_MEMO){%>
										  <%=frmSpecialJournalMain.getErrorMsg(FrmSpecialJournalMain.FRM_CONTACT_ID)%>
										  <%}%></span></font>
										  </td>
                                        ...
                                        <tr>
                                          ...
                                          <td width="17%">
                                            ...
                                            <%}%></td>
											<td width="23%" class="style1" nowrap><% if(updateJournal.equalsIgnoreCase("Y") || (iCommand != Command.EDIT)){%>
                                        ...
                                        <tr>
                                          <td>&nbsp;</td>
                                          <td colspan="2"><b><%=strTitleMain[SESS_LANGUAGE][11]%></b></td>
                                          <td><b>:</b></td>
                                          <td><input type="hidden" readonly name="<%=frmSpecialJournalMain.fieldNames[FrmSpecialJournalMain.FRM_FR_CONTACT_ID]%>" value="<%=objSpecialJournalMain.getFrcontactId()%>" size="25">
                                            <% if(updateJournal.equalsIgnoreCase("Y") || (iCommand != Command.EDIT)){%>
                                            <span class="msgErrComment">
                                            <input type="text" name="<%=frmSpecialJournalMain.fieldNames[FrmSpecialJournalMain.FRM_FR_CONTACT_ID]%>_TEXT" value="<%=contactNameFr%>" size="25" onKeyDown="javascript:entCntFrom()">
                                            <a href="javascript:cmdOpenContact()"><img border="0" src="<%=approot%>/dtree/img/folderopen.gif"></a> *) </span>
                                            <%}else{%>
                                            <%=contactNameFr%>
                                            <%}%></td>
                                          <td>&nbsp;</td>
                                        ...
                                        <tr>
                                          ...
                                          <td colspan="2"><b><%=strTitleMain[SESS_LANGUAGE][6]%></b></td>
                                          <td><strong>:</strong></td>
                                          <td><a id="address_fr"></a><%=cntList.getBussAddress() == null? "-" : cntList.getBussAddress()%><font face="Verdana, Arial, Helvetica, sans-serif" ><span class="fielderror"><%=frmSpecialJournalMain.getErrorMsg(FrmSpecialJournalMain.FRM_FR_CONTACT_ID)%></span></font></td>
                                          <td>&nbsp;</td>
                                          <td>&nbsp;</td>
                                          <td>&nbsp;</td>
                                          <td>&nbsp;</td>
                                        ...
                                        <tr>
                                          ...
                                          <td colspan="2">&nbsp;</td>
                                          <td>&nbsp;</td>
                                          <td><a id="townfr"></a><%=cntList.getTown() == null? "-" : cntList.getTown()%>&nbsp;&nbsp;&nbsp;&nbsp;<%=cntList.getPostalCodeCompany() == null? "-" : cntList.getPostalCodeCompany()%></td>
                                          <td>&nbsp;</td>
                                          <td>&nbsp;</td>
                                          <td>&nbsp;</td>
                                          <td>&nbsp;</td>
                                        ...
                                        <tr>
                                          ...
                                          <td colspan="2">&nbsp;</td>
                                          <td>&nbsp;</td>
                                          <td><a id="countryfr"></a><%=cntList.getProvince() == null? "-" : cntList.getProvince()%>&nbsp;&nbsp;<%=cntList.getCountry() == null? "-" : cntList.getCountry()%></td>
                                          <td>&nbsp;</td>
                                          <td>&nbsp;</td>
                                          <td>&nbsp;</td>
                                          <td>&nbsp;</td>
                                        ...
                                        <tr>
                                          ...
                                          <td colspan="6">
										  	<% if(updateJournal.equalsIgnoreCase("Y") || (iCommand != Command.EDIT)){%>								  			  
											<span class="msgErrComment"><input name="<%=frmSpecialJournalMain.fieldNames[FrmSpecialJournalMain.FRM_DESCRIPTION]%>" type="text" value="<%=objSpecialJournalMain.getDescription()%>" size="100" onKeyDown="javascript:entDescription()"> 
                                   			  **)</span>
                                                    <%}else{%>
                                        ...
                                        <tr>
                                          <td>&nbsp;</td>
                                          <td colspan="2">&nbsp;</td>
                                          <td colspan="6"><span class="msgErrComment"><font face="Verdana, Arial, Helvetica, sans-serif"><em>
										  <% if ((ctrlSpecialJournalMain.getMessage()).length() == 0) { %>
										  </em></font></span><font face="Verdana, Arial, Helvetica, sans-serif">
										  <%if (ctrlErr != ctrlSpecialJournalMain.RSLT_FORM_INCOMPLETE) { %>
                                            ...
                                            <%}%>
										  </font><font face="Verdana, Arial, Helvetica, sans-serif"><em></em><span class="msgErrComment"><em>
										  <%}%>
										  </em></span></font></td>
                                        </tr>
                                        <tr>
                                          <td>&nbsp;</td>
                                          <td colspan="2">&nbsp;</td>
                                          <td colspan="6" class="style1"><font face="Verdana, Arial, Helvetica, sans-serif">
                                            <% if(updateJournal.equalsIgnoreCase("Y") || (iCommand != Command.EDIT)){%>
                                            	<font face="Verdana, Arial, Helvetica, sans-serif">*)<%=strRequired%>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;**) <%=strMaxCaracter%></font>
                                            	</font>
											<%}%>
										</td>
                                        </tr>
                                  ...
                                  <tr>
									...
									<%if(objectClass != null && objectClass.size() > 0){%>
									  ...
									  <tr>
										...
										<%if (ctrlErr == ctrlSpecialJournalMain.RSLT_ERR_TRANS_DATE || ctrlErr == ctrlSpecialJournalMain.RSLT_ERR_ENTRY_DATE) { %>
