...
public static final String subMasterTitle[] = {
	"Jurnal Non Kas","Non Cash Journal"
};
...
public String listJurnalDetailCoA(int iCommand, int language,
        ...
        for(int i=0; i<objectClass.size(); i++){
			...
			if(vListAccount != null && vListAccount.size() > 0){
                ...
                rowx.add(deptObject.getDepartment());
                rowx.add("<div align=\"left\"><a href=\"javascript:cmdEdit('"+i+"')\">"+strAccName+"</a></div>");
                rowx.add("<div align=\"left\">"+contactList.getCompName()==null?contactList.getPersonName():contactList.getCompName()+"</div>");
                rowx.add("<div align=\"center\">"+objectCurrencyType.getCode()+"</div>");
                rowx.add("<div align=\"right\">"+FRMHandler.userFormatStringDecimal(specJournalDetail.getCurrencyRate())+"</div>");
                rowx.add("<div align=\"right\">"+Formater.formatNumber((specJournalDetail.getAmount() / specJournalDetail.getCurrencyRate()), "##,###.##")+"</div>");
...
Vector vListAccount = PstAccountLink.getVectObjListAccountLink(0, PstPerkiraan.ACC_GROUP_EXPENSE);
...
function cmdopen(){
	closeWindow();
	myWindow = window.open("activity.jsp","src_donor_comp_noncash_det","height=500,width=800,status=yes,toolbars=no,menubar=no,location=no,scrollbars=yes");
}

function cmdopencontact(){
	closeWindow();
	myWindow = window.open("employee_list.jsp?command=<%=Command.LIST%>","src_cnt_noncash_det","height=500,width=800,status=yes,toolbars=no,menubar=no,location=no,scrollbars=yes");
}
						  ...
						  <tr><td></td></tr>
                            <tr>
                              <td width="94%" valign="top"><div align="center">
                              	<table width="100%" border="0" cellpadding="1" cellspacing="0" bgcolor="99CC99">
                                  ...
                                  <tr> 
									...
									<%}else{%>
										<%=listJurnalDetailCoA(iCommand,SESS_LANGUAGE,frmSpecialJournalDetail,objectClass,vDepartmentOid,objSpecialJournalDetail,specialJournalDetailAssignt,vlistCurrencyType,indexEdit,approot,vListAccount,ctrlErrDetail,strBalanceLength)%>
									<%}%>
									</td>
                                  </tr>
                                  <tr>
