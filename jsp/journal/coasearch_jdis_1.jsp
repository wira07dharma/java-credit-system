	...
	String result = "";
		if(objectClass!=null && objectClass.size()>0){
			...
			for(int i=0; i<objectClass.size(); i++){
				...
				result ="<script language=\"JavaScript\"> cmdEdit('"+(perkiraan.getOID()+"','"+
...
String sAccCode = FRMQueryString.requestString(request,"acc_code");
String accountName = FRMQueryString.requestString(request,"account_name");
String sDebetAmount = FRMQueryString.requestString(request,"dDebetAmount");
String sCreditAmount = FRMQueryString.requestString(request,"dCreditAmount");
...
function cmdEdit(oid,number,name){
	...
	self.opener.document.forms.<%=FrmJournalDistribution.FRM_JOURNAL_DISTRIBUTION%>.<%=FrmJournalDistribution.fieldNames[FrmJournalDistribution.FRM_ID_PERKIRAAN]%>.value = oid;
        // alert(self.opener.document.forms.<%=FrmJournalDistribution.FRM_JOURNAL_DISTRIBUTION%>.account_code);
	self.opener.document.forms.<%=FrmJournalDistribution.FRM_JOURNAL_DISTRIBUTION%>.account_code.value = number;
         //alert(self.opener.document.forms.<%=FrmJournalDistribution.FRM_JOURNAL_DISTRIBUTION%>.account_name);
	self.opener.document.forms.<%=FrmJournalDistribution.FRM_JOURNAL_DISTRIBUTION%>.account_name.value = name;
        /*
      ...
      <td>
        <table width="100%" border="0" cellpadding="1" cellspacing="1" class="listgenactivity">
          ...
          <tr> 
            <td colspan="3">
			<%if(vect.size() == 1){%>
				<%out.println(selectOne(SESS_LANGUAGE, vect));%>
			<%}else{%>		
