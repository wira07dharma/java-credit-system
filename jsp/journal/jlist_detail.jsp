...
public String drawList(FRMHandler objFRMHandler, Vector objectClass, long oid, int language)
{
	...
	ctrlist.dataFormat(strTitle[language][1],"8%","center","center");
	...
	/*ctrlist.dataFormat(strTitle[language][9],"17%","center","left");
	...
	try
	{
		...
		for (int i = 0; i < objectClass.size(); i++) 
		{
			 ...
			 for(int k=0; k< vectListJd.size(); k++){
			 	jDetail = (JurnalDetail) vectListJd.get(k);	
				System.out.println("ini k ke -  "+k);
				idt = idt + k;			
				if(jDetail.getIdPerkiraan()==perk.getIdParent()){
					try{
						 debet = jDetail.getDebet();
						 credit = jDetail.getKredit();
					}catch(Exception e){
					}
					break;
				}else{
			 ...
			 if(!novoucher.equals(jUmum.getSJurnalNumber())){
			 	 novoucher = jUmum.getSJurnalNumber();
				...
				//System.out.println("idt ======> "+idt);
				 if(idt < 0 ){
				 	rowx.add("");					
					rowx.add("<div align=\"center\"><b>"+strTitle[language][9]+"</b></div>");				 
				 	rowx.add("<div align=\"center\"><b>"+strTitle[language][4]+"</b></div>");           		 			 
				 	rowx.add("<div align=\"center\"><b>"+strTitle[language][5]+"</b></div>");		
				 }else{
				 	rowx.add(" ");
				 	rowx.add(perk.getNama());
					 if(jDetail.getDebet() > 0){			 			
			 			rowx.add(objFRMHandler.userFormatStringDecimal(debet)); 		 	 		 		 
			 			rowx.add(" ");						
			 		}else{			 			
			 			rowx.add(" "); 		 	 		 		 
			 			rowx.add(objFRMHandler.userFormatStringDecimal(credit));
			 		} 
				 }  			 
			 }
