...
public static String strHeader[][] = {
	{"PERKIRAAN","Anggaran","Realisasi","Pencapaian","Rp","%"},
	{"ACCOUNT","Budget","Actual","VAR to Budget","IDR","%"}
};
...
public static final String listTitle[] = {
	"Laba(Rugi) Tahunan ","Profit and Loss Annual Detail"
};
...
private static Vector listChild(ProfitLoss objProfitLoss, int language, double dblValueReport, double dYearBudget, 
double dYearActual, double totYearBudget, double totYearActual){	
	...
	try{
		...
		vRowx.add("<div align=\"right\">"+formatNumber((objProfitLoss.getYtdBudget() == 0? 0 : (objProfitLoss.getYtdValue() - objProfitLoss.getYtdBudget())/objProfitLoss.getYtdBudget() * 100))+"</div>");	
		...
		vRowR.add(formatNumber((objProfitLoss.getYtdBudget() == 0? 0 : (objProfitLoss.getYtdValue() - objProfitLoss.getYtdBudget())/objProfitLoss.getYtdBudget() * 100)));	
		vRowR.add(child);
...
private static Vector subTotalChild(Perkiraan objPerkiraan, int language, double dblValueReport, double dYearBudget, double dYearActual){
	Vector vResult = new Vector(1,1);	
				...
				for(int i = 0; i < vData.size(); i++){
					...
					if(i > 0){
						...
						if(idParent != oidOldParent){
							...
							Vector subTotalChild = (Vector)subTotalChild(oldObjParentPerkiraan, language, dblValueReport, dYearBudget, dYearActual);
							...
							Vector vChild = (Vector)listChild(objProfitLoss, language, dblValueReport, dYearBudget,	dYearActual, totYearBudget, totYearActual);
							...
							if(vChild != null && vChild.size() > 0){
								...
								totYearBudget = Double.parseDouble(vChild.get(3).toString());
								totYearActual = Double.parseDouble(vChild.get(4).toString());
								rowR = (Vector)vChild.get(5);
								dataToReport.add(rowR);						
							}
														
						}else{
							Vector vChild = (Vector)listChild(objProfitLoss, language, dblValueReport, dYearBudget, dYearActual, 
							totYearBudget, totYearActual);
						...
						if(i == (vData.size() - 1)){
							Vector subTotalChild = (Vector)subTotalChild(objParentPerkiraan, language, dblValueReport, dYearBudget, dYearActual);
							if(subTotalChild != null && subTotalChild.size() > 0){
								rowx = (Vector)subTotalChild.get(0);
								listData.add(rowx);
						...
						if(vData.size() == 1){
										Vector vChild = (Vector)listChild(objProfitLoss, language, dblValueReport, dYearBudget, dYearActual, totYearBudget, 
										totYearActual);
							...
							Vector vChildFirstIndex = (Vector)listChild(objProfitLoss, language, dblValueReport, dYearBudget, dYearActual, totYearBudget, 
							totYearActual);
							
							if(vChildFirstIndex != null && vChildFirstIndex.size() > 0){
								rowx = (Vector)vChildFirstIndex.get(0);
								listData.add(rowx);
								dYearBudget = Double.parseDouble(vChildFirstIndex.get(1).toString());
								dYearActual = Double.parseDouble(vChildFirstIndex.get(2).toString());
								totYearBudget = Double.parseDouble(vChildFirstIndex.get(3).toString());
								totYearActual = Double.parseDouble(vChildFirstIndex.get(4).toString());	
								
								rowR = (Vector)vChildFirstIndex.get(5);
								dataToReport.add(rowR);						
							}
						}						
					}
		...
		if(objClass != null && objClass.size() > 0){			
			...
			if(vCoGS != null && vCoGS.size() > 0){
				...
				try{
					...
					Vector subTotalCoGS = (Vector)subTotalGroup(language, totYtdBudget, totYtdActual, totGroupYearBudget, 
										totGroupYearActual, dblValueReport, 3);
			...
			if(vExp != null && vExp.size() > 0){
				...
				try{
					...
					Vector subTotalExp = (Vector)subTotalGroup(language, totYtdBudget, totYtdActual, totGroupYearBudget, 
										totGroupYearActual, dblValueReport, 5);
			...
			dPLYtdBudget = totYtdRevBudget - totYtdCoGSBudget - totYtdExpBudget + totYtdOthBudget;
...
<%
	...
	if(session.getValue("PL_ANNUAL_DETAIL")!=null){   
		session.removeValue("PL_ANNUAL_DETAIL");
	}
	session.putValue("PL_ANNUAL_DETAIL",vecSess);
...
function report(){
    ...
    if(val!=0){*/
        ...
        document.frmTrialBalance.action = "PL_YTD_Var.jsp";
...
function cmdBack(){
	...
	document.frmTrialBalance.action = "PL_YTD_Var.jsp#up";
	document.frmTrialBalance.submit();
}
</script>
<!-- #EndEditable -->
...
function reportPdf(){	 
		var linkPage = "<%=reportroot%>report.labaRugi.PLAnnualDetailPdf";       
		window.open(linkPage);  				
}

function printXLS(){	 
		var linkPage = "<%=reportroot%>report.labaRugi.PLAnnualDetailXls";       
		window.open(linkPage);  				
}
