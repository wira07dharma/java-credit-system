...
<html>
<head>
<title>Depreciation Loading ...</title>
...
int reportType = FRMQueryString.requestInt(request,SessOtherAsset.deFieldNames[SessOtherAsset.FLD_TYPE]);
int depType = FRMQueryString.requestInt(request,SessOtherAsset.deFieldNames[SessOtherAsset.FLD_DEP_TYPE]);
long accountId = FRMQueryString.requestLong(request,SessOtherAsset.deFieldNames[SessOtherAsset.FLD_ACC_NAME]);
long assetGroup = FRMQueryString.requestLong(request,SessOtherAsset.deFieldNames[SessOtherAsset.FLD_ASSET_GROUP]);
long assetId = FRMQueryString.requestLong(request,SessOtherAsset.deFieldNames[SessOtherAsset.FLD_ASSET_NO]);
long periodId = FRMQueryString.requestLong(request,SessOtherAsset.deFieldNames[SessOtherAsset.FLD_PERIODE]);
int perInterval = SessPeriode.getPeriodInterval(periodId);

String strType = "";
Date selectedDate = null;
if(reportType==SessOtherAsset.REPORT_MONTLY){
	try{
		int selectedYr = FRMQueryString.requestInt(request,SessOtherAsset.deFieldNames[SessOtherAsset.FLD_MONTH] + "_yr");
		int selectedMn = FRMQueryString.requestInt(request,SessOtherAsset.deFieldNames[SessOtherAsset.FLD_MONTH] + "_mn");
...
if(reportType==SessOtherAsset.REPORT_PERIODIC){
...
Vector vectDepretiation = SessOtherAsset.listDepretiation(SESS_LANGUAGE,accountId,assetGroup,assetId,depType);
...
if(session.getValue("OTHER_REPORT")!=null){
	session.removeValue("OTHER_REPORT");
}
session.putValue("OTHER_REPORT",listDepresiation);
%>

<script language="JavaScript">
	document.location="<%=reportroot%>session.report.OtherAssetPdf?gettime=<%=System.currentTimeMillis()%>"; 
</script>
