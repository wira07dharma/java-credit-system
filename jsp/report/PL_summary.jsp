...
public static final String listTitle[] = {
	"Rekap Laba(Rugi) Tahunan","Profit and Loss Annual Summary"
};
...
private static Vector listChild(ProfitLoss objProfitLoss, int language, double dblValueReport, double dYearActual){	
		...
		if(vData != null && vData.size() > 0){
			try{
				for(int i = 0; i < vData.size(); i++){
					...
					Vector vChild = (Vector)listChild(objProfitLoss, language, dblValueReport, dYearActual);
							
							if(vChild != null && vChild.size() > 0){
								rowx = (Vector)vChild.get(0);
								listData.add(rowx);
								dYearActual = Double.parseDouble(vChild.get(1).toString());
								rowR = (Vector)vChild.get(2);
		...
		if(objClass != null && objClass.size() > 0){			
			...
			if((vOthIncome != null && vOthIncome.size() > 0) || (vOthExpenses != null && vOthExpenses.size() > 0) || (vTax != null && vTax.size() > 0)){
				...
				try{
					...
					if(vTempRowOE != null && vTempRowOE.size() > 0){
...
<%
	...
	if(session.getValue("PL_SUMMARY")!=null){   
		session.removeValue("PL_SUMMARY");
	}
	session.putValue("PL_SUMMARY",vecSess);
...
function report(){
    ...
    if(val!=0){*/
        ...
        document.frmTrialBalance.action = "PL_summary.jsp";
...
function cmdBack(){
	...
	document.frmTrialBalance.action = "PL_summary.jsp#up";
	document.frmTrialBalance.submit();
}
...
function reportPdf(){	 
		var linkPage = "<%=reportroot%>report.labaRugi.PLAnnualSummaryPdf";       
		window.open(linkPage);  				
}

function printXLS(){	 
		var linkPage = "<%=reportroot%>report.labaRugi.PLAnnualSummaryXls";       
		window.open(linkPage);  				
}
