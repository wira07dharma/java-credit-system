...
public static final String listTitle[] = {
	"Rekap Laba(Rugi) Data Bulanan Tahunan","Profit and Loss Summary With MTD YTD"
};
...
<%
	...
	if(session.getValue("PL_SUM_YTDMTD")!=null){   
		session.removeValue("PL_SUM_YTDMTD");
	}
	session.putValue("PL_SUM_YTDMTD",vecSess);
...
function report(){
    ...
    if(val!=0){*/
        ...
        document.frmTrialBalance.action = "PL_summary_MTDYTD.jsp";
...
function cmdBack(){
	...
	document.frmTrialBalance.action = "PL_summary_MTDYTD.jsp#up";
	document.frmTrialBalance.submit();
}
...
function reportPdf(){	 
		var linkPage = "<%=reportroot%>report.labaRugi.PLSummaryMTDYTDPdf";       
		window.open(linkPage);  				
}

function printXLS(){	 
		var linkPage = "<%=reportroot%>report.labaRugi.PLSummaryMTDYTDXls";       
		window.open(linkPage);  				
}
