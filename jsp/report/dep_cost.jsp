<%@ page language="java" %>


<!--import aiso-->
<%@ page import="com.dimata.aiso.entity.jurnal.*,
                 com.dimata.util.Formater,
                 com.dimata.util.Command,
                 com.dimata.harisma.entity.masterdata.PstDepartment,
                 com.dimata.harisma.entity.masterdata.Department,
                 com.dimata.aiso.entity.report.DepartmentalCost,
                 com.dimata.aiso.session.report.SessWorkSheet,
                 com.dimata.aiso.session.report.SessNeraca,
                 com.dimata.aiso.entity.periode.PstPeriode,
                 com.dimata.aiso.entity.periode.Periode,
                 com.dimata.aiso.entity.masterdata.Perkiraan,
                 com.dimata.interfaces.chartofaccount.I_ChartOfAccountGroup,
                 com.dimata.aiso.entity.masterdata.PstPerkiraan,
                 com.dimata.aiso.entity.report.ProfitLoss,
                 com.dimata.aiso.session.report.SessDepCost" %>

<!--import java-->
<%@ page import="java.util.Date" %>

<!--import qdep-->
<%@ page import="com.dimata.gui.jsp.*" %>
<%@ page import="com.dimata.qdep.form.*" %>
<%@ include file = "../main/javainit.jsp" %>
<% int  appObjCode =  AppObjInfo.composeObjCode(AppObjInfo.G1_REPORT, AppObjInfo.G2_REPORT_TRIAL_BALANCE, AppObjInfo.OBJ_REPORT_TRIAL_BALANCE_PRIV); %>
<%@ include file = "../main/checkuser.jsp" %>

<%
/* Check privilege except VIEW, view is already checked on checkuser.jsp as basic access*/
boolean privView=userSession.checkPrivilege(AppObjInfo.composeCode(appObjCode, AppObjInfo.COMMAND_VIEW));
boolean privSubmit=userSession.checkPrivilege(AppObjInfo.composeCode(appObjCode, AppObjInfo.COMMAND_SUBMIT));

//if of "hasn't access" condition
if(!privView && !privSubmit){
%>
<script language="javascript">
	window.location="<%=approot%>/nopriv.html";
</script>
<!-- if of "has access" condition -->
<%
}else{
%>

<%!
private static int FLAG_PRINT = 0;

public static String strTitleReportPdf[][] = {
	{"LAPORAN BIAYA PER DEPARTMENT","Periode","Departemen","Satuan Mata Uang"," dalam Rp.","Data tidak ditemukan. Silahkan periksa kembali data"},
	{"EXPENSES BY DEPARTMENT","Period ","Department ","Currency Unit"," in IDR Mio","Data not found. Please check data and try again."}
};
public static String strHeader[][] = {
	{"PERKIRAAN","Anggaran","Realisasi","Pencapaian Bulan Ini","Pencapaian Tahun Ini","s.d Bulan Ini","Bulan Ini","Rp","%"},
	{"ACCOUNT","Budget","Actual","MTD VAR to Budget","YTD VAR to Budget","Month-to-Date","Year-to-Date","IDR","%"}
};

public static String strTitleContent[] = {"Total Biaya Per Departemen","Total Department Expenses"};

public static final String masterTitle[] = {
	"Laporan","Report"
};

public static final String listTitle[] = {
	"Biaya per Departemen","Expenses by Department"
};

private static String formatNumber(double value){
	String strNumber = "";
		try{
				if(value < 0)
					strNumber = "("+Formater.formatNumber((value * -1), "##,###.##")+")";
				else
					strNumber = Formater.formatNumber(value, "##,###.##");
			}catch(Exception e){
				System.out.println("Exception on formatNumber ::: "+e.toString());
			}	
	return strNumber;
}

public static boolean checkDate(Date startDate, Date endDate){
	try{
		if(startDate.after(endDate)){
			return true;
		}
	}catch(Exception e){
		System.out.println("Exception on Dep_cost.checkDate() ::::: "+e.toString());
	}
	return false;
}

private static String accNameBaseOnLanguage(Perkiraan objPerkiraan, int language){
	String accName = "";
		try{
			if(language == com.dimata.util.lang.I_Language.LANGUAGE_FOREIGN){
				accName = objPerkiraan.getAccountNameEnglish();
			}else{
				accName = objPerkiraan.getNama();
			}
		}catch(Exception e){
			accName = "";
			System.out.println("Exception on accNameBaseOnLanguage() ::: "+e.toString());
		}
	return accName;
}

private static String accNameByLanguage(DepartmentalCost objDepartmentalCost, int language){
	String accName = "";
		try{
			if(language == com.dimata.util.lang.I_Language.LANGUAGE_FOREIGN){
				accName = objDepartmentalCost.getAccNameEnglish();
			}else{
				accName = objDepartmentalCost.getAccName();
			}
		}catch(Exception e){
			accName = "";
			System.out.println("Exception on accNameBaseOnLanguage() ::: "+e.toString());
		}
	return accName;
}

private static Vector listChild(DepartmentalCost objDepartmentalCost, int language, double dblValueReport, double dCurrBudget, 
double dYearBudget, double dCurrActual, double dYearActual, double totCurrBudget,  double totYearBudget, double totCurrActual, 
double totYearActual){	
	Vector vRowx = new Vector(1,1);
	Vector vRowR = new Vector(1,1);
	Vector vResult = new Vector(1,1);
	String child = "c";	
	try{
		vRowx = new Vector();
		vRowx.add("&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;"+accNameByLanguage(objDepartmentalCost, language));			
		vRowx.add("<div align=\"right\">"+formatNumber(objDepartmentalCost.getMtdBudget() / dblValueReport)+"</div>");
		vRowx.add("<div align=\"right\">"+formatNumber(objDepartmentalCost.getYtdBudget() / dblValueReport)+"</div>");
		vRowx.add("<div align=\"right\">"+formatNumber(objDepartmentalCost.getMtdActual() / dblValueReport)+"</div>");
		vRowx.add("<div align=\"right\">"+formatNumber(objDepartmentalCost.getYtdActual() / dblValueReport)+"</div>");
		vRowx.add("<div align=\"right\">"+formatNumber((objDepartmentalCost.getMtdActual() - objDepartmentalCost.getMtdBudget()) / dblValueReport)+"</div>");
		vRowx.add("<div align=\"right\">"+formatNumber((objDepartmentalCost.getMtdBudget() == 0? 0 : (objDepartmentalCost.getMtdActual() - objDepartmentalCost.getMtdBudget())/objDepartmentalCost.getMtdBudget() * 100))+"</div>");
		vRowx.add("<div align=\"right\">"+formatNumber((objDepartmentalCost.getYtdActual() - objDepartmentalCost.getYtdBudget()) / dblValueReport)+"</div>");			
		vRowx.add("<div align=\"right\">"+formatNumber((objDepartmentalCost.getYtdBudget() == 0? 0 : (objDepartmentalCost.getYtdActual() - objDepartmentalCost.getYtdBudget())/objDepartmentalCost.getYtdBudget() * 100))+"</div>");	
		
		vRowR = new Vector();
		vRowR.add("        "+accNameByLanguage(objDepartmentalCost, language));			
		vRowR.add(formatNumber(objDepartmentalCost.getMtdBudget() / dblValueReport));
		vRowR.add(formatNumber(objDepartmentalCost.getYtdBudget() / dblValueReport));
		vRowR.add(formatNumber(objDepartmentalCost.getMtdActual() / dblValueReport));
		vRowR.add(formatNumber(objDepartmentalCost.getYtdActual() / dblValueReport));
		vRowR.add(formatNumber((objDepartmentalCost.getMtdActual() - objDepartmentalCost.getMtdBudget()) / dblValueReport));
		vRowR.add(formatNumber((objDepartmentalCost.getMtdBudget() == 0? 0 : (objDepartmentalCost.getMtdActual() - objDepartmentalCost.getMtdBudget())/objDepartmentalCost.getMtdBudget() * 100)));
		vRowR.add(formatNumber((objDepartmentalCost.getYtdActual() - objDepartmentalCost.getYtdBudget()) / dblValueReport));			
		vRowR.add(formatNumber((objDepartmentalCost.getYtdBudget() == 0? 0 : (objDepartmentalCost.getYtdActual() - objDepartmentalCost.getYtdBudget())/objDepartmentalCost.getYtdBudget() * 100)));	
		vRowR.add(child);
		
		dCurrBudget += objDepartmentalCost.getMtdBudget();
		dYearBudget += objDepartmentalCost.getYtdBudget();
		dCurrActual += objDepartmentalCost.getMtdActual();
		dYearActual += objDepartmentalCost.getYtdActual();
		
		totCurrBudget += objDepartmentalCost.getMtdBudget();
		totYearBudget += objDepartmentalCost.getYtdBudget();
		totCurrActual += objDepartmentalCost.getMtdActual();
		totYearActual += objDepartmentalCost.getYtdActual();
		
	}catch(Exception e){
		vResult = new Vector(1,1);
		System.out.println("Exception on listChild() ::: "+e.toString());
	}
	vResult.add(vRowx);
	vResult.add(""+dCurrBudget);
	vResult.add(""+dYearBudget);
	vResult.add(""+dCurrActual);
	vResult.add(""+dYearActual);
	vResult.add(""+totCurrBudget);
	vResult.add(""+totYearBudget);
	vResult.add(""+totCurrActual);
	vResult.add(""+totYearActual);
	vResult.add(vRowR);
	return vResult;
}    

private static Vector subTotalChild(Perkiraan objPerkiraan, int language, double dblValueReport, double dCurrBudget, 
double dYearBudget, double dCurrActual, double dYearActual){
	Vector vResult = new Vector(1,1);	
	Vector rowx = new Vector(1,1);
	Vector rowR = new Vector(1,1);
	String parent = "p";	
		try{
			rowx = new Vector();
			rowx.add("<b>Sub Total&nbsp;"+objPerkiraan.getNoPerkiraan()+"&nbsp;"+accNameBaseOnLanguage(objPerkiraan, language)+"</b>");
			rowx.add("<b><div align=\"right\">"+formatNumber(dCurrBudget / dblValueReport)+"</div></b>");
			rowx.add("<b><div align=\"right\">"+formatNumber(dYearBudget / dblValueReport)+"</div></b>");
			rowx.add("<b><div align=\"right\">"+formatNumber(dCurrActual / dblValueReport)+"</div></b>");
			rowx.add("<b><div align=\"right\">"+formatNumber(dYearActual / dblValueReport)+"</div></b>");
			rowx.add("<b><div align=\"right\">"+formatNumber((dCurrActual - dCurrBudget) / dblValueReport)+"</div></b>");
			rowx.add("<b><div align=\"right\">"+formatNumber((dCurrBudget == 0? 0 : (dCurrActual - dCurrBudget) / dCurrBudget * 100))+"</div></b>");
			rowx.add("<b><div align=\"right\">"+formatNumber((dYearActual - dYearBudget) / dblValueReport)+"</div>");			
			rowx.add("<b><div align=\"right\">"+formatNumber((dYearBudget == 0? 0 : (dYearActual - dYearBudget)/dYearBudget * 100))+"</div></b>");
			
			rowR = new Vector();
			rowR.add("Sub Total "+objPerkiraan.getNoPerkiraan()+" "+accNameBaseOnLanguage(objPerkiraan, language));
			rowR.add(formatNumber(dCurrBudget / dblValueReport));
			rowR.add(formatNumber(dYearBudget / dblValueReport));
			rowR.add(formatNumber(dCurrActual / dblValueReport));
			rowR.add(formatNumber(dYearActual / dblValueReport));
			rowR.add(formatNumber((dCurrActual - dCurrBudget) / dblValueReport));
			rowR.add(formatNumber((dCurrBudget == 0? 0 : (dCurrActual - dCurrBudget) / dCurrBudget * 100)));
			rowR.add(formatNumber((dYearActual - dYearBudget) / dblValueReport));			
			rowR.add(formatNumber((dYearBudget == 0? 0 : (dYearActual - dYearBudget)/dYearBudget * 100)));
			rowR.add(parent);
			
		}catch(Exception e){
			vResult = new Vector(1,1);
			System.out.println("Exception on subTotalChild() ::: "+e.toString());
		}
	vResult.add(rowx);
	vResult.add(rowR);	
	return vResult;
}

private static Vector totExpense(int language, double totPLCurrBudget,  double totPLYearBudget, double totPLCurrActual, 
double totPLYearActual, double dblValueReport){
	Vector vResult = new Vector(1,1);
	Vector rowx = new Vector(1,1);
	Vector rowR = new Vector(1,1);		
		try{
			rowx = new Vector();
			rowx.add("<b><font size=\"3\">"+strTitleContent[language]+"</font></b>");
			rowx.add("<div align=\"right\"><b><font size=\"2\">"+formatNumber(totPLCurrBudget / dblValueReport)+"</font></b></div>");
			rowx.add("<div align=\"right\"><b><font size=\"2\">"+formatNumber(totPLYearBudget / dblValueReport)+"</font></b></div>");
			rowx.add("<div align=\"right\"><b><font size=\"2\">"+formatNumber(totPLCurrActual / dblValueReport)+"</font></b></div>");
			rowx.add("<div align=\"right\"><b><font size=\"2\">"+formatNumber(totPLYearActual / dblValueReport)+"</font></b></div>");
			rowx.add("<div align=\"right\"><b><font size=\"2\">"+formatNumber((totPLCurrActual - totPLCurrBudget) / dblValueReport)+"</font></b></div>");
			rowx.add("<div align=\"right\"><b><font size=\"2\">"+formatNumber(totPLCurrBudget == 0? 0 : ((totPLCurrActual - totPLCurrBudget)/totPLCurrBudget) * 100)+"</font></b></div>");
			rowx.add("<div align=\"right\"><b><font size=\"2\">"+formatNumber((totPLYearActual - totPLYearBudget) / dblValueReport)+"</font></b></div>");
			rowx.add("<div align=\"right\"><b><font size=\"2\">"+formatNumber(totPLYearBudget == 0? 0 : ((totPLYearActual - totPLYearBudget)/totPLYearBudget) * 100)+"</font></b></div>");
			
			rowR = new Vector();
			rowR.add(strTitleContent[language]);
			rowR.add(formatNumber(totPLCurrBudget / dblValueReport));
			rowR.add(formatNumber(totPLYearBudget / dblValueReport));
			rowR.add(formatNumber(totPLCurrActual / dblValueReport));
			rowR.add(formatNumber(totPLYearActual / dblValueReport));
			rowR.add(formatNumber((totPLCurrActual - totPLCurrBudget) / dblValueReport));
			rowR.add(formatNumber(totPLCurrBudget == 0? 0 : ((totPLCurrActual - totPLCurrBudget)/totPLCurrBudget) * 100));
			rowR.add(formatNumber((totPLYearActual - totPLYearBudget) / dblValueReport));
			rowR.add(formatNumber(totPLYearBudget == 0? 0 : ((totPLYearActual - totPLYearBudget)/totPLYearBudget) * 100));
        	
		}catch(Exception e){
			vResult = new Vector(1,1);
			System.out.println("Exception on totProfitLoss() ::: "+e.toString());
		}
	vResult.add(rowx);
	vResult.add(rowR);		
	return vResult;
}

private static Vector listParent(Perkiraan objPerkiraan, int language){
	Vector vResult = new Vector(1,1);
	Vector rowx = new Vector(1,1);
	Vector rowR = new Vector(1,1);
	String parent = "p";
	try{
		rowx = new Vector();
		rowx.add("<b>"+objPerkiraan.getNoPerkiraan()+"&nbsp;"+accNameBaseOnLanguage(objPerkiraan, language)+"</b>");
		rowx.add("");
		rowx.add("");
		rowx.add("");
		rowx.add("");
		rowx.add("");
		rowx.add("");
		rowx.add("");
		rowx.add("");
		
		rowR = new Vector();
		rowR.add(objPerkiraan.getNoPerkiraan()+" "+accNameBaseOnLanguage(objPerkiraan, language));
		rowR.add("");
		rowR.add("");
		rowR.add("");
		rowR.add("");
		rowR.add("");
		rowR.add("");
		rowR.add("");
		rowR.add("");
		rowR.add(parent);		
					
	}catch(Exception e){
		vResult = new Vector(1,1);
		System.out.println("Exception on listParent() ::: "+e.toString());
	}
	vResult.add(rowx);
	vResult.add(rowR);
	return vResult;
}

private static Vector space(){
	Vector vResult = new Vector(1,1);
	Vector rowx = new Vector(1,1);
	Vector rowR = new Vector(1,1);
	String child = "c";
		try{
			rowx = new Vector();
			rowx.add("&nbsp;");
			rowx.add("&nbsp;");
			rowx.add("&nbsp;");
			rowx.add("&nbsp;");
			rowx.add("&nbsp;");
			rowx.add("&nbsp;");
			rowx.add("&nbsp;");
			rowx.add("&nbsp;");
			rowx.add("&nbsp;");
			
			rowR = new Vector();
			rowR.add(" ");
			rowR.add(" ");
			rowR.add(" ");
			rowR.add(" ");
			rowR.add(" ");
			rowR.add(" ");
			rowR.add(" ");
			rowR.add(" ");
			rowR.add(" ");
			rowR.add(child);
			
		}catch(Exception e){
			vResult = new Vector(1,1);
			System.out.println("Exception on listTitleContent() ::: "+e.toString());
		}
	vResult.add(rowx);	
	vResult.add(rowR);	
	return vResult;
}

public static Vector listDepartmentalCost(Vector objClass, int language, double dblValueReport){
	Vector result = new Vector(1,1);
    ControlList ctrlist = new ControlList();
    ctrlist.setAreaWidth("100%");
    ctrlist.setListStyle("listgen");
    ctrlist.setTitleStyle("listgentitle");
    ctrlist.setCellStyle("listgensell");
	ctrlist.setCellStyleOdd("listgensellOdd");
    ctrlist.setHeaderStyle("listgentitle");	
	
    ctrlist.addHeader(strHeader[language][0],"30%","2","0");
    ctrlist.addHeader(strHeader[language][1],"10%","0","2");
    ctrlist.addHeader(strHeader[language][5],"10%","0","0");
    ctrlist.addHeader(strHeader[language][6],"10%","0","0");
	ctrlist.addHeader(strHeader[language][2],"10%","0","2");
    ctrlist.addHeader(strHeader[language][5],"10%","0","0");
    ctrlist.addHeader(strHeader[language][6],"10%","0","0");
	ctrlist.addHeader(strHeader[language][3],"10%","0","2");
    ctrlist.addHeader(strHeader[language][7],"10%","0","0");
    ctrlist.addHeader(strHeader[language][8],"5%","0","0");
	ctrlist.addHeader(strHeader[language][4],"10%","0","2");
    ctrlist.addHeader(strHeader[language][7],"10%","0","0");
    ctrlist.addHeader(strHeader[language][8],"5%","0","0"); 

    Vector lstData = ctrlist.getData();
    Vector lstLinkData = ctrlist.getLinkData();

	// For pdf and xls report
	Vector vectDataToReport = new Vector();
	Vector detailToReport = new Vector(1,1);
	
	int iExistingData = 0; 
	double totGroupCurrBudget = 0.0;
	double totGroupYearBudget = 0.0;
	double totGroupCurrActual = 0.0; 
	double totGroupYearActual = 0.0;
	
	double dCurrBudget = 0.0;
	double dYearBudget = 0.0;
	double dCurrActual = 0.0;
	double dYearActual = 0.0;
	double totCurrBudget = 0.0;  
	double totYearBudget = 0.0;
	double totCurrActual = 0.0;
	double totYearActual = 0.0;
	
	Vector rowx = new Vector();
	Vector rowR = new Vector();
	long oidOldParent = 0;
	Perkiraan oldObjParentPerkiraan = new Perkiraan();
	Perkiraan objParentPerkiraan = new Perkiraan();
	
	DepartmentalCost objOldDepartmentalCost = new DepartmentalCost();
	
	try{
		if(objClass != null && objClass.size() > 0){	
			for(int i = 0; i < objClass.size(); i++){
					DepartmentalCost objDepartmentalCost = (DepartmentalCost)objClass.get(i);
					long idParent = objDepartmentalCost.getIdParent();
					try{
						objParentPerkiraan = PstPerkiraan.fetchExc(idParent);
					}catch(Exception e){
						objParentPerkiraan = new Perkiraan();
						System.out.println("Exception on fetch objParentPerkiraan ::: "+e.toString());
					}//End try catch
					if(i > 0){
						objOldDepartmentalCost = (DepartmentalCost)objClass.get(i - 1);
						oidOldParent = objOldDepartmentalCost.getIdParent();
						
						if(idParent != oidOldParent){
							try{
								oldObjParentPerkiraan = PstPerkiraan.fetchExc(oidOldParent);
							}catch(Exception e){
								oldObjParentPerkiraan = new Perkiraan();
								System.out.println("Exception on fetch oldObjParentPerkiraan ::: "+e.toString());
							}//End try catch
							
							Vector subTotalChild = (Vector)subTotalChild(oldObjParentPerkiraan, language, dblValueReport, dCurrBudget, 
													dYearBudget, dCurrActual, dYearActual);
							if(subTotalChild != null && subTotalChild.size() > 0){
								rowx = (Vector)subTotalChild.get(0);
								lstData.add(rowx);
								
								rowR = (Vector)subTotalChild.get(1);
								detailToReport.add(rowR); //Index 0
							}//End if(subTotalChild != null && subTotalChild.size() > 0)
							
							dCurrBudget = 0.0;
							dYearBudget = 0.0;
							dCurrActual = 0.0;
							dYearActual = 0.0;
							
							Vector vTempSpaceParent = (Vector)space();
									rowx = (Vector)vTempSpaceParent.get(0);
									rowR = (Vector)vTempSpaceParent.get(1);
									lstData.add(rowx);	
									detailToReport.add(rowR); //Index 0
										
							Vector vParent = (Vector)listParent(objParentPerkiraan, language);
							if(vParent != null && vParent.size() > 0){
								rowx = (Vector)vParent.get(0);
								lstData.add(rowx);
								
								rowR = (Vector)vParent.get(1);
								detailToReport.add(rowR);	//Index 1
							}//End if(vParent != null && vParent.size() > 0)							
							
							Vector vChild = (Vector)listChild(objDepartmentalCost, language, dblValueReport, dCurrBudget, dYearBudget, 
											dCurrActual, dYearActual, totCurrBudget,  totYearBudget, totCurrActual,totYearActual);
							
							if(vChild != null && vChild.size() > 0){
								rowx = (Vector)vChild.get(0);
								lstData.add(rowx);
								dCurrBudget = Double.parseDouble(vChild.get(1).toString());
								dYearBudget = Double.parseDouble(vChild.get(2).toString());
								dCurrActual = Double.parseDouble(vChild.get(3).toString());
								dYearActual = Double.parseDouble(vChild.get(4).toString());
								totCurrBudget = Double.parseDouble(vChild.get(5).toString());  
								totYearBudget = Double.parseDouble(vChild.get(6).toString());
								totCurrActual = Double.parseDouble(vChild.get(7).toString());
								totYearActual = Double.parseDouble(vChild.get(8).toString());
								rowR = (Vector)vChild.get(9);//Index 2
								detailToReport.add(rowR);						
							} // if(vChild != null && vChild.size() > 0)
														
						}else{//if(idParent != oidOldParent)
							Vector vChild = (Vector)listChild(objDepartmentalCost, language, dblValueReport, dCurrBudget, dYearBudget, 
											dCurrActual, dYearActual, totCurrBudget,  totYearBudget, totCurrActual,totYearActual);
							
							if(vChild != null && vChild.size() > 0){
								rowx = (Vector)vChild.get(0);
								lstData.add(rowx);
								dCurrBudget = Double.parseDouble(vChild.get(1).toString());
								dYearBudget = Double.parseDouble(vChild.get(2).toString());
								dCurrActual = Double.parseDouble(vChild.get(3).toString());
								dYearActual = Double.parseDouble(vChild.get(4).toString());
								totCurrBudget = Double.parseDouble(vChild.get(5).toString());  
								totYearBudget = Double.parseDouble(vChild.get(6).toString());
								totCurrActual = Double.parseDouble(vChild.get(7).toString());
								totYearActual = Double.parseDouble(vChild.get(8).toString());
								rowR = (Vector)vChild.get(9);
								detailToReport.add(rowR);							
							}//End if(vChild != null && vChild.size() > 0)
						}// End if(idParent != oidOldParent)
						
						if(i == (objClass.size() - 1)){						
							Vector subTotalChild = (Vector)subTotalChild(objParentPerkiraan, language, dblValueReport, dCurrBudget, 
													dYearBudget, dCurrActual, dYearActual);
							if(subTotalChild != null && subTotalChild.size() > 0){
								rowx = (Vector)subTotalChild.get(0);
								lstData.add(rowx);
								
								rowR = (Vector)subTotalChild.get(1);
								detailToReport.add(rowR);	
							}// End if(subTotalChild != null && subTotalChild.size() > 0)
						}// End if(i == (objClass.size() - 1))			
					}else{// if(i > 0)											
						Vector vParent = (Vector)listParent(objParentPerkiraan, language);
						if(vParent != null && vParent.size() > 0){
							rowx = (Vector)vParent.get(0);
							lstData.add(rowx);
							
							rowR = (Vector)vParent.get(1);
							detailToReport.add(rowR);	
						}// End if(vParent != null && vParent.size() > 0)					
							
						if(objClass.size() == 1){
										Vector vChild = (Vector)listChild(objDepartmentalCost, language, dblValueReport, dCurrBudget, dYearBudget, 
														dCurrActual, dYearActual, totCurrBudget,  totYearBudget, totCurrActual,totYearActual);
										
										if(vChild != null && vChild.size() > 0){
											rowx = (Vector)vChild.get(0);
											lstData.add(rowx);
											dCurrBudget = Double.parseDouble(vChild.get(1).toString());
											dYearBudget = Double.parseDouble(vChild.get(2).toString());
											dCurrActual = Double.parseDouble(vChild.get(3).toString());
											dYearActual = Double.parseDouble(vChild.get(4).toString());
											totCurrBudget = Double.parseDouble(vChild.get(5).toString());  
											totYearBudget = Double.parseDouble(vChild.get(6).toString());
											totCurrActual = Double.parseDouble(vChild.get(7).toString());
											totYearActual = Double.parseDouble(vChild.get(8).toString());
											
											rowR = (Vector)vChild.get(9);
											detailToReport.add(rowR);								
										}//End if(vChild != null && vChild.size() > 0)
							
									Vector subTotalChild = (Vector)subTotalChild(objParentPerkiraan, language, dblValueReport, dCurrBudget, 
															dYearBudget, dCurrActual, dYearActual);
									if(subTotalChild != null && subTotalChild.size() > 0){
										rowx = (Vector)subTotalChild.get(0);
										lstData.add(rowx);
										
										rowR = (Vector)subTotalChild.get(1);
										detailToReport.add(rowR);		
									}//End if(subTotalChild != null && subTotalChild.size() > 0)
						}else{// if(objClass.size() == 1){ 
							Vector vChildFirstIndex = (Vector)listChild(objDepartmentalCost, language, dblValueReport, dCurrBudget, dYearBudget, 
											dCurrActual, dYearActual, totCurrBudget,  totYearBudget, totCurrActual,totYearActual);
							
							if(vChildFirstIndex != null && vChildFirstIndex.size() > 0){
								rowx = (Vector)vChildFirstIndex.get(0);
								lstData.add(rowx);
								dCurrBudget = Double.parseDouble(vChildFirstIndex.get(1).toString());
								dYearBudget = Double.parseDouble(vChildFirstIndex.get(2).toString());
								dCurrActual = Double.parseDouble(vChildFirstIndex.get(3).toString());
								dYearActual = Double.parseDouble(vChildFirstIndex.get(4).toString());
								totCurrBudget = Double.parseDouble(vChildFirstIndex.get(5).toString());  
								totYearBudget = Double.parseDouble(vChildFirstIndex.get(6).toString());
								totCurrActual = Double.parseDouble(vChildFirstIndex.get(7).toString());
								totYearActual = Double.parseDouble(vChildFirstIndex.get(8).toString());	
								
								rowR = (Vector)vChildFirstIndex.get(9);
								detailToReport.add(rowR);						
							}// End if(vChildFirstIndex != null && vChildFirstIndex.size() > 0)
						}// End if(objClass.size() == 1){ 						
					}// End if(i > 0)						
			}// End looping	
			
			vectDataToReport.add(detailToReport);
			//------------------------------------------- Total Expenses ---------------------------------------
			Vector vExpenses = (Vector)totExpense(language, totCurrBudget, totYearBudget, totCurrActual,totYearActual, dblValueReport);
			rowR = (Vector)vExpenses.get(1);
			vectDataToReport.add(rowR); // Index ke 14
			if(vExpenses != null && vExpenses.size() > 0){
				rowx = (Vector)vExpenses.get(0);
				lstData.add(rowx);
			}
		
		}
		FLAG_PRINT = 1; 
		if(totCurrActual != 0 || totCurrActual != 0)
			iExistingData = 1;
	}catch(Exception e){
		result = new Vector(1,1);
	}
	result.add(ctrlist.drawList());	
	result.add(vectDataToReport);
	result.add(""+iExistingData);
	return result;
}


%>

<!-- JSP Block -->
<%

    int iCommand = FRMQueryString.requestCommand(request);
    long periodId = FRMQueryString.requestLong(request,SessWorkSheet.FRM_NAMA_PERIODE);
	long endPeriodId = FRMQueryString.requestLong(request,"END_PERIOD");	
    long oidDepartment = FRMQueryString.requestLong(request,"DEPARTMENT");
	int iIndex = FRMQueryString.requestInt(request,"index");
    String strCmbOption[] = {"- Silahkan pilih -", "- Please select -"};	
	String strCmdPrint[] = {"Cetak Laporan", "Print Report"};
	String strCmdExport[] = {"Ekspor ke Excel", "Export To Excel"};
    String strCmbFirstSelection = strCmbOption[SESS_LANGUAGE];
    Date today = new Date();
	String linkPdf = reportrootfooter+"report.labaRugi.ProfitLossDeptPdf printed on "+Formater.formatDate(today,"EEEE, dd MMMM yyyy, hh:mm a");
	String linkXls = reportrootfooter+"report.labaRugi.ProfitLossDeptXls printed on "+Formater.formatDate(today,"EEEE, dd MMMM yyyy, hh:mm a");
	String pathImage = reportrootimage+"company121.jpg";
	FRMHandler frmHandler = new FRMHandler();
	frmHandler.setDecimalSeparator(sUserDecimalSymbol); 
	frmHandler.setDigitSeparator(sUserDigitGroup);
	
	
    Vector list = new Vector();
	String fromPeriodName = "";
	String toPeriodName = "";
    String namedepartment = "";
	Date startDatePeriod = new Date();
	Date endDatePeriod = new Date();
	
    if(iCommand==Command.LIST)
	    list = SessDepCost.listDepartmentalCost(periodId,endPeriodId,oidDepartment);
	boolean bcheckDate = false;
	try{
            /* From Period */
			Periode period = PstPeriode.fetchExc(periodId);
            fromPeriodName = period.getNama();
			startDatePeriod = period.getTglAwal();
			namedepartment = PstDepartment.fetchExc(oidDepartment).getDepartment();
			
			/* To Period */
			Periode objToPeriod = PstPeriode.fetchExc(endPeriodId);
			endDatePeriod = objToPeriod.getTglAwal();
            toPeriodName = objToPeriod.getNama();
			bcheckDate = checkDate(startDatePeriod,endDatePeriod);			
        }catch(Exception e){
			Periode period = new Periode();
			System.out.println("Exception on fetch objPeriod ::: "+e.toString());
		}
	
	
	// process data on control drawlist
	String sExistData = "";
	int iExistData = 0;
	Vector vectResult = new Vector();
	String listData = "";
	Vector vectDataToReport = new Vector();
	if(list != null && list.size() > 0){
		try{
			vectResult = listDepartmentalCost(list, SESS_LANGUAGE, dValueReport);
			if(vectResult != null && vectResult.size() > 0){
				listData = String.valueOf(vectResult.get(0));     
	    		vectDataToReport = (Vector)vectResult.get(1);
				sExistData = vectResult.get(2).toString();
				iExistData = Integer.parseInt(sExistData);
			}
		}catch(Exception e){
			System.out.println("Exception on getListDepartmentalCost :::: "+e.toString());
		}
	}
	
	Vector vecHeader = new Vector(1,1);
	for(int i=0; i < strHeader[SESS_LANGUAGE].length; i++){
		vecHeader.add(strHeader[SESS_LANGUAGE][i]);
	}
	
	Vector vecTitle = new Vector(1,1);
	for(int t=0; t < strTitleReportPdf[SESS_LANGUAGE].length; t++){
		vecTitle.add(strTitleReportPdf[SESS_LANGUAGE][t]);
	}
	
	Vector vecSess = new Vector();
	vecSess.add(fromPeriodName);// Index 0 
	vecSess.add(namedepartment);// Index 1
	vecSess.add(linkPdf);// Index 2
	vecSess.add(linkXls);// Index 3	
	vecSess.add(vectDataToReport);// Index 4
	vecSess.add(vecHeader);// Index 5
	vecSess.add(vecTitle);// Index 6
	vecSess.add(pathImage);// Index 7
	vecSess.add(toPeriodName);// Index 8
	
	if(session.getValue("DEPARTMENTAL_COST")!=null){   
		session.removeValue("DEPARTMENTAL_COST");
	}
	session.putValue("DEPARTMENTAL_COST",vecSess);
	
	long ndPeriodId = endPeriodId;
	Vector vEndPeriodKey = new Vector(1,1);
	Vector vEndPeriodVal = new Vector(1,1);

/**
* Declare Commands caption
*/
String strReport = SESS_LANGUAGE==com.dimata.util.lang.I_Language.LANGUAGE_FOREIGN ? "Preview Report" : "Tampilkan Laporan";
String strStartPeriod = SESS_LANGUAGE==com.dimata.util.lang.I_Language.LANGUAGE_FOREIGN ? "From" : "Dari";
String strEndPeriod = SESS_LANGUAGE==com.dimata.util.lang.I_Language.LANGUAGE_FOREIGN ? "To" : "Sampai";
String strBlankData = SESS_LANGUAGE==com.dimata.util.lang.I_Language.LANGUAGE_FOREIGN ? "Data not found" : "Data tidak ditemukan";
String strChkDep = SESS_LANGUAGE==com.dimata.util.lang.I_Language.LANGUAGE_FOREIGN ? "Department not select" : "Departemen belum dipilih";
String strErrorPeriod[] = {"Periode awal mesti lebih kecil dari periode akhir","Start period after end period"};
String strBack[] = {"Kembali ke Pencarian","Back to Search"};
%>
<!-- End of JSP Block -->
<html><!-- #BeginTemplate "/Templates/main-menu-left-frames.dwt" --><!-- DW6 -->
<head>
<!-- #BeginEditable "doctitle" -->
<title>Accounting Information System Online</title>
<script language="javascript">
function report(){
	var departmentID = Math.abs(document.frmTrialBalance.DEPARTMENT.value);   
	document.frmTrialBalance.command.value ="<%=Command.LIST%>";
	document.frmTrialBalance.action = "dep_cost.jsp";
	<%if(bcheckDate){%>
		alert("Erorr. \n<%=strErrorPeriod[SESS_LANGUAGE]%>");
		
	<%}else{%>
		if(departmentID == 0){
			alert("Error. \n<%=strChkDep%>");
		}else{	
			document.frmTrialBalance.submit();
		}
	<%}%>
}

function cmdRefress(){
	 document.frmTrialBalance.command.value ="<%=Command.REFRESH%>";
	 document.frmTrialBalance.submit();
}

function cmdBack(){
	 document.frmTrialBalance.command.value ="<%=Command.BACK%>";
     document.frmTrialBalance.action = "dep_cost.jsp#up";
	 document.frmTrialBalance.submit();
}

function reportPdf(){	 
		var linkPage = "<%=reportroot%>report.departmentalCost.DepartmentalCostPdf";       
		window.open(linkPage);  				
}

function printXLS(){	 
		var linkPage = "<%=reportroot%>report.departmentalCost.DepartmentalCostXls";       
		window.open(linkPage);  				
}

function changePeriod(){
	var periodId = Math.abs(document.frmTrialBalance.<%=SessWorkSheet.FRM_NAMA_PERIODE%>.value);
	var endPeriodId =  Math.abs(document.frmTrialBalance.END_PERIOD.value);
	
	/*for(var a=document.frmTrialBalance.END_PERIOD.length-1; a>-1; a--){
				document.frmTrialBalance.END_PERIOD.options.remove(a);
			 	}*/
		<%
			
			Vector vPeriod = (Vector)PstPeriode.list(0,0,"","");
			if(vPeriod != null && vPeriod.size() > 0){
				for(int i = 0; i < vPeriod.size(); i++){
					Periode objPeriode = (Periode)vPeriod.get(i);
					Periode nextObjPeriode = new Periode();
					
					//if(nextObjPeriode.getOID() != 0){
			%>
			/*var option = document.createElement("OPTION");
			option.value = "<%=objPeriode.getOID()%>";
			option.text = "<%=objPeriode.getNama()%>";							
			document.frmTrialBalance.END_PERIOD.add(option);*/
					
				/*if(periodId > endPeriodId){	
					<%
					if(i < (vPeriod.size() - 1)){%>
						<%nextObjPeriode = (Periode)vPeriod.get(i+1);
						
					}
					%>
					if(endPeriodId == 0){
						<%if(iCommand == Command.LIST){%>
							<%if(objPeriode.getOID() != endPeriodId){%>
								<%if(nextObjPeriode.getOID() != 0){%>
								var nOption = document.createElement("OPTION");
								nOption.value = "<%=nextObjPeriode.getOID()%>";
								nOption.text = "<%=nextObjPeriode.getNama()%>";
								document.frmTrialBalance.END_PERIOD.add(nOption);
								<%}%>
							<%}else{%>
								var option = document.createElement("OPTION");
								option.value = "<%=objPeriode.getOID()%>";
								option.text = "<%=objPeriode.getNama()%>";							
								document.frmTrialBalance.END_PERIOD.add(option);
							<%}%>
						<%}else{%>
							var option = document.createElement("OPTION");
							option.value = "<%=objPeriode.getOID()%>";
							option.text = "<%=objPeriode.getNama()%>";							
							document.frmTrialBalance.END_PERIOD.add(option);
						<%}%>
					}else{
					
					<%if(nextObjPeriode.getOID() != 0){%>
					var nOption = document.createElement("OPTION");
					nOption.value = "<%=nextObjPeriode.getOID()%>";
					nOption.text = "<%=nextObjPeriode.getNama()%>";
			 		document.frmTrialBalance.END_PERIOD.add(nOption);
					<%}%>
					}
				}else{
				
					if(periodId == endPeriodId){
						<%if(nextObjPeriode.getOID() != 0){%>
						var nOption = document.createElement("OPTION");
						nOption.value = "<%=nextObjPeriode.getOID()%>";
						nOption.text = "<%=nextObjPeriode.getNama()%>";
						document.frmTrialBalance.END_PERIOD.add(nOption);
						<%}%>
					}else{
					var option = document.createElement("OPTION");
					option.value = "<%=objPeriode.getOID()%>";
					option.text = "<%=objPeriode.getNama()%>";
					
					document.frmTrialBalance.END_PERIOD.add(option);
					}
				}*/
				<%//}
				}
			}
		%>
}

</script>
<!-- #EndEditable -->
<!-- #BeginEditable "headerscript" -->
<SCRIPT language=JavaScript>

/*function hideObjectForMenuJournal(){
	}

function hideObjectForMenuReport(){
	}

function hideObjectForMenuPeriod(){
	}

function hideObjectForMenuMasterData(){
	}

function hideObjectForMenuSystem(){
	}

//*****************
function showObjectForMenu(){

}*/



</SCRIPT>
<!-- #EndEditable -->
<meta http-equiv="Content-Type" content="text/html; charset=iso-8859-1">
<link rel="stylesheet" href="../style/main.css" type="text/css">
<link rel="StyleSheet" href="../dtree/dtree.css" type="text/css" />
	<script type="text/javascript" src="../dtree/dtree.js"></script>
</head> 

<body class="bodystyle" leftmargin="0" topmargin="0" marginwidth="0" marginheight="0">    
<table width="100%" border="0" cellspacing="1" cellpadding="1" height="100%">
  <tr> 
    <td width="91%" valign="top" align="left" bgcolor="#99CCCC"> 
      <table width="100%" border="0" cellspacing="0" cellpadding="0" bgcolor="#FFFFFF">
        <tr> 
          <td height="20" class="contenttitle" >&nbsp;<!-- #BeginEditable "contenttitle" -->
            <%=masterTitle[SESS_LANGUAGE]%> : <font color="#CC3300"><%=listTitle[SESS_LANGUAGE].toUpperCase()%></font><!-- #EndEditable --></td>
        </tr>
        <tr> 
          <td valign="top"><!-- #BeginEditable "content" -->
            <form name="frmTrialBalance" method="post" action="">
            <input type="hidden" name="command" value="<%=iCommand%>">
			<input type="hidden" name="index" value="<%=iIndex%>">
              <table width="100%" cellspacing="0" cellpadding="0">
                <tr>
                  <td colspan="2">&nbsp;
                  </td>
                </tr>
              </table>
			  <%if(privView && privSubmit){%>
              <table width="100%" border="0" height="94">
                <tr>
                  <td><!-- Start Search -->
				  	<table width="100%">
						<tr>
						  <td width="7%" height="23" nowrap><%=strTitleReportPdf[SESS_LANGUAGE][1]%> <%=strStartPeriod%></td>
                  <td height="23" width="1%">
                    <div align="center">:</div>
                  </td>
                  <td height="23" width="7%">
                      <div align="left">
                          <%
						  String strOrderBy = PstPeriode.fieldNames[PstPeriode.FLD_TGLAWAL]+" DESC";
                      Vector vtPeriod = PstPeriode.list(0,0,"",strOrderBy);
                      Vector vPeriodKey = new Vector(1,1);
                      Vector vPeriodVal = new Vector(1,1);
                      for(int k=0;k<vtPeriod.size();k++){
                        Periode periode = (Periode)vtPeriod.get(k);

                        vPeriodKey.add(""+periode.getOID());
                        vPeriodVal.add(periode.getNama());
                      }
                      out.println(ControlCombo.draw(SessWorkSheet.FRM_NAMA_PERIODE,null,""+periodId,vPeriodKey,vPeriodVal,"onChange=\"javascript:cmdRefress()\""));
				  %>
                    </div></td><td width="10%"><div align="center"><%=strEndPeriod%></div></td>
                  <td width="13%">
                      <div align="left">
                          <%
						  String sOrderBy = PstPeriode.fieldNames[PstPeriode.FLD_TGLAWAL]+" DESC";
                      Vector vtEndPeriod = PstPeriode.list(0,0,"",sOrderBy);
                      vEndPeriodKey = new Vector(1,1);
                      vEndPeriodVal = new Vector(1,1);
					  Periode periodeEnd = new Periode();
					  
                      for(int l=0;l<vtEndPeriod.size();l++){
                        periodeEnd = (Periode)vtEndPeriod.get(l);

                        vEndPeriodKey.add(""+periodeEnd.getOID());
                        vEndPeriodVal.add(periodeEnd.getNama());
                      }
                      out.println(ControlCombo.draw("END_PERIOD","",null,""+endPeriodId,vEndPeriodKey,vEndPeriodVal,"onChange=\"javascript:cmdRefress()\""));
				  %>
                    </div></td>
					<td width="62%">&nbsp;
					</td>
						</tr>
                <tr>
                  <td width="7%" height="23"><%=strTitleReportPdf[SESS_LANGUAGE][2]%></td>
                  <td height="23" width="1%">
                  <div align="center">:</div>
                  </td>
                  <td height="23" colspan="4"><%
                  Vector vectDept = new Vector();
                  Vector vUsrCustomDepartment = PstDataCustom.getDataCustom(userOID, "hrdepartment");
                  if(vUsrCustomDepartment!=null && vUsrCustomDepartment.size()>0){
                  int iDataCustomCount = vUsrCustomDepartment.size();
                    for(int i=0; i<iDataCustomCount; i++){
                        DataCustom objDataCustom = (DataCustom) vUsrCustomDepartment.get(i);
                        Department dept = new Department();
                        try{
                            dept = PstDepartment.fetchExc(Long.parseLong(objDataCustom.getDataValue()));
                        }catch(Exception e){
                            System.out.println("Err >> department : "+e.toString());
                        }
                        vectDept.add(dept);
                    }
                  }

                  Vector vectDeptKey = new Vector();
                  Vector vectDeptName = new Vector();
                  vectDeptKey.add("0");
                  vectDeptName.add(strCmbFirstSelection);
                  int iDeptSize = vectDept.size();
                  for (int deptNum = 0; deptNum < iDeptSize; deptNum++)
                  {
                      Department dept = (Department) vectDept.get(deptNum);
                      vectDeptKey.add(String.valueOf(dept.getOID()));
                      vectDeptName.add(dept.getDepartment());
                  }
                  String strSelectedDept = String.valueOf(oidDepartment);
                %>
                <%=ControlCombo.draw("DEPARTMENT", "", null, strSelectedDept, vectDeptKey, vectDeptName,"onChange=\"javascript:cmdRefress()\"")%>
                    </td>
                </tr>
                <tr>
                  <td height="23">&nbsp;</td>
                  <td height="23">&nbsp;</td>
                  <td height="23" colspan="4">&nbsp;</td>
                </tr>
                <tr>
                  <td width="7%" height="23">&nbsp;</td>
                  <td height="23" width="1%">
                    <div align="center"><b></b></div>
                  </td>
                  <td height="23" colspan="4"></span></a>
                  <input name="btnViewReport" type="button" onClick="javascript:report()" value="<%=strReport%>"></td>
						</tr>
					</table>
				  </td><!-- End Search -->
                </tr>
                <tr>
                  <td colspan="3" height="2">&nbsp;</td>
                </tr>
				
                <%if(iCommand==Command.LIST && iExistData > 0){%>
				<tr>
                  <td colspan="3" height="2"><b>&nbsp;&nbsp;<%=masterTitle[SESS_LANGUAGE]%> : <%=strTitleReportPdf[SESS_LANGUAGE][4]%></b></td>
                </tr>
                <tr>
					<%if(list != null && list.size() > 0){%>	
                  <td colspan="2" height="2"><%=listData%></td>
				  <%}else{%>				
                </tr>
                <tr>
                  <td colspan="3" class="msgerror"><%=strBlankData%></td>
				  <%}%>
                </tr>
						<%if((FLAG_PRINT == 1) && (list != null && list.size() > 0)){%>
						<tr>
							<td >
								<table width="18%" border="0" cellspacing="1" cellpadding="1">
								<tr>
									<td width="83%" nowrap><b><input name="btnPrint" type="button" onClick="javascript:reportPdf()" value="<%=strCmdPrint[SESS_LANGUAGE]%>"></td>
									<td width="83%" nowrap><input name="btnExportToXls" type="button" onClick="javascript:printXLS()" value="<%=strCmdExport[SESS_LANGUAGE]%>"></td>
									<td width="83%" nowrap><input name="btnBack" type="button" onClick="javascript:cmdBack()" value="<%=strBack[SESS_LANGUAGE]%>"></td>
								</tr>
							  </table>
							</td>
						</tr>
						<%}%>
              </table>
			  <%}else{%>
			   	<%if(iCommand == Command.LIST){%>
			   	<table width="100%">
					<tr><td class="msgerror"><%=strTitleReportPdf[SESS_LANGUAGE][5]%></td></tr>
				</table>
				<%}%>
			   <%}%>
			  <%}else{%>
              <table width="100%" cellspacing="0" cellpadding="0">
                <tr>
                  <td colspan="2"><font color="#FF0000"><i><%//=strTitle[SESS_LANGUAGE][2]%></i></font></td>
                </tr>
              </table>
			  <%}%>
			  <script language="javascript">
					  		changePeriod();
					  </script>		
            </form>
            <!-- #EndEditable --></td>
        </tr>
        <tr> 
          <td height="100%"> 
            <p>&nbsp;</p>
            <p>&nbsp;</p>
            <p>&nbsp;</p>
            <p>&nbsp;</p>
            <p>&nbsp;</p>
            <p>&nbsp;</p>
            <p>&nbsp;</p>
            <p>&nbsp;</p>
            <p>&nbsp;</p>
            <p>&nbsp;</p>
            <p>&nbsp;</p>
            <p>&nbsp;</p>
            <p>&nbsp;</p>
            <p>&nbsp;</p>
            <p>&nbsp;</p>
            <p>&nbsp;</p>
            <p>&nbsp;</p>
            <p>&nbsp;</p>
            <p>&nbsp;</p>
            <p>&nbsp;</p>
            <p>&nbsp;</p>
          </td>
        </tr>
      </table>
    </td>
  </tr>
  <tr> 
    <td colspan="2" height="20" class="footer"> 
      <%@ include file = "../main/footer.jsp" %>
    </td>
  </tr>
</table>
</body>
<!-- #EndTemplate --></html>
<!-- endif of "has access" condition -->
<%}%>