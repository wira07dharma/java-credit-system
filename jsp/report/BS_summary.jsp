...
private static final String strTitleReporPDF[][] = {
	{"BALANCE SHEET","Per","dalam jutaan Rp."},
	{"BALANCE SHEET","As of","in IDR Mio"}
};
...
public static final String listTitle[] = {
	"Neraca Rekap","Balance Sheet Summary"
};
 ...
 */
private static String getAccountName(Neraca objNeraca, int language){
	String accName = "";
		if(objNeraca != null){
			try{
				if(language == com.dimata.util.lang.I_Language.LANGUAGE_FOREIGN)
					accName = objNeraca.getAccountNameEnglish();
				else
					accName = objNeraca.getNamaPerkiraan();	
			}catch(Exception e){
 ...
 */
private static Vector getChild(Neraca objNeraca, int language, double prevAmount, double currAmount, double dValueReport){
	...
	Vector vTempPDF = new Vector(1,1);
		if(objNeraca != null){
			try{
				...
				rowx.add("<b>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;"+objNeraca.getAccNumber()+"&nbsp;"+getAccountName(objNeraca, language)+"</b>");
				...
				rowR.add("        "+objNeraca.getAccNumber()+" "+getAccountName(objNeraca, language));
				...
				rowR.add(formatNumberPDF(objNeraca.getPrevValue() == 0? 0 : (objNeraca.getValue() - objNeraca.getPrevValue()) / (objNeraca.getPrevValue() < 0? (objNeraca.getPrevValue() * -1) : objNeraca.getPrevValue()) * 100));
				vTempPDF.add(rowR);
	...
	double totCurrAmount = 0.0;
		try{
			for(int i = 0; i < vDetail.size(); i++){
				...
				Vector listChild = (Vector)getChild(objNeraca, language, prevAmount, currAmount, dValueReport);
...
private Vector drawBalanceSheet(Vector vData, int language, String strPrevYear, String strCurrYear, double dValueReport){
	...
	ctrlist.addHeader(strHeader[language][3],"15%","0","0");
								...
								try{
											try{
												for(int p = 0; p < rowLE.size(); p++){
													...
													lstData.add(rowx);
												}// End for(int p = 0; p < rowLE.size(); p++)
											}catch(Exception e){
											...
											try{
												for(int al = 0; al < rowLEPDF.size(); al++){
													...
													vTempDataPDFLE.add(rowR);
												}// End for(int al = 0; al < rowLEPDF.size(); al++)
												...
												if(rowSubTotalLE != null && rowSubTotalLE.size() > 0){
													try{
														for(int r = 0; r < rowSubTotalLE.size(); r++){
															...
															lstData.add(rowx);
														}// End for(int r = 0; r < rowSubTotalLE.size(); r++)
													}catch(Exception e){
														System.out.println("Exception on loop rowSubTotalLE :::: "+e.toString());
													}// End Try catch
												}// End if(rowSubTotalLE != null && rowSubTotalLE.size() > 0)
												...
												if(rowSubTotalLEPDF != null && rowSubTotalLEPDF.size() > 0){
													try{
														for(int am = 0; am < rowSubTotalLEPDF.size(); am++){
															...
															vTempDataPDFLE.add(rowR);
														}// End for(int am = 0; am < rowSubTotalLEPDF.size(); am++)
														//vTempDetailPDFLE.add(vTempSubTotalPDFLE);	
													}catch(Exception e){
														System.out.println("Exception on loop rowSubTotalLEPDF ::: "+e.toString());
													}// End try catch											
												}// End if(rowSubTotalLEPDF != null && rowSubTotalLEPDF.size() > 0)
										 }// End if(vTempSubTotalLE != null && vTempSubTotalLE.size() > 0)
										 ...
										 Vector vSpace = (Vector)getSpace();
											if(vSpace != null && vSpace.size() > 0){
												for(int w = 0; w < vSpace.size(); w++){
													...
													lstData.add(rSpace);
												}// End for(int w = 0; w < vSpace.size(); w++)
											}// End if(vSpace != null && vSpace.size() > 0)
										} //End if(vData.get(m+1) != null)				
								}catch(Exception e){
...
<%
    ...
    if(iCommand==Command.LIST)
        list = SessNeraca.listSummaryBalanceSheet(periodId,oidDepartment);
	...
	if(session.getValue("BS_SUMMARY")!=null){   
		session.removeValue("BS_SUMMARY");
	}
	session.putValue("BS_SUMMARY",vecSess);
...
function report(){
    ...
    //if(val!=0){
        ...
        document.frmTrialBalance.action = "BS_summary.jsp";
...
function cmdBack(){
	...
	document.frmTrialBalance.action = "BS_summary.jsp#up";
	document.frmTrialBalance.submit();
}
...
function printXLS(){	 
		var linkPage = "<%=reportroot%>report.neraca.BalanceSheetSummaryXls";       
		window.open(linkPage);  				
}
          ...
          <td valign="top"><!-- #BeginEditable "content" -->
            ...
            <input type="hidden" name="command" value="<%=iCommand%>">
              ...
              <table width="100%" border="0" height="94">
                ...
                <tr>
                  <td colspan="3" height="2">&nbsp;&nbsp;<b><%=masterTitle[SESS_LANGUAGE]%> : <%=strTitleReporPDF[SESS_LANGUAGE][2]%></b></td>
                </tr>
                <tr>
