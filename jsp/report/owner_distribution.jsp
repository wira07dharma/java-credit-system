...
function openDistributionGL(bus_center_id, period_id, acc_id){		
		window.open("odr_gl.jsp?bus_center_id="+bus_center_id+"&period_id="+period_id+"&acc_id="+acc_id+"&command="+<%=Command.LIST%>);  				      
    }

function openDistributionDetail(bus_center_id, period_id, acc_id){		
		window.open("odr_detail.jsp?bus_center_id="+bus_center_id+"&period_id="+period_id+"&acc_id="+acc_id+"&command="+<%=Command.LIST%>);  				      
    }

function openDistributionGLByGroup(bus_center_id, period_id, accGroup){		
		window.open("odr_gl.jsp?bus_center_id="+bus_center_id+"&period_id="+period_id+"&acc_group="+accGroup+"&command="+<%=Command.LIST%>);  				      
    }
			...
			vBisnisCenterKey.add(objBCenter.getBussCenterName());
                ...
                <tr>
				  ...
				  <table class="listgen" width="100%" border="0" cellspacing="2" cellpadding="2">
                    ...
                    %>
                    <tr>
                      <td class="listgensell">&nbsp; <% if(item.getAccount().getPostable()==PstPerkiraan.ACC_POSTED){%>
                          <a href="javascript:openDistributionGL('<%=busCenterId%>','<%=periodId%>','<%=item.getAccount().getOID()%>')"> <%=item.getAccount().getNoPerkiraan()%> </a>
                                  <%} else {%> <%=item.getAccount().getNoPerkiraan()%> <%}%></td>
                      <td class="listgensell">&nbsp;<% if(item.getAccount().getPostable()==PstPerkiraan.ACC_POSTED){%>
                          <a href="javascript:openDistributionDetail('<%=busCenterId%>','<%=periodId%>','<%=item.getAccount().getOID()%>')"> <%=item.getAccount().getNama()%> </a>
                                  <%} else {%> <%=item.getAccount().getNama()%> <%}%></td>
                    ...
                    %>
                    <tr>
                      <td class="<%=(idx>0?"listgensell":"listgentitle2")%>">&nbsp;
                      <% if(item.getAccount().getPostable()==PstPerkiraan.ACC_POSTED){%>
                          <a href="javascript:openDistributionGL('<%=busCenterId%>','<%=periodId%>','<%=item.getAccount().getOID()%>')"> <%=item.getAccount().getNoPerkiraan()%> </a>
                                  <%} else {%> <%=item.getAccount().getNoPerkiraan()%> <%}%>
                      </td>
                    ...
                    <tr>
                      ...
                      <% if(item.getAccount().getPostable()==PstPerkiraan.ACC_POSTED){%>
                          <a href="javascript:openDistributionGL('<%=busCenterId%>','<%=periodId%>','<%=item.getAccount().getOID()%>')"> <%=item.getAccount().getNoPerkiraan()%> </a>
                                  <%} else {%> 
                                   <a href="javascript:openDistributionGLByGroup('<%=busCenterId%>','<%=periodId%>','<%=OwnerDistributionReport.GROUP_ACC_COMMON_AREA%>')"> <%=item.getAccount().getNoPerkiraan()%> </a>
                                  <%} %>
                      </td>
                      <td class="<%=(idx>0?"listgensell":"listgentitle2")%>">&nbsp;<%=item.getAccount().getNama()%></td>
                      <td class="<%=(idx>0?"listgensell":"listgentitle2")%>">&nbsp;</td>
                    ...
                    <tr>
                      ...
                      <td class="listgentitle2">&nbsp;
                          <%
                           OwnDisRepAccount bfto = own.getBalanceFundToOwner();
                          %>
                      </td>
                      <td align="right" valign="middle" class="listgentitle2">&nbsp;<%=Formater.formatNumber(bfto.getPrev2PeriodSaldo(),"###,###.##")%></td>
                      <td align="right" valign="middle" class="listgentitle2">&nbsp;<%=Formater.formatNumber(bfto.getPrev1PeriodSaldo(),"###,###.##")%></td>
                      <td align="right" valign="middle" class="listgentitle2">&nbsp;<%=Formater.formatNumber(bfto.getThisPeriodSaldo(),"###,###.##")%></td>
                      <td class="listgentitle2">&nbsp;</td>
                    </tr>
                    <tr>
                      <td class="listgensell">&nbsp;</td>
                    ...
                    <tr>
                      <td class="listgensell">&nbsp;</td>
                      <td class="listgensell">&nbsp;40% of Total Three months operational</td>
                      <%
