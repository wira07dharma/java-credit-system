	...
	for(int i=0; i<objectClass.size(); i++) {
		 ...
		 if(index==i && (iCommand==Command.EDIT || iCommand==Command.ASK)){
			...
			rowx.add("<input type=\"text\" name=\""+frmTicketMaster.fieldNames[frmTicketMaster.FRM_DESCRIPTION] +"\"  class=\"elemenForm\" value=\""+objTicketMaster.getDescription()+"\" size=\"70\">"+
					 "<input type=\"hidden\" name=\""+frmTicketMaster.fieldNames[frmTicketMaster.FRM_TYPE] +"\"  value=\""+PstTicketMaster.MASTER_ROUTE+"\">"
					);
		}else{
			rowx.add(""+(i+1+iStart));
...
String whereClause = PstTicketMaster.fieldNames[PstTicketMaster.FLD_TYPE]+" = "+PstTicketMaster.MASTER_ROUTE;
...
function cmdAdd(){
	...
	document.frmcontactclass.action="ticket_master_route.jsp";
