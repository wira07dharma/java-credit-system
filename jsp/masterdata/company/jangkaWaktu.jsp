...
<%@page import="com.dimata.sedana.form.masterdata.FrmJangkaWaktu"%>
<%@page import="com.dimata.sedana.form.masterdata.CtrlJangkaWaktu"%>
...
<%!
    ...
    public static final String textListHeader[][]
            = {
                {"No", "Jangka Waktu", "Nama Loket", "Jenis Loket", "Lokasi"},
                {"No", "Range Time", "Locket Name", "Locket Type", "Location"}
            };

    public String drawList(int iCommand, FrmJangkaWaktu frmObject, JangkaWaktu objEntity, Vector objectClass, long jenisSimpananId, int languange, int vectorNumber, int start) {
        ...
        for (int i = 0; i < objectClass.size(); i++) {
            ...
            JangkaWaktu jw = (JangkaWaktu) objectClass.get(i);
            rowx = new Vector();
            if (jenisSimpananId == jw.getOID()) {
                index = i;
            }
            if (index == i && (iCommand == Command.EDIT || iCommand == Command.ASK)) {        
                rowx.add("<input type=\"hidden\" name=\"" + frmObject.fieldNames[FrmJangkaWaktu.FRM_FIELD_JANGKA_WAKTU_ID] + "\" value=\"" + jw.getOID() + "\" class=\"\">" + indexNumber + ".");
                rowx.add("<input type=\"text\" name=\"" + frmObject.fieldNames[FrmJangkaWaktu.FRM_FIELD_JANGKA_WAKTU] + "\" value=\"" +jw.getJangkaWaktu()+ "\" class=\"\">");
                rowx.add("-");
            } else {
                rowx.add("" + indexNumber + ".");
                rowx.add(""+jw.getJangkaWaktu());
                rowx.add(String.valueOf("<center><a class=\"btn btn-warning btn-xs\" href=\"javascript:cmdEdit('" + jw.getOID() + "')\">Edit</a> <a class=\"btn btn-success btn-xs\" href=\"javascript:cmdFormula('" + jw.getOID() + "')\">Rumus</a></center>"));
            }
            lstData.add(rowx);
        ...
        if (iCommand == Command.ADD || (iCommand == Command.SAVE && frmObject.errorSize() > 0)) {
            rowx.add("<input type=\"hidden\" name=\"" + frmObject.fieldNames[FrmJangkaWaktu.FRM_FIELD_JANGKA_WAKTU_ID] + "\" value=\"" + objEntity.getOID() + "\" class=\"\">" + indexNumber + "");
            rowx.add("<input type=\"text\" name=\"" + frmObject.fieldNames[FrmJangkaWaktu.FRM_FIELD_JANGKA_WAKTU] + "\" value=\"" +objEntity.getJangkaWaktu()+ "\" class=\"\">");
            rowx.add("-");
        }
        lstData.add(rowx);
...
<%
    ...
    String whereClause = "" + PstJangkaWaktu.fieldNames[PstJangkaWaktu.FLD_JANGKA_WAKTU] + " LIKE '%" + keyword + "%'";;
    String orderClause = "" + PstJangkaWaktu.fieldNames[PstJangkaWaktu.FLD_JANGKA_WAKTU];
    ...
    CtrlJangkaWaktu ctrlJangkaWaktu = new CtrlJangkaWaktu(request);
    iErrCode = ctrlJangkaWaktu.action(iCommand, oidJangkaWaktu);
    FrmJangkaWaktu frmJangkaWaktu = ctrlJangkaWaktu.getForm();
    JangkaWaktu jangkaWaktu = ctrlJangkaWaktu.getJangkaWaktu();
    msgString = ctrlJangkaWaktu.getMessage();

    Vector listJangkaWaktu = new Vector(1, 1);
    int vectSize = PstJangkaWaktu.getCount(whereClause);
    if (iCommand == Command.FIRST || iCommand == Command.PREV || iCommand == Command.NEXT || iCommand == Command.LAST) {
        start = ctrlJangkaWaktu.actionList(iCommand, start, vectSize, recordToGet);
    }

    listJangkaWaktu = PstJangkaWaktu.list(start, recordToGet, whereClause, orderClause);
...
<html><!-- #BeginTemplate "/Templates/main-menu-left-frames.dwt" --><!-- DW6 -->
    <head>
        ...
        <script language="JavaScript">
            function cmdAdd() {
                ...
                document.frmJangkaWaktu.action = "jangkaWaktu.jsp";
            ...
            function cmdFormula(oidJangkaWaktu) {
                document.frmJangkaWaktu.hidden_jangka_waktu_id.value = oidJangkaWaktu;
                document.frmJangkaWaktu.command.value = "<%=Command.EDIT%>";
                document.frmJangkaWaktu.prev_command.value = "<%=prevCommand%>";
                document.frmJangkaWaktu.action = "jangkaWaktuFormula.jsp";
                document.frmJangkaWaktu.submit();
            }
    ...
    <body style="background-color: #eaf3df;">
        ...
        <section class="content">
            <div class="box box-success">
                ...
                <div class="box-body">
                    <form name="cari" method="get" action="jangkaWaktu.jsp" class="form-inline">
                    ...
                    <form name="frmJangkaWaktu" method ="post" action="">
                        ...
                        <%= drawList(iCommand, frmJangkaWaktu, jangkaWaktu, listJangkaWaktu, oidJangkaWaktu, SESS_LANGUAGE, vectSize, start)%>
    ...
    %>
    <script language="javascript">
        document.frmJangkaWaktu.<%=FrmJangkaWaktu.fieldNames[FrmJangkaWaktu.FRM_FIELD_JANGKA_WAKTU]%>.focus();
    </script>
    <%
        }
    %>
