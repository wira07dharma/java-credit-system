<%-- 
    Document   : report_kredit_per_shift
    Created on : Nov 6, 2017, 8:54:42 AM
    Author     : Dimata 007
--%>
...
<% int  appObjCode =  AppObjInfo.composeObjCode(AppObjInfo.G1_SEDANA, AppObjInfo.G2_REPORT, AppObjInfo.OBJ_KREDIT_PER_SHIFT); %>
...
<%  //  
    ...
    Vector<Transaksi> listKreditShift = SessReportKredit.listKreditPerShift(""
    ...
    <body style="background-color: #eaf3df;">
        <div class="main-page">
            ...
            <section class="content">
                    ...
                    <form id="form_cari" class="form-horizontal" method="post" action="?command=<%=Command.LIST%>">                            
                        ...
                        <div class="box-footer" style="border-color: lightgray">
                            <div class="pull-right">
                                ...
                                <% if (iCommand == Command.LIST && !listKreditShift.isEmpty()) {%>
                                &nbsp;
                ...
                <%if (listKreditShift.isEmpty()) {%>

                <div class="box box-success">
                    <div class="box-header text-bold">
                        Data transaksi kredit tidak ditemukan.
                    </div>
                </div>

                <%}%>

                <%for (int i = 0; i < listKreditShift.size(); i++) {%>

                <div class="box box-success">

                    <div class="box-header with-border" style="border-color: lightgray">
                        <h3 class="box-title">Tanggal Opening &nbsp;:&nbsp; <a><%=Formater.formatDate(listKreditShift.get(i).getTanggalTransaksi(), "dd MMMM yyyy")%></a></h3>
                    ...
                    <div class="box-body">
                        ...
                        <%
                            String textDate = Formater.formatDate(listKreditShift.get(i).getTanggalTransaksi(), "yyyy-MM-dd");
                            ...
                            Vector listLoketShift = SessReportKredit.listKreditPerShift(""
                            ...
                            for (int j = 0; j < listLoketShift.size(); j++) {
                                ...
                                l = PstLocation.fetchExc(ml.getLocationId());
                                AppUser user = PstAppUser.fetch(ct.getAppUserId());
                                Vector listTransaksi = SessReportKredit.listKreditPerShift(""
                                        + " DATE(t." + PstTransaksi.fieldNames[PstTransaksi.FLD_TANGGAL_TRANSAKSI] + ") = '" + textDate + "'"
                        ...
                        <table class="table table-bordered">
                            ...
                            <%
                                ...
                                for (int k = 0; k < listTransaksi.size(); k++) {
                                    Transaksi transaksi = (Transaksi) listTransaksi.get(k);
                                    Pinjaman p = PstPinjaman.fetchExc(transaksi.getPinjamanId());
                                    ...
                                    } else if (transaksi.getUsecaseType() == Transaksi.USECASE_TYPE_KREDIT_ANGSURAN_PAYMENT) {
                                        nilaiTransaksi = SessReportKredit.getTotalAngsuranDibayarByTransaksi(transaksi.getOID());
                                    }
                                    totalTransaksi += nilaiTransaksi;
                                    CashTeller ctl = PstCashTeller.fetchExc(transaksi.getTellerShiftId());
                            %>
                            ...
                            <tr>
                                <td colspan="5" style="background-color: #eaf3df" class="text-right"><b>Total Transaksi &nbsp;:&nbsp; Rp <span class="money"><%=totalTransaksi%></span></b></td>
                            </tr>
                        </table>
        ...
        <div style="size: A5;" class="container">
            <div style="overflow: auto">
                ...
                <div style="width: 50%; float: right; text-align: right">
                    <span style="width: 100%; display: inline-block; font-weight: 400; font-size: 20px;">LAPORAN KREDIT PER SHIFT</span>
