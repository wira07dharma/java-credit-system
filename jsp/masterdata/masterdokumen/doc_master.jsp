<%-- 
    Document   : doc_master
    Created on : 19-Dec-2017, 11:47:57
    Author     : Gunadi
--%>
...
<%!
    public static final String textListTitle[]
            = {
                "Template Dokumen", "Document Template"
            };

    public static final String textListHeader[][]
            = {
                {"No", "Jenis", "Nama Template", "Format Nomor Dokumen", "Deskripsi", "Aksi"},
                {"No", "Type", "Template Name", "Document Number Format", "Description", "Action"}
            };

    public String drawList(int iCommand, FrmDocMaster frmObject, DocMaster objEntity, Vector objectClass, long docMasterId, int languange, int vectorNumber, int start) {
        ...
        Vector docType_val = new Vector(1, 1);
        ...
        if (listDocType != null && listDocType.size() > 0) {
            for (int x = 0; x < listDocType.size(); x++) {
                DocType docType = (DocType) listDocType.get(x);
                docType_key.add(docType.getType_name());
                docType_val.add(String.valueOf(docType.getOID()));
            }
        }
        ...
        for (int i = 0; i < objectClass.size(); i++) {
            ...
            if (index == i && (iCommand == Command.EDIT || iCommand == Command.ASK)) {
                ...
                rowx.add("<center><input type=\"hidden\" name=\"" + frmObject.fieldNames[FrmDocMaster.FRM_FIELD_DOC_MASTER_ID] + "\" value=\"" + docMaster.getOID() + "\" class=\"formElemen\">" + indexNumber + ".</center>");
                rowx.add(ControlCombo.draw(frmObject.fieldNames[FrmDocMaster.FRM_FIELD_DOC_TYPE_ID], null, "" + docMaster.getDoc_type_id(), docType_val, docType_key, ""));
                rowx.add("<input type=\"text\" name=\"" + frmObject.fieldNames[FrmDocMaster.FRM_FIELD_TITLE] + "\" value=\"" + docMaster.getDoc_title() + "\" class=\"formElemen\">");
                rowx.add("<input type=\"text\" name=\"" + frmObject.fieldNames[FrmDocMaster.FRM_FIELD_DOC_NUMBER] + "\" value=\"" + docMaster.getDoc_number() + "\" class=\"formElemen\">");
                rowx.add("<textarea cols=\"35\" rows=\"2\" type=\"text\" name=\"" + frmObject.fieldNames[FrmDocMaster.FRM_FIELD_DESCRIPTION] + "\" value=\"" + docMaster.getDescription() + "\" class=\"formElemen\">" + docMaster.getDescription() + "</textarea>");

            } else {

                DocType docType = new DocType();
                if (docMaster.getDoc_type_id() != 0) {
                    try {
                ...
                rowx.add(String.valueOf(docMaster.getDoc_title()));
                rowx.add(String.valueOf(docMaster.getDoc_number()));
                rowx.add(String.valueOf(docMaster.getDescription()));

            }
            rowx.add(String.valueOf("<center>"
                    + "<a class=\"btn-warning btn-sm\" style=\"color:#FFF\" href=\"javascript:cmdEdit('" + docMaster.getOID() + "')\">Edit</a>"
                    + " <a class=\"btn-info btn-sm\" style=\"color:#FFF\" href=\"javascript:cmdTemplate('" + docMaster.getOID() + "')\">Template</a>"
                    + "</center>"));
        ...
        if (iCommand == Command.ADD || (iCommand == Command.SAVE && frmObject.errorSize() > 0)) {
            rowx.add("<center><input type=\"hidden\" name=\"" + frmObject.fieldNames[FrmDocMaster.FRM_FIELD_DOC_MASTER_ID] + "\" value=\"" + objEntity.getOID() + "\" class=\"formElemen\">" + (indexNumber + 1) + ".</center>");
            rowx.add(ControlCombo.draw(frmObject.fieldNames[FrmDocMaster.FRM_FIELD_DOC_TYPE_ID], null, "" + objEntity.getDoc_type_id(), docType_val, docType_key, ""));
            rowx.add("<input type=\"text\" name=\"" + frmObject.fieldNames[FrmDocMaster.FRM_FIELD_TITLE] + "\" value=\"" + objEntity.getDoc_title() + "\" class=\"formElemen\">");
            rowx.add("<input type=\"text\" name=\"" + frmObject.fieldNames[FrmDocMaster.FRM_FIELD_DOC_NUMBER] + "\" value=\"" + objEntity.getDoc_number() + "\" class=\"formElemen\">");
            rowx.add("<textarea cols=\"35\" rows=\"2\" type=\"text\" name=\"" + frmObject.fieldNames[FrmDocMaster.FRM_FIELD_DESCRIPTION] + "\" value=\"" + objEntity.getDescription() + "\" class=\"formElemen\">" + objEntity.getDescription() + "</textarea>");
            rowx.add("-");
        }
        lstData.add(rowx);
...
<%
    ...
    String whereClause = "" + PstDocMaster.fieldNames[PstDocMaster.FLD_DOC_TITLE] + " LIKE '%" + keyword + "%'"
            + " OR " + PstDocMaster.fieldNames[PstDocMaster.FLD_DESCRIPTION] + " LIKE '%" + keyword + "%'"
            + " OR " + PstDocMaster.fieldNames[PstDocMaster.FLD_DOC_CODE] + " LIKE '%" + keyword + "%'"
            + " OR " + PstDocMaster.fieldNames[PstDocMaster.FLD_DOC_NUMBER] + " LIKE '%" + keyword + "%'"
            + "";
    String orderClause = "" + PstDocMaster.fieldNames[PstDocMaster.FLD_DOC_TITLE];
...
<html><!-- #BeginTemplate "/Templates/main-menu-left-frames.dwt" --><!-- DW6 -->
    <head>
        ...
        <script language="JavaScript">
            function cmdAdd() {
                document.frmDocMaster.hidden_docmaster_id.value = "0";
            ...
            function cmdAsk(oidDocMaster) {
                document.frmDocMaster.hidden_docmaster_id.value = oidDocMaster;
            ...
            function cmdTemplate(oidDocMaster) {
                document.frmDocMaster.hidden_docmaster_id.value = oidDocMaster;
                document.frmDocMaster.command.value = "<%=Command.EDIT%>";
                document.frmDocMaster.prev_command.value = "<%=prevCommand%>";
                document.frmDocMaster.action = "doc_template.jsp";
                document.frmDocMaster.submit();
            }
        </script>
        <!-- #EndEditable -->
    ...
    <body style="background-color: #eaf3df;">
        <section class="content-header">
            <h1>Template Dokumen<small></small></h1>
            <ol class="breadcrumb">
        ...
        <section class="content">
            <div class="box box-success">
                <div class="box-header with-border">
                    <h3 class="box-title">Daftar Template Dokumen</h3>
                </div>
                <div class="box-body">
                    <form name="cari" method="get" action="doc_master.jsp">
                    ...
                    <form name="frmDocMaster" method ="post" action="">
                        ...
                        <%= drawList(iCommand, frmDocMaster, docMaster, listDocMaster, oidDocMaster, SESS_LANGUAGE, vectSize, start)%>
            ...
            <div class="box box-danger">
                ...
                <div class="box-body">
                    <%
                        ...
                        arr.put(SessHistory.document[SessHistory.DOC_MASTER_DOKUMEN]);
                        obj.put("doc", arr);
                        obj.put("time", "");
            ...
            <a href="doc_master.jsp?show_history=<%= (showHistory == 0) ? "1" : "0"%>"><%= (showHistory == 0) ? "Lihat Riwayat Perubahan" : "Sembunyikan Riwayat Perubahan"%></a>
    ...
    %>
    <script language="javascript">
        document.frmDocMaster.<%=FrmDocMaster.fieldNames[FrmDocMaster.FRM_FIELD_TITLE]%>.focus();
    </script>
    <%
        }
    %>
