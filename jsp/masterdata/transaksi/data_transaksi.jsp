...
<%
  ...
  if (open.size() < 1) {
    String redirectUrl = approot + "/tellershift/open_teller_shift.jsp?redir=" + approot + "/masterdata/transaksi/data_transaksi.jsp";
...
<%!
  ...
  public static final String systemText[][] = {
         {"Lihat"}, {"View"}
  };
  ...
  public String drawListDataTabungan(int language, Vector objectClass, long oid, Vector listSaldo, SearchTabungan parameter) {
    ...
    ctrlist.dataFormat(strTitle[language][5], "1%", "center", "center");
    ...
    for (int i = 0; i < objectClass.size(); i++) {
      ...
      rowx.add(String.valueOf("<a class=\"btn-success btn-sm\" style=\"color:#FFF\" href=\"javascript:cmdEdit('" + trx.getOID() + "')\">"+systemText[language][0]+"</a>"));
...
<%
  ...
  int recordToGet = 10;
  String whereClause = PstTransaksi.fieldNames[PstTransaksi.FLD_TIPE_ARUS_KAS]+" = '" + Transaksi.TIPE_ARUS_KAS_BERKURANG + "'"
  ...
  <head>
    ...
    <script language="JavaScript">
      ...
      function addNew() {
        ...
        document.<%=FrmDataTabungan.FRM_TABUNGAN_NAME%>.action = "<%=approot%>/Penarikan";
                  ...
                  <div class="form-group">
                    ...
                    <div class="col-sm-9">
                      ...
                      <div style="padding-left: 0;" class="col-sm-6">
                        <input name="<%=FrmSearchTabungan.FRM_FIELD_NASABAH_NO_REKENING %>" value="<%=searchTabungan.getNoRekening() %>" type="text" data-onselect="setTabungan" data-action="<%=approot + "/Penarikan"%>" data-for="<%=AjaxSetoran.SEARCH_NO_TABUNGAN%>" required="" class="form-control autocomplete" id="search-personRekening" placeholder="No. Tabungan" style="margin-bottom: 5px;">
                      </div>
                      <div style="padding-right: 0;" class="col-sm-6">
                        <input name="<%=FrmSearchTabungan.FRM_FIELD_NASABAH_NAMA %>" value="<%=searchTabungan.getNama() %>" type="text" data-onselect="setMemberName" data-action="<%=approot + "/Penarikan"%>" data-for="<%=AjaxSetoran.SEARCH_MEMBER_NAME%>" required="" class="form-control autocomplete" id="search-personName" placeholder="Nama">
                      </div>
                    </div>
                  </div>
                  <div class="form-group">
