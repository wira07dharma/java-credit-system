...
public String drawList(int language, Vector objectClass, int start){
	...
	if(objectClass!=null && objectClass.size()>0){
		...
		ctrlist.setLinkPrefix("javascript:cmdEditXXX('");
		ctrlist.setLinkSufix("')");
		ctrlist.reset();
		...
		for(int i=0; i<objectClass.size(); i++){
            ...
            if(pos==1){
               psn = "Postable";
               rowx.add("<a href=\"javascript:cmdEdit('"+perkiraan.getOID()+"', '"+strAccName+"')\">"+perkiraan.getNoPerkiraan()+"</a>");
            }else{
              psn = "Header";
			...
			rowx.add(psn);
			 ...
			 lstData.add(rowx);
			 //lstLinkData.add(perkiraan.getOID()+"','"+strAccName+"'");
		}
...
String pageTitle = iCommand == Command.LIST?"LIST ACCOUNT" : "SEARCH ACCOUNT";  
...
if(iCommand==Command.FIRST || iCommand==Command.PREV || iCommand==Command.NEXT || iCommand==Command.LAST || iCommand == Command.NONE){		
...
function cmdEdit(oid,name){
	self.opener.document.forms.<%=FrmBussCenterBudget.FRM_BUSSINESS_CENTER_BUDGET%>.<%=FrmBussCenterBudget.fieldNames[FrmBussCenterBudget.FRM_ID_PERKIRAAN]%>.value = oid;
	self.opener.document.forms.<%=FrmBussCenterBudget.FRM_BUSSINESS_CENTER_BUDGET%>.<%=FrmBussCenterBudget.fieldNames[FrmBussCenterBudget.FRM_ID_PERKIRAAN]%>_TEXT.value = name;
	self.close();
}
...
<BODY onBlur="self.focus()" bgcolor="#FFFFFF" class="menuleft">
  <script language="JavaScript">
	...
	<form name="frmaccountsearch" method="post" action="">
      ...
      <td height="20" class="title" align="center"><b><font size="3"><%=pageTitle%></font></b></td>
        </tr>
		<tr>
		  <td>
		  <%
      ...
      <td>
        <table width="100%" border="0" cellspacing="1" cellpadding="1">
		  ...
		  <tr><td>&nbsp;</td></tr>
          <tr>
            <td colspan="3" align="right"><font face="Verdana, Arial, Helvetica, sans-serif" size="2">
				  <%if(iCommand == Command.LIST || cmd == Command.LIST){%>
