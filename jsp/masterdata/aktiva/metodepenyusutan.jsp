...
{
	{"Metode Penyusutan","Harus diisi"},
	{"Depreciation Method","Required"}
};

public static final String textListHeader[][] =
{
	{"Kode","Metode Penyusutan","Tipe"},
	{"Code","Depreciation Method","Type"}
};
...

{
	...
	for (int i = 0; i < objectClass.size(); i++){
		Aktiva aktiva = (Aktiva)objectClass.get(i);
...
long oidAktiva = FRMQueryString.requestLong(request, "hidden_standart_rate_id");

boolean privManageData = true; 
int recordToGet = 10;
String msgString = "";
int iErrCode = FRMMessage.NONE;
String whereClause = PstAktiva.fieldNames[PstAktiva.FLD_TYPE]+"="+PstAktiva.TYPE_AKTIVA_METODE_PENYUSUTAN;
...
CtrlAktiva ctrlAktiva = new CtrlAktiva(request);
Vector listAktiva = new Vector(1,1);

iErrCode = ctrlAktiva.action(iCommand , oidAktiva);
FrmAktiva frmAktiva = ctrlAktiva.getForm();

int vectSize = PstAktiva.getCount(whereClause);
Aktiva aktiva = ctrlAktiva.getAktiva();
msgString =  ctrlAktiva.getMessage();
...
//	start = PstAktiva.findLimitStart(aktiva.getOID(),recordToGet, whereClause);

if((iCommand == Command.FIRST || iCommand == Command.PREV )||
  (iCommand == Command.NEXT || iCommand == Command.LAST)){
		start = ctrlAktiva.actionList(iCommand, start, vectSize, recordToGet);
} 

listAktiva = PstAktiva.list(start,recordToGet, whereClause , orderClause);

if (listAktiva.size() < 1 && start > 0)
{
...
function cmdAdd(){
	...
	document.frmstandartrate.action="metodepenyusutan.jsp";
	document.frmstandartrate.submit();
}

function cmdAsk(oidAktiva){
	document.frmstandartrate.hidden_standart_rate_id.value=oidAktiva;
        ...
        <tr> 
          <td valign="top"><!-- #BeginEditable "content" --> 
            <form name="frmstandartrate" method ="post" action="">
              ...
              <input type="hidden" name="hidden_standart_rate_id" value="<%=oidAktiva%>">
              <input type="hidden" name="<%=FrmAktiva.fieldNames[FrmAktiva.FRM_TYPE] %>" value="<%=PstAktiva.TYPE_AKTIVA_METODE_PENYUSUTAN%>">
              <table width="100%" border="0" cellspacing="0" cellpadding="0">
                <tr align="left" valign="top"> 
                  <td height="8"  colspan="3" valign="top"> 
                    <table width="100%" border="0" cellspacing="0" cellpadding="0">
                      ...
                      <tr align="left" valign="top"> 
                        <td height="22" valign="middle" colspan="3"> <%= drawList(listAktiva,oidAktiva,SESS_LANGUAGE,sUserDecimalSymbol,sUserDigitGroup)%> </td>
                      </tr>
                      <tr align="left" valign="top"> 
                ...
                <tr align="left" valign="top"> 
                  <td height="8" colspan="3"> 
                    <%if((iCommand ==Command.ADD)||(iCommand==Command.SAVE)&&(frmAktiva.errorSize()>0)||(iCommand==Command.EDIT)||(iCommand==Command.ASK)){%>
                    <table width="100%" border="0" cellspacing="2" cellpadding="0">
                      ...
                      <tr align="left" valign="top">
                        ...
                        <td height="21" colspan="2"> <%
                          ...
                          for(int k=0;k < PstAktiva.metodePenyusutanNames.length;k++){
                            vAktivaKey.add(""+k);
                            vAktivaVal.add(PstAktiva.metodePenyusutanNames[k]);
                          }
                          out.println(ControlCombo.draw(FrmAktiva.fieldNames[FrmAktiva.FRM_TYPE_METODE_PENYUSUTAN],"",null,""+aktiva.getTypeMetodePenyusutan(),vAktivaKey,vAktivaVal,""));%>
                      <tr align="left" valign="top">
                        <td height="21" valign="top" width="10%" nowrap>&nbsp;<%=textListHeader[SESS_LANGUAGE][0]%></td>
                        <td height="21" valign="top" width="1%">:</td>
                        <td height="21" colspan="2"> 
                          <input type="text" name="<%=frmAktiva.fieldNames[FrmAktiva.FRM_KODE] %>"  value="<%=aktiva.getKode()%>">
                          * <%= frmAktiva.getErrorMsg(FrmAktiva.FRM_KODE) %>
                      <tr align="left" valign="top">
                        <td height="21" valign="top" width="10%" nowrap>&nbsp;<%=textListHeader[SESS_LANGUAGE][1]%></td>
                        <td height="21" valign="top" width="1%">:</td>
                        <td height="21" colspan="2">
                          <input type="text" name="<%=frmAktiva.fieldNames[FrmAktiva.FRM_NAME] %>"  value="<%=aktiva.getNama()%>">
                          * <%= frmAktiva.getErrorMsg(FrmAktiva.FRM_NAME) %>
                      <tr align="left" valign="top" >
